const kbdUtil = (function () {
  ...
  function hasShortcutModifier(charModifier, currentModifiers) {
    ...
    let sum = 0;
    for (const k in currentModifiers) {
      if (mods[k]) {
        ++sum;
      }
    }
  ...
  function hasCharModifier(charModifier, currentModifiers) {
    ...
    for (let i = 0; i < charModifier.length; ++i) {
  ...
  function ModifierSync(charModifier) {
    ...
    function syncKeyEvent(evt, down) {
      const obj = getKeysym(evt);
      const keysym = obj ? obj.keysym : null;
    ...
    return {
      ...
      keydown(evt) { return syncKeyEvent(evt, true); },
      keyup(evt) { return syncKeyEvent(evt, false); },
      // Call this with a non-keyboard event (such as mouse events) to use its modifier state to synchronize anyway
      syncAny(evt) { return sync(evt); },

      // is a shortcut modifier down?
      hasShortcutModifier() { return hasShortcutModifier(charModifier, state); },
      // if a char modifier is down, return the keys it consists of, otherwise return null
      activeCharModifier() { return hasCharModifier(charModifier, state) ? charModifier : null; },
    };
  }
  ...
  function getKeysym(evt) {
    ...
    if (codepoint) {
      const res = keysyms.fromUnicode(substituteCodepoint(codepoint));
      if (res) {
        return res;
      }
    }
...
function KeyEventDecoder(modifierState, next) {
  ...
  function process(evt, type) {
    const result = { type };
    const keyId = kbdUtil.getKey(evt);
    if (keyId) {
      result.keyId = keyId;
    }

    const keysym = kbdUtil.getKeysym(evt);

    const hasModifier = modifierState.hasShortcutModifier() || !!modifierState.activeCharModifier();
    ...
    const isShift = evt.keyCode === 0x10 || evt.key === 'Shift';

    // Should we prevent the browser from handling the event?
    // Doing so on a keydown (in most browsers) prevents keypress from being generated
    // so only do that if we have to.
    const suppress = !isShift && (type !== 'keydown' || modifierState.hasShortcutModifier() || !!kbdUtil.nonCharacterKey(evt));

    // If a char modifier is down on a keydown, we need to insert a stall,
    // so VerifyCharModifier knows to wait and see if a keypress is comnig
    const stall = type === 'keydown' && modifierState.activeCharModifier() && !kbdUtil.nonCharacterKey(evt);

    // if a char modifier is pressed, get the keys it consists of (on Windows, AltGr is equivalent to Ctrl+Alt)
    const active = modifierState.activeCharModifier();

    // If we have a char modifier down, and we're able to determine a keysym reliably
    // then (a) we know to treat the modifier as a char modifier,
    // and (b) we'll have to "escape" the modifier to undo the modifier when sending the char.
    if (active && keysym) {
      let isCharModifier = false;
  ...
  return {
    ...
    releaseAll() { next({ type: 'releaseall' }); },
  };
}
...
function TrackKeyState(next) {
  ...
  return function (evt) {
    let last = state.length !== 0 ? state[state.length - 1] : null;
      ...
      case 'releaseall':
        for (var i = 0; i < state.length; ++i) {
          for (var key in state[i].keysyms) {
            const keysym = state[i].keysyms[key];
            next({ keyId: 0, keysym, type: 'keyup' });
          }
        }
        state = [];
    }
