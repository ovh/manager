...
Util.load_scripts = function (files) {
  const head = document.getElementsByTagName('head')[0]; let script;


  const ls = Util._loading_scripts; const
...
Util.getPosition = (function () {
  ...
  function scrollDist() {
    ...
    if (html.scrollTop && document.documentElement.scrollTop) { myScrollTop = html.scrollTop; } else if (html.scrollTop || document.documentElement.scrollTop) { myScrollTop = html.scrollTop + document.documentElement.scrollTop; } else if (document.body.scrollTop) { myScrollTop = document.body.scrollTop; } else { myScrollTop = 0; }

    // get the scrollLeft part
    if (html.scrollLeft && document.documentElement.scrollLeft) { myScrollLeft = html.scrollLeft; } else if (html.scrollLeft || document.documentElement.scrollLeft) { myScrollLeft = html.scrollLeft + document.documentElement.scrollLeft; } else if (document.body.scrollLeft) { myScrollLeft = document.body.scrollLeft; } else { myScrollLeft = 0; }

    return [myScrollLeft, myScrollTop];
  }

  return function (obj) {
    let curleft = 0; let curtop = 0; let scr = obj; let
      fixed = false;
...
Util.getEventPosition = function (e, obj, scale) {
  ...
  const realx = docX - pos.x;
  const realy = docY - pos.y;
  const x = Math.max(Math.min(realx, obj.width - 1), 0);
  const y = Math.max(Math.min(realy, obj.height - 1), 0);
  return {
    x: x / scale, y: y / scale, realx: realx / scale, realy: realy / scale,
  };
};
...
Util.Engine = {
  ...
  presto: (function () { return !(!window.opera); }()),
...
if (Util.Engine.webkit) {
  ...
  Util.Engine.webkit = (function (v) {
    const re = new RegExp('WebKit/([0-9\.]*) ');
