...
function Keyboard(defaults) {
  ...
  const keyDownList = []; // List of depressed keys
  ...
  const k = KeyEventDecoder(kbdUtil.ModifierSync(),
    VerifyCharModifier(
      TrackKeyState(
    ...
    if (e.touches || e.changedTouches) {
      ...
      if (down == 1) {
        if (doubleClickTimer == null) {
          ...
          const xs = lastTouchPos.x - pos.x;
          const ys = lastTouchPos.y - pos.y;
