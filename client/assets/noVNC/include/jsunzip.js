...
function JSUnzip() {
  ...
  this.open = function (data) {
    ...
    let endOfCentralDirectory = this.data.length - 22;
    while (endOfCentralDirectory >= 0 && this.getInt(endOfCentralDirectory, 4) != 0x06054b50) --endOfCentralDirectory;
    if (endOfCentralDirectory < 0) return { status: false, error: 'Invalid data' };
    if (this.getInt(endOfCentralDirectory + 4, 2) != 0 || this.getInt(endOfCentralDirectory + 6, 2) != 0) return { status: false, error: 'No multidisk support' };

    const entriesInThisDisk = this.getInt(endOfCentralDirectory + 8, 2);
    const centralDirectoryOffset = this.getInt(endOfCentralDirectory + 16, 4);
    const globalCommentLength = this.getInt(endOfCentralDirectory + 20, 2);
    this.comment = this.data.slice(endOfCentralDirectory + 22, endOfCentralDirectory + 22 + globalCommentLength);

    let fileOffset = centralDirectoryOffset;

    for (let i = 0; i < entriesInThisDisk; ++i) {
      ...
      const compressionMethod = this.getInt(fileOffset + 10, 2);
      if (compressionMethod != 0 && compressionMethod != 8) return { status: false, error: 'Unsupported compression method' };

      const lastModFileTime = this.getInt(fileOffset + 12, 2);
      const lastModFileDate = this.getInt(fileOffset + 14, 2);
      const lastModifiedDate = this.getDOSDate(lastModFileDate, lastModFileTime);

      const crc = this.getInt(fileOffset + 16, 4);
      // TODO: crc

      const compressedSize = this.getInt(fileOffset + 20, 4);
      const uncompressedSize = this.getInt(fileOffset + 24, 4);

      const fileNameLength = this.getInt(fileOffset + 28, 2);
      const extraFieldLength = this.getInt(fileOffset + 30, 2);
      const fileCommentLength = this.getInt(fileOffset + 32, 2);

      const relativeOffsetOfLocalHeader = this.getInt(fileOffset + 42, 4);

      const fileName = this.data.slice(fileOffset + 46, fileOffset + 46 + fileNameLength);
      const fileComment = this.data.slice(fileOffset + 46 + fileNameLength + extraFieldLength, fileOffset + 46 + fileNameLength + extraFieldLength + fileCommentLength);

      if (this.getInt(relativeOffsetOfLocalHeader + 0, 4) != 0x04034b50) return { status: false, error: 'Invalid data' };
      const localFileNameLength = this.getInt(relativeOffsetOfLocalHeader + 26, 2);
      const localExtraFieldLength = this.getInt(relativeOffsetOfLocalHeader + 28, 2);
      const localFileContent = relativeOffsetOfLocalHeader + 30 + localFileNameLength + localExtraFieldLength;
  ...
  this.read = function (fileName) {
    const fileInfo = this.files[fileName];
    if (fileInfo) {
      if (fileInfo.compressionMethod == 8) {
        ...
        const result = tinf.uncompress(this.data, fileInfo.localFileContent);
 ...
 * ---------------------- */
  ...
  this.read_bits = function (d, num, base) {
    ...
    const ret = read_bits_direct(d.source, d.bitcount, d.tag, d.sourceIndex, num);
    d.bitcount = ret[0];
    d.tag = ret[1];
    d.sourceIndex = ret[2];
    return ret[3] + base;
  ...
  this.decode_symbol = function (d, t) {
    while (d.bitcount < 16) {
      d.tag |= (d.source[d.sourceIndex++] & 0xff) << d.bitcount;
      d.bitcount += 8;
    }
  ...
  this.decode_trees = function (d, lt, dt) {
    const code_tree = new this.TREE();
    const lengths = new Array(288 + 32);
    ...
    for (i = 0; i < hclen; ++i) {
      ...
      const clen = this.read_bits(d, 3, 0);
    ...
    for (num = 0; num < hlit + hdist;) {
      const sym = this.decode_symbol(d, code_tree);
 ...
 * ----------------------------- */
  ...
  this.inflate_block_data = function (d, lt, dt) {
    // js optimization.
    const ddest = d.dest;
    let ddestlength = ddest.length;

    while (1) {
      let sym = this.decode_symbol(d, lt);
      ...
      if (sym < 256) {
        ...
        if (offs < 0) throw (`Invalid zlib offset ${offs}`);
  ...
  this.inflate_uncompressed_block = function (d) {
    ...
    if (d.bitcount > 7) {
      const overflow = Math.floor(d.bitcount / 8);
