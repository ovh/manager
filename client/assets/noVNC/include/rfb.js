...
function RFB(defaults) {
  ...
  let init_vars; let updateState; let fail; let handle_message;


  let init_msg; let normal_msg; let framebufferUpdate; let print_stats;


  let pixelFormat; let clientEncodings; let fbUpdateRequest; let fbUpdateRequests;


  let keyEvent; let pointerEvent; let clientCutText;


  let getTightCLength; let extract_data_uri;


  let keyPress; let mouseButton; let mouseMove;
  ...
  let rfb_state = 'disconnected';


  let rfb_version = 0;


  const rfb_max_version = 3.8;
  ...
  const def_con_timeout = Websock_native ? 2 : 5;


  /* Mouse state */

  let mouse_buttonMask = 0;
  ...
  that.set_local_cursor = function (cursor) {
    ...
    } else if (display.get_cursor_uri()) {
 ...
 */
  updateState = function (state, statusMsg) {
    ...
    switch (state) {
      ...
      case 'failed':
        ...
        setTimeout(() => { updateState('disconnected'); }, 50);
  ...
  init_msg = function () {
    ...
    let strlen; let reason; let length; let sversion; let cversion; let repeaterID;


    let i; let types; let num_types; let challenge; let response; let bpp; let depth;


    let big_endian; let red_max; let green_max; let blue_max; let red_shift;


    let green_shift; let blue_shift; let true_color; let name_length; let
      is_repeater;
  ...
  normal_msg = function () {
    ...
    let ret = true; let msg_type; let length; let text;


    let c; let first_colour; let num_colours; let red; let green; let
      blue;
  ...
  framebufferUpdate = function () {
    let now; let hdr; let fbu_rt_diff; let
      ret = true;
  ...
  encHandlers.RRE = function display_rre() {
    ...
    let color; let x; let y; let width; let height; let
      chunk;
  ...
  encHandlers.HEXTILE = function display_hextile() {
    ...
    let subencoding; let subrects; let color; let cur_tile;


    let tile_x; let x; let w; let tile_y; let y; let h; let xy; let s; let sx; let sy; let wh; let sw; let sh;


    const rQ = ws.get_rQ(); let
      rQi = ws.get_rQi();
  ...
  function display_tight(isTightPNG) {
    ...
    let ctl; let cmode; let clength; let color; let img; let
      data;
    let filterId = -1; let resetStreams = 0; let
      streamId = -1;
    const rQ = ws.get_rQ(); const
      rQi = ws.get_rQi();
    ...
    const decompress = function (data) {
      ...
      const uncompressed = FBU.zlibs[streamId].uncompress(data, 0);
    ...
    const indexedToRGB = function (data, numColors, palette, width, height) {
      // Convert indexed (palette based) image data to RGB
      // TODO: reduce number of calculations inside loop
      const dest = [];
      let x; let y; let b; let w; let w1; let dp; let
        sp;
      if (numColors === 2) {
    ...
    const handlePalette = function () {
      ...
      ws.rQshiftBytes(3);
      const palette = ws.rQshiftBytes(paletteSize);
      ...
      // Convert indexed (palette based) image data to RGB
      const rgb = indexedToRGB(data, numColors, palette, FBU.width, FBU.height);
    ...
    const handleCopy = function () {
      let raw = false;
      const uncompressedSize = FBU.width * FBU.height * fb_depth;
  ...
  encHandlers.Cursor = function set_cursor() {
    let x; let y; let w; let h; let pixelslength; let
      masklength;
 ...
 */
  ...
  fbUpdateRequests = function () {
    const cleanDirty = display.getCleanDirtyReset();


    let arr = []; let i; let cb; let
      db;
