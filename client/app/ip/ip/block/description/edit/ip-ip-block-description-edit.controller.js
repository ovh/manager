angular.module('Module.ip.controllers').controller('IpEditIpDescriptionCtrl', ($scope, $rootScope, $translate, Ip, Alerter) => {
    ...
    Ip.editIpDescription($scope.data.ipBlock.ipBlock, $scope.model.description || '')
      .then(
        () => {
          ...
          Alerter.success($translate.instant('ip_description_edit_success', { t0: $scope.data.ipBlock.ipBlock }));
        },
        (reason) => {
          Alerter.alertFromSWS($translate.instant('ip_description_edit_failure', { t0: $scope.data.ipBlock.ipBlock }), reason);
        },
      )
      .finally(() => {
        $scope.resetAction();
