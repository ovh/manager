  ...
  function loadServer() {
    ...
    return $q
      ...
      .then((data) => {
        ...
        _.set($scope.server, 'expiration', moment([expiration.year(), expiration.month(), expiration.date()]).toDate());

        const creation = moment.utc(serviceInfos.creation);
        _.set($scope.server, 'creation', moment([creation.year(), creation.month(), creation.date()]).toDate());
