  ...
  .controller('DedicatedCloudUserEditCtrl', class DedicatedCloudUserEditCtrl {
    ...
    $onInit() {
      ...
      return this.DedicatedCloud.getUserDetail(
        ...
        this.Alerter.error(`${this.$translate.instant('dedicatedCloud_USER_edit_load_error')} ${_.get(err, 'message', err)}`);
      }).finally(() => {
        this.loading = false;
      });
    }
    ...
    editUser() {
      return this.DedicatedCloud.updateUser(
        ...
        this.$state.go('^');
        this.Alerter.error(`${this.$translate.instant('dedicatedCloud_USER_edit_error')} ${_.get(err, 'message') || err}`);
      });
    }
  });
