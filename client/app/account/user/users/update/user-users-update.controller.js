...
angular.module('UserAccount').controller('UserAccountUsersUpdateCtrl', class UserAccountUsersUpdateCtrl {
  ...
  updateUser() {
    ...
    this.usersService.updateUser(this.user)
      .then(() => {
        this.alerter.success(this.$translate.instant('user_users_update_success_message', { login: this.user.login }), 'userUsers');
      })
      .catch((err) => {
        this.alerter.error(`${this.$translate.instant('user_users_update_error_message')} ${_.get(err, 'message', err)}`, 'userUsers');
      })
      .finally(() => {
        this.loader = false;
