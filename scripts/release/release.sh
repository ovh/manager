...
version() {
  ...
  if "${DRY_RELEASE}"; then
    ...
    printf "%s\n" "Releasing"
    node_modules/.bin/lerna version --ignore=@ovh-ux/manager-react-components --conventional-prerelease=@ovh-ux/shell,@ovh-ux/url-builder,@ovh-ux/manager-react-components,@ovh-ux/manager-react-shell-client,@ovh-ux/request-tagger --conventional-commits --no-commit-hooks --no-git-tag-version --no-push  --yes
  fi
}
