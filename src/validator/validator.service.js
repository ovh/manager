...
export default function () {
  ...
  this.isValidDomain = function isValidDomain(domain, opts = {}) {
    ...
    if (domain) {
      ...
      // Check wildcard
      if (!inError && punycodeVersion.indexOf('*') !== -1 && (opts.canBeginWithWildcard ? !/^(?:\*\.)[^*]+$/.test(punycodeVersion) : true)) {
        inError = true;
      }
      ...
      if (!inError) {
        angular.forEach(dotSplit, (sub) => {
          ...
          if (sub.indexOf('_') !== -1 && (opts.canBeginWithUnderscore ? !/^_[^_]+$/.test(sub) : true)) {
            inError = true;
          }
        });
      }
