...
import { applyTemplates } from '../dist/src/kernel/template/apply-templates.js';
import { OUTPUT_BASE_DIR } from '../dist/src/playbook/config/playbook-constants.js';
 ...
 *
 * Accepts `"1"` or `"true"` (case-sensitive) as `true`.
 * Returns `undefined` when `v` is `undefined` so callers can apply defaults.
 *
 * @param {string | undefined} v - Raw environment value.
 * @returns {boolean | undefined} `true`/`false` if parseable, otherwise `undefined`.
 */
function booleanFromEnv(v) {
...
async function run() {
  ...
  const flagAnswers = {
    ...
    // eslint-disable-next-line no-undef
    usePreset: argv.usePreset ?? booleanFromEnv(process.env.USE_PRESET),
