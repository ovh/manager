...
/**
 * Ensure a leading slash and trim spaces.
 *
 * @example
 ...
 * ensureLeadingSlash("   ")      // ""
 *
 * @param p - Path-like string.
 * @returns Normalized path (may be empty string).
 */
...
export function splitCombinedEndpoint(value?: string): string {
  ...
  return ensureLeadingSlash(pathPart);
}
...
/**
 * Extract all GET operation `apiPath` strings from an OpenAPI-like structure:
 * `{ [endpointsKey]: { get: { operationList: [{ apiPath: string }, ...] } } }`
 *
 * @param from - Root object containing endpoints.
 * @param endpointsKey - Either `"apiV2Endpoints"` or `"apiV6Endpoints"`.
 * @returns List of `apiPath` strings.
 */
export function extractApiPaths(
  from: unknown,
  endpointsKey: 'apiV2Endpoints' | 'apiV6Endpoints',
): string[] {
  if (!asDict(from)) return [];
  const endpoints = from[endpointsKey];
  if (!asDict(endpoints)) return [];
  const get = endpoints.get;
  if (!asDict(get)) return [];
  const opList = get.operationList;
