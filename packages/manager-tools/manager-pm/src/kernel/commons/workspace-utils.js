...
export async function getApplicationId(packageName) {
  ...
  try {
    const stdout = await new Promise((resolve, reject) => {
      execFile("yarn", ["workspaces", "info"], (error, stdout, stderr) => {
        if (error) {
          reject(new Error(`Failed to run yarn workspaces info: ${stderr || error.message}`));
          return;
        }
        resolve(stdout);
      });
    ...
    const id = basename(workspace.location);
    logger.info(`📦 Application ID for ${packageName} is "${id}"`);
    return id;
  } catch (err) {
    logger.error(`❌ Failed to get application ID for ${packageName}: ${err.message}`);
...
export function getApplications() {
  ...
  logger.debug(`Applications sample: ${apps.slice(0, 5).map((a) => a.value).join(", ")}${apps.length > 5 ? " ..." : ""}`);
  return apps;
}
