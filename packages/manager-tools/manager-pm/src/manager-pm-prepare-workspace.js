#!/usr/bin/env node
/**
 * CLI tool to prepare legacy workspace.
 *
...
async function main() {
  logger.info('🚀 manager-pm pm:prepare:legacy:workspace CLI started...');
  const start = Date.now();

  try {
    await updateRootWorkspacesFromCatalogs();
    const elapsed = ((Date.now() - start) / 1000).toFixed(2);
    logger.success(`✅ manager-pm pm:prepare:legacy:workspace completed in ${elapsed}s`);
  } catch (err) {
    logger.error('❌ manager-pm pm:prepare:legacy:workspace failed:');
    logger.error(err.stack || err.message || err);
    await handleProcessAbortSignals('pm:prepare:legacy:workspace-error', err);
    process.exit(1);
  }
}
