...
angular.module("ovh-angular-line-diagnostics").controller("LineDiagnosticsCtrl", function () {
    LineDiagnosticsCtrl.$inject = ["$interval", "$state", "$timeout", "$translate", "$uibModal", "atInternet", "LineDiagnostics", "LineDiagnosticFactory", "TucToast", "DIAGNOSTICS_CONSTANTS"];
    function LineDiagnosticsCtrl($interval, $state, $timeout, $translate, $uibModal, atInternet, LineDiagnostics, LineDiagnosticFactory, TucToast, DIAGNOSTICS_CONSTANTS) {
    ...
    _createClass(LineDiagnosticsCtrl, [{
        ...
        value: function $onInit() {
            ...
            this.LineDiagnosticsService.loadTranslations().then(function () {
                ...
                _this.TucToast.error(_this.$translate.instant("tools_lineDiagnostics_error_loading_translations"));
            });
        }
    }, {
        key: "$onDestroy",
        ...
        value: function runLineDiagnostic(requestParam) {
            ...
            return this.LineDiagnosticsService.getRunDiagnostic({
                ...
                if (!_.isEmpty(error)) {
                    ...
                    _this3.TucToast.error([_this3.$translate.instant("tools_lineDiagnostics_diagnostic_run_error"), error.message].join(" "));
                }
            }).finally(function () {
        ...
        value: function cancelLineDiagnostic() {
            ...
            return this.LineDiagnosticsService.getCancelDiagnostic({ number: this.lineNumber, serviceName: this.serviceName }).$promise.catch(function (error) {
                _this4.TucToast.error([_this4.$translate.instant("tools_lineDiagnostics_diagnostic_cancel_error"), _.get(error, "data.message", "")].join(" "));
            });
        }
    }, {
        key: "getNextAction",
