...
export default class {
  ...
  reformatOpening(opening) {
    ...
    return result.map((relayDay, i) => {
      ...
      if (relayDay.days.length > 1) {
        resultRelayDay.days = `${this.$translate.instant(`components_mondial_relay_${relayDay.days[0]}`)}. ${
          this.$translate.instant('components_mondial_relay_hours_to')} ${
          this.$translate.instant(`components_mondial_relay_${relayDay.days[relayDay.days.length - 1]}`).toLowerCase()}.`;
      } else if (relayDay.days.length) {
        resultRelayDay.days = this.$translate.instant(`components_mondial_relay_${relayDay.days[0]}_long`);
      }
  ...
  getResult(data) {
    if (data && data.relayPoints) {
      ...
      this.map.markers = map(this.foundRelays, (relay, index) => {
        this.map.bounds = this.constructor.appendBound(this.map.bounds, relay, !index);
        return {
          lat: relay.lat,
          lng: relay.lng,
  ...
  search(filter) {
    ...
    return this.mondialRelayService.v6().search(
      ...
      .catch((err) => {
        this.message = this.$translate.instant('components_mondial_relay_not_found');
