...
const PaymentMehtod = ({ environment }) => {
  ...
  const { getDefaultPaymentMethod, isEnterpriseAccount } = usePaymentMethod(environment);
  ...
  return (!isEnterpriseAccount() && (
    <div
      className={`${cssBaseClassName} mb-4`}
    >
      <a
      ...
      {isLoading ? (
        <div
          className="oui-skeleton oui-skeleton_s"
        >
          <div
            className="oui-skeleton__loader"
          >
          </div>
        </div>
      ) : (
