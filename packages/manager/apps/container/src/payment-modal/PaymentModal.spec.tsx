...
describe('PaymentModel.component', () => {
  ...
  it('should render no modal if card is valid', async () => {
    const spy = vi
      .spyOn(ManagerApi, 'fetchIcebergV6')
  ...
  it('should render modal if card is expired', async () => {
    const spy = vi
      .spyOn(ManagerApi, 'fetchIcebergV6')
      .mockResolvedValue(mockExpiredResponse);
    const { queryByTestId } = renderPaymentModal();
    await waitFor(
      () => {
        expect(spy).toHaveBeenCalled();
    ...
    expect(queryByTestId('paymentModal')).toHaveTextContent(
      `payment_modal_description_${PAYMENT_ALERTS.EXPIRED_CARD}`,
    );
  });
  ...
  it('should render modal if card is soon expired', async () => {
    const spy = vi
      .spyOn(ManagerApi, 'fetchIcebergV6')
      .mockResolvedValue(mockSoonExpiredResponse);
    const { queryByTestId } = renderPaymentModal();
    await waitFor(
      () => {
        expect(spy).toHaveBeenCalled();
    ...
    expect(queryByTestId('paymentModal')).toHaveTextContent(
      `payment_modal_description_${PAYMENT_ALERTS.SOON_EXPIRED_CARD}`,
    );
  });
});
