...
export default function LiveChat({
  ...
  let url = resolveChatTarget(ADRIELLY_PREPROD_TARGET);

  if (window.location.hostname.includes('labeu')) {
    url = resolveChatTarget(ADRIELLY_LABEU_TARGET);
  }
    ...
    >
      ...
      {chatType === 'SNOW' && snowContext.session_id && snowContext.queue && (
        <ChatDialog
          ...
          showHeader={false}
          visible={!chatbotReduced}
          url={generateSnowChatUrl(
            resolveChatTarget(SNOW_CHAT_TARGET),
            snowContext,
          )}
          key={chatType}
          onClose={handleCloseChat}
