...
const fetchOnboardingPreferences = () =>
  v6
    .get<OnboardingPreferences>(
      `/me/preferences/manager/${onboardingPreferencesKey}`,
    )
    .then((response) => {
      const { data } = response;
...
export const useOnboardingPreferences = ({ enabled }: { enabled?: boolean }) =>
  useQuery({
...
export const useCreateOnboardingPreferences = ({
  onSuccess,
}: MutationProps) => {
...
export const useUpdateOnboardingPreferences = ({
  onSuccess,
}: MutationProps) => {
  const queryClient = useQueryClient();
