...
export function createCmp(
  deps: {
    ...
    loaderUrls?: Record<'production' | 'preproduction', string>;
  } = {},
): Cmp {
  const injectScript = deps.injectScript ?? defaultInjectScript;
  const loaderUrls = deps.loaderUrls ?? CMP_LOADER_URLS;
  ...
  const load = ({ locale, region }: CmpLoadParams): void => {
    ...
    // The same build artifact is deployed to every environment — the
    // effective loader URL is selected at runtime from the hostname.
    const environment = CMP_PROD_HOSTNAME_RE.test(window.location.hostname)
      ? 'production'
      : 'preproduction';
    const loaderUrl = loaderUrls[environment];

    // No loader URL provided at build time for this environment: the CMP is
    // unavailable here — flag it so the caller falls back to the legacy
    // consent modal immediately, without waiting for the timeout.
    if (!loaderUrl) {
      markFailed();
      return;
    }
    ...
    injectScript(loaderUrl, markFailed);
  };
