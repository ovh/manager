...
export const getGameFirewallRuleQueryKey = ({ ip, ipOnGame }: GetGameFirewallRuleParams) => [
  `get/ip/${ip ? encodeURIComponent(ip) : ''}/game/${
    ipOnGame ? encodeURIComponent(ipOnGame) : ''
  }/rule`,
];
...
export const getGameFirewallRuleList = async ({
  ...
  v6.get<number[]>(
    `/ip/${ip ? encodeURIComponent(ip) : ''}/game/${
      ipOnGame ? encodeURIComponent(ipOnGame) : ''
    }/rule`,
  );
...
export const getGameFirewallRuleDetails = async ({
  ...
  ipOnGame,
  ruleId,
}: GetGameFirewallRuleDetailsParams): Promise<ApiResponse<IpGameFirewallRule>> => {
  if (!ip || !ipOnGame) {
