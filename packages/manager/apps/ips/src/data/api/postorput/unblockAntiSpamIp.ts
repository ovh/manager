...
export const unblockAntiSpamIpQueryKey = (params: UnblockAntiSpamIpParams) => [
  `post/ip/${encodeURIComponent(params.ip)}/spam/${
    params.ipBlocked ? encodeURIComponent(params.ipBlocked) : ''
  }/unblock`,
];
...
export const unblockAntiSpamIp = async ({
  ...
  return apiClient.v6.post<void>(
    `/ip/${encodeURIComponent(ip)}/spam/${ipBlocked ? encodeURIComponent(ipBlocked) : ''}/unblock`,
    {},
  );
};
