import { IpAntihackStateEnum, IpMitigationStateEnum, IpSpamStateEnum } from '@/data/api';
...
export const useIpHasAlerts = ({ ip, subIp, enabled = true }: UseIpHasAlertsParams) => {
  ...
  return {
    ...
    hasAlerts: {
      ...
      spam: ipSpam?.filter(
        (spam) => spam.state === IpSpamStateEnum.BLOCKED && isSubIpBlocked(spam.ipSpamming, subIp),
      ),
