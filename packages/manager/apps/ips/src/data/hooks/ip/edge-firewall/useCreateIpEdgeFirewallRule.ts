...
export const hasPortRangeError = (port?: string) => {
  ...
  const startPortNumber = parseInt(startPort, 10);
  const endPortNumber = parseInt(endPort, 10);
...
export const formatPortRangeValue = (port?: string) => {
  ...
  if (parseInt(startPort, 10) === IP_EDGE_FIREWALL_PORT_MIN) {
    return `lt ${endPort}`;
  }

  if (parseInt(endPort, 10) === IP_EDGE_FIREWALL_PORT_MAX) {
    return `gt ${startPort}`;
  }

  return `range ${startPort} ${endPort}`;
};
