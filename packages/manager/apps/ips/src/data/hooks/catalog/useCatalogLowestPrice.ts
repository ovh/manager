...
const isIpv6Plan = (plan: CatalogIpPlan | PccCatalogPlan) => plan.planCode.includes('ip-v6');
...
export const useCatalogLowestPrice = () => {
  ...
  return {
    ...
    ipv6LowestPrice:
      data?.data?.plans.filter(isIpv6Plan).reduce(getLowestPlanPrice, Number.POSITIVE_INFINITY) ??
      null,
  };
};
