...
export const SequenceColumn = (rule: IpEdgeFirewallRule & { isNew?: boolean }) => {
  const { newSequence, setNewSequence, ruleSequenceList } = React.useContext(
    EdgeNetworkFirewallContext,
  );
  const nextAvailableSequence = validSequenceNumbers.find((num) => !ruleSequenceList.includes(num));
  ...
  return rule?.isNew ? (
    <OdsFormField className="w-full">
      <OdsSelect
        ...
        onOdsChange={(e) => setNewSequence(parseInt(e.detail.value as string, 10))}
      >
        {validSequenceNumbers.map((sequence) => (
          <option key={sequence} value={sequence} disabled={ruleSequenceList.includes(sequence)}>
            {sequence}
          </option>
        ))}
      </OdsSelect>
