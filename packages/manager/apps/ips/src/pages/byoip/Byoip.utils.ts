...
export const getConfigValues = (configs: ProductConfiguration[] = [], configName: string) =>
  configs.find((config) => config.name === configName)?.values || [];
...
export function canAggregateByoipIp({
  ...
  const { range, isGroup } = ipFormatter(ip);
  ...
  if (isByoipSlice) {
    return parentIpRange && parentIpRange > 24 && parentIpRange < 31;
  }

  return !isGroup || (range && range > 24 && range < 31);
}
...
export function canSliceByoipIp({
  ...
  const { range, isGroup } = ipFormatter(ip);
  return ipDetails?.bringYourOwnIp && !isByoipSlice && isGroup && range && range < 32;
}
