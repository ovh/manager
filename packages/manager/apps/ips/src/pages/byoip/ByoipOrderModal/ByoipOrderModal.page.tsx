...
import { OdsButton, OdsCheckbox, OdsModal, OdsText } from '@ovhcloud/ods-components/react';
...
import { ByoipPayloadParams, ConfigItem, getByoipProductSettings } from '../Byoip.utils';
...
export const ByoipOrderModal: React.FC<{ isOpen: boolean }> = ({ isOpen = true }) => {
  ...
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>(
    declarationItems.reduce((acc, item) => ({ ...acc, [item.declaration]: false }), {}),
  );

  const declaratiosChecked = Object.values(checkedItems).every(Boolean);

  const handleCheckboxChange = (id: string) => (e: CustomEvent<OdsCheckboxChangeEventDetail>) => {
  ...
  return (
    <OdsModal isOpen={isOpen} isDismissible onOdsClose={handleClose}>
      ...
      <div className="mb-3 flex flex-col gap-3">
        <OdsText preset={ODS_TEXT_PRESET.span}>{t('ip_byoip_disclaimer_description')}</OdsText>
        ...
        {declarationItems.map(({ declaration, translationKey }) => (
          ...
          >
            ...
            <OdsText preset={ODS_TEXT_PRESET.paragraph}>{t(translationKey)}</OdsText>
          </label>
        ))}
      ...
      <div className="flex justify-end gap-3">
        ...
        <OdsButton
          ...
          onClick={() => {
            ...
            const keys = Object.keys(payload) as Array<keyof ByoipPayloadParams>;
            ...
            const campusConfig = config.find(({ label }) => label === CONFIG_NAME.CAMPUS);
            ...
            const campusId = config.findIndex(({ label }) => label === CONFIG_NAME.CAMPUS);
