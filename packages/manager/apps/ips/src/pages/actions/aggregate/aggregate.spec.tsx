...
describe('AggregateModal', () => {
  it('shows warning when no aggregates', async () => {
    const { container } = await renderTest({
      hasAggregates: false,
      nbIp: 6,
      initialRoute: urls.aggregate.replace(':parentId', fromIpToId('241.94.186.48/28')),
    });
    ...
    await waitFor(
      () =>
        expect(
          screen.getByText(
            labels.aggregateSlice.aggregateModalTitle.replace('{{ ip }}', '241.94.186.48/28'),
          ),
        ).toBeVisible(),
  ...
  it('shows select and children IPs when aggregates exist', async () => {
    ...
    await waitFor(
      () =>
        expect(
          within(modal).getByText(labels.aggregateSlice.aggregateModalDescription),
        ).toBeInTheDocument(),
      WAIT_FOR_DEFAULT_OPTIONS,
    );
