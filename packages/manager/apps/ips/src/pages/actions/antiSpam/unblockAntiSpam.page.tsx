...
export default function AntiSpamModal() {
  ...
  const toBeUnblocked = useMemo(
    () => ipSpam?.find((spam) => spam.ipSpamming === ip && spam.state === IpSpamStateEnum.BLOCKED),
    [ipSpam, ip],
  );
  ...
  const { isPending, mutate: unblockAntiSpamHandler } = useMutation({
    mutationFn: () => unblockAntiSpamIp({ ip: parentIp, ipBlocked: toBeUnblocked?.ipSpamming }),
    onSuccess: () => {
      clearNotifications();
      trackPage({
  ...
  return (
    <Modal
      ...
      isPrimaryButtonDisabled={unblockingDate ? isBefore(Date.now(), unblockingDate) : true}
