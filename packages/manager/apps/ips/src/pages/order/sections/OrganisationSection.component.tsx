...
export const OrganisationSection: React.FC = () => {
  ...
  const handleOrganisationChange = (event: OdsSelectCustomEvent<OdsSelectChangeEventDetail>) => {
    const updatedOrganisation =
      event.detail.value !== NO_ORGANISATION ? (event.detail.value as string) : undefined;
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        >
          <option value={NO_ORGANISATION}>{t('organisation_select_none')}</option>
          {organisations
            ?.filter(isAvailableOrganisation(selectedPlanCode as string))
            .map((orgId) => (
      ...
      <Links
        ...
        onClickReturn={() => {
          void navigate(urls.manageOrganisations);
        }}
      />
    </OrderSection>
  );
