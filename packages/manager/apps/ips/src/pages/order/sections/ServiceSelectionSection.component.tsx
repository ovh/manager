...
const getServiceType = (
  ...
  if (servicesByCategory[IpTypeEnum.PCC]?.some(({ serviceName }) => serviceName === serviceId)) {
    return ServiceType.dedicatedCloud;
  }
  if (
    servicesByCategory[IpTypeEnum.DEDICATED]?.some(({ serviceName }) => serviceName === serviceId)
  ) {
    return ServiceType.server;
  }
  if (servicesByCategory[IpTypeEnum.VPS]?.some(({ serviceName }) => serviceName === serviceId)) {
    return ServiceType.vps;
  }
  return servicesByCategory[IpTypeEnum.VRACK]?.some(({ serviceName }) => serviceName === serviceId)
    ? ServiceType.vrack
    : ServiceType.unknown;
};
...
export const ServiceSelectionSection: React.FC = () => {
  ...
  const { serviceByCategory, isLoading, isError, error } = useGetProductServices([
  ...
  const { isServiceInfoLoading, hasServiceInfoError, serviceStatus } = useCheckServiceAvailability({
  ...
  return (
    <OrderSection title={t('service_selection_title')}>
      {isError && (
        <OdsMessage color={ODS_MESSAGE_COLOR.critical}>{t('error_message', { error })}</OdsMessage>
      )}
      <OdsFormField className="w-full">
        ...
        <OdsCombobox
          ...
          onOdsChange={(event) => {
            ...
            setSelectedServiceType(getServiceType(serviceId, serviceByCategory));
            if (serviceId) {
              trackClick({
        ...
        >
          {ipVersion === IpVersion.ipv4 && (
            <OdsComboboxGroup>
              <span slot="title">
                {t('service_selection_select_dedicated_cloud_option_group_label')}
              </span>
              {dedicatedCloud?.map((service: ServiceInfoWithId) => (
                <ComboboxServiceItem
                  key={service.id}
          ...
          {ipVersion === IpVersion.ipv4 && (
            <OdsComboboxGroup>
              <span slot="title">
                {t('service_selection_select_dedicated_server_option_group_label')}
              </span>
              {server?.map((service: ServiceInfoWithId) => (
                <ComboboxServiceItem
                  key={service.id}
                  name={service.serviceName}
                  {...service}
          ...
          {ipVersion === IpVersion.ipv4 && (
            <OdsComboboxGroup>
              <span slot="title">{t('service_selection_select_vps_option_group_label')}</span>
              {vps?.map((service: ServiceInfoWithId) => (
                <ComboboxServiceItem
                  key={service.id}
                  name={service.serviceName}
                  {...service}
          ...
          <OdsComboboxGroup>
            <span slot="title">{t('service_selection_select_vrack_option_group_label')}</span>
            {vrack?.map((service: ServiceInfoWithId) => (
              <ComboboxServiceItem
                key={service.id}
                name={service.serviceName}
                {...service}
        ...
        {!!selectedService && (
          <div slot="helper">
            <div className="mt-1">
              {!isServiceInfoLoading && (serviceStatus !== 'ok' || hasServiceInfoError) && (
                <OdsMessage
                  className="block max-w-[384px]"
                  color={ODS_MESSAGE_COLOR.danger}
                  isDismissible={false}
                >
                  {t(`service_selection_${serviceStatus || 'expired'}_error_message`)}
                </OdsMessage>
              )}
              <ServiceRegion />
            </div>
