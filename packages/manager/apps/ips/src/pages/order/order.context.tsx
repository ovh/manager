...
export type OrderContextType = {
  ...
  setSelectedGeolocation: React.Dispatch<React.SetStateAction<string | undefined>>;
  selectedOrganisation?: string;
  setSelectedOrganisation: React.Dispatch<React.SetStateAction<string | undefined>>;
...
export const OrderContextProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  const [ipVersion, setIpVersion] = React.useState<IpVersion | undefined>(undefined);
  const [selectedService, setSelectedService] = React.useState<string | undefined>(undefined);
  const [selectedServiceType, setSelectedServiceType] = React.useState<ServiceType | undefined>(
    undefined,
  );
  const [selectedRegion, setSelectedRegion] = React.useState<string | undefined>(undefined);
  const [selectedOffer, setSelectedOffer] = React.useState<IpOffer | undefined>(undefined);
  const [selectedPlanCode, setSelectedPlanCode] = React.useState<string | undefined>(undefined);
  ...
  const [selectedGeolocation, setSelectedGeolocation] = React.useState<string | undefined>(
    undefined,
  );
  const [selectedOrganisation, setSelectedOrganisation] = React.useState<string | undefined>(
    undefined,
  );
