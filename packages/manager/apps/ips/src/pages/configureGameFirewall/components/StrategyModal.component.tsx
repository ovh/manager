...
export const StrategyModal: React.FC = () => {
  ...
  const { isPending: isUpdatePending, mutate: updateGameFirewall } = useUpdateIpGameFirewall({
    ip,
    ipOnGame,
    onSuccess: (variables) => {
      addSuccess(
  ...
  return (
    <Modal
      ...
      onPrimaryButtonClick={() => updateGameFirewall({ firewallModeEnabled: !firewallModeEnabled })}
