 ...
 */
export const IpEdgeFirewall = ({ ip, ipOnFirewall, isByoipSlice }: IpEdgeFirewallProps) => {
  const { expiredIps } = useContext(ListingContext);
  const { isGroup, ipAddress } = ipFormatter(ip);

  const enabled = React.useMemo(
    () =>
      ((isGroup && !!ipOnFirewall) || !isGroup) && expiredIps.indexOf(ip) === -1 && !isByoipSlice,
  ...
  return (
    <SkeletonCell isLoading={isLoading} enabled={enabled} error={error} ip={ipOnFirewall ?? ip}>
      <IpEdgeFirewallDisplay ip={ipOnFirewall ?? ip} ipEdgeFirewall={ipEdgeFirewall} />
    </SkeletonCell>
  );
};
