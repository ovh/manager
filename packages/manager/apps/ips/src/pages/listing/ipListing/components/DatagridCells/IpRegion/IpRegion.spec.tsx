...
describe('IpRegion Component', () => {
  it('Should display all ip regions', async () => {
    ...
    await waitFor(() => {
      expect(getByText(ipDetailsList?.[3]?.regions[0] as string)).toBeDefined();
      expect(getByText(ipDetailsList?.[3]?.regions[1] as string)).toBeDefined();
      expect(getByText(ipDetailsList?.[3]?.regions[2] as string)).toBeDefined();
    });
  });
});
