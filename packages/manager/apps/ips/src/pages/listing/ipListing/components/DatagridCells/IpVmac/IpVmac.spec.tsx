...
const useGetIpDetailsMock = vi.hoisted(() =>
  vi.fn(() => ({ ipDetails: undefined as IpDetails | undefined, isLoading: true })),
);
...
describe('IpVmac Component', () => {
  it('Should display if vmac exist for a given ip', async () => {
    ...
    const { getByText } = renderComponent({ ip: ipDetailsList[0]?.ip as string });
    await waitFor(() => {
  ...
  it('Should not display if vmac exist for not a given ip', async () => {
    ...
    const { queryByText } = renderComponent({ ip: ipDetailsList[0]?.ip as string });
    await waitFor(() => {
  ...
  it('Should display nothing if not linked to a dedicated server', async () => {
    ...
    const { queryByText } = renderComponent({ ip: ipDetailsList[1]?.ip as string });
    await waitFor(() => {
      expect(queryByText(`10.0.0.1`)).toBeNull();
    });
  });
