...
export const IpCell = ({ ip, parentIpGroup }: IpCellProps) => {
  ...
  const { onGoingAggregatedIps, onGoingSlicedIps, onGoingCreatedIps } = useContext(ListingContext);
    ...
    <>
      ...
      <div className="mt-2">
        {onGoingAggregatedIps.includes(ip) && (
          <OdsBadge label={t('aggregate_in_progress')} color={ODS_BADGE_COLOR.information} />
        )}
        {onGoingSlicedIps.includes(ip) && (
          <OdsBadge label={t('slice_in_progress')} color={ODS_BADGE_COLOR.information} />
        )}
        {onGoingCreatedIps.includes(ip) && (
          <OdsBadge label={t('creation_in_progress')} color={ODS_BADGE_COLOR.information} />
        )}
      </div>
    </>
  );
