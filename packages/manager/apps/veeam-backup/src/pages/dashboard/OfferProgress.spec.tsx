    ...
    ({
      ...
      it(contextLabel, async () => {
        ...
        if (mock.isLoading) {
          await waitFor(() => {
            expect(
              container.getElementsByTagName('osds-skeleton'),
            ).toHaveLength(expectedSkeletons);
          });
        } else if (mock.isError && mock.error?.response?.status !== 404) {
          await waitFor(() => {
            expect(screen.getByText(expectedError as string));
          });
        } else {
          await waitFor(() => {
            expect(
              container.querySelector('osds-progress-bar'),
            ).toHaveAttribute('value', expectedProgressBarValue);

            expect(screen.getByText(expectedConsumedStorage as string));
            expect(screen.getByText(expectedPrimaryRegion as string));
            if (expectedSecondaryRegion) {
              expect(
                screen.getByText(expectedSecondaryRegion),
              ).toBeInTheDocument();
            } else {
