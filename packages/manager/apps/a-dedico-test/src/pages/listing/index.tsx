...
export default function Listing() {
  ...
  const {
    ...
    queryKey: ['a-dedico-test', `/dedicated/server`],
  });
  ...
  const onSearch = (props: any) => {
    console.info('searchTest !!!', props);
  };

  if (columns?.length > 0) console.info('columns : ', columns);

  return (
    <BaseLayout
      breadcrumb={
        <Breadcrumb rootLabel={appConfig.rootLabel} appName="a-dedico-test" />
      }
      header={header}
    >
      <React.Suspense>
