    ...
    <>
      ...
      >
        <Text preset={TEXT_PRESET.paragraph}>
          {t(`${NAMESPACES.FORM}:all_fields_are_required`)}
        </Text>
        <div className="flex w-full md:w-1/2">
          <FormField className="w-full md:pr-6">
            <FormFieldLabel>{t(`${NAMESPACES.FORM}:country`)}</FormFieldLabel>
            <Controller
              name="country"
              control={control}
              rules={{ required: true }}
        ...
        <div className="flex">
          <Controller
            ...
            render={({ field: { name, value, onBlur, onChange } }) => (
              <FormField invalid={!!errors?.companyName?.message} className="w-full pr-6 md:w-1/2">
                <FormFieldLabel>{t(`${NAMESPACES.FORM}:companyName`)}</FormFieldLabel>
                <Input
          ...
          <FormField className="w-full pl-6 md:w-1/2 ">
            <FormFieldLabel>{t(`${NAMESPACES.FORM}:language`)}</FormFieldLabel>
            <Controller
              name="language"
              control={control}
              rules={{ required: true }}
          ...
          <Controller
            ...
            render={({ field: { name, value, onBlur, onChange } }) => (
              <FormField invalid={!!errors?.emailAddress?.message} className="w-full pr-6 md:w-1/2">
                <FormFieldLabel>{t(`${NAMESPACES.FORM}:email`)}</FormFieldLabel>
                <Input
                  type={INPUT_TYPE.text}
                  name={name}
                  value={value}
          ...
          <Controller
            ...
            render={({ field: { name, value, onBlur, onChange } }) => (
              <FormField invalid={!!errors?.phoneNumber?.message} className="w-full pl-6 md:w-1/2">
                <FormFieldLabel>{t(`${NAMESPACES.FORM}:phone`)}</FormFieldLabel>
                <Input
                  type={INPUT_TYPE.text}
                  name={name}
                  value={value}
