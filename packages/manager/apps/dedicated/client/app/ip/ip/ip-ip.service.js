...
angular
  ...
  .service(
    'Ip',
    function Ip($rootScope, $http, $q, constants, IpRange, $location, OvhHttp) {
      const self = this;
      ...
      this.getNichandleCountryEnum = () =>
        self
          .getMeModels()
          .then((models) => models['nichandle.CountryEnum'].enum);
      ...
      this.moveIpBlockToPark = (block) =>
        $http
          .post(
            `${swsProxypassPath}/ip/${window.encodeURIComponent(block)}/park`,
          )
          .then(
            (data) => data.data,
            (http) => $q.reject(http.data),
