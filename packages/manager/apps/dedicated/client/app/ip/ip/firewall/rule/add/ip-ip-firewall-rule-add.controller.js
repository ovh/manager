...
angular
  ...
  .controller(
    'IpFirewallAddRuleCtrl',
    ($scope, $rootScope, $translate, Ip, IpFirewall, Alerter, Validator) => {
      ...
      $scope.addRule = function addRule() {
        ...
        $scope.rule.source = isEmpty(get($scope.rule, 'source', '').trim())
          ? null
          : $scope.rule.source;
        $scope.rule.sourcePort = isEmpty(
          get($scope.rule, 'sourcePort', '').trim(),
        )
          ? null
