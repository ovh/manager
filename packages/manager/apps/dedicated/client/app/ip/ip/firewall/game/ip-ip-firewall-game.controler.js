...
angular.module('Module.ip.controllers').controller('IpGameFirewallCtrl', function IpGameFirewallCtrl($scope, $rootScope, $translate, Ip, IpGameFirewall, Alerter, $q) {
      ...
      .then(
        (firewall) => {
          ...
          if (firewall.state !== self.constantes.OK) {
            IpGameFirewall
              .pollFirewallState(self.datas.selectedBlock, self.datas.selectedIp)
              .then((_firewall) => {
    ...
    IpGameFirewall.getRules(self.datas.selectedBlock, self.datas.selectedIp).then(
      (rules) => {
        ...
        angular.forEach(rules, (ruleId) => {
          tablePromise.push(
            getRule(ruleId).then( // eslint-disable-line
              (rule) => {
  ...
  function getRule(ruleId) {
    return IpGameFirewall
      .getRule(self.datas.selectedBlock, self.datas.selectedIp, ruleId)
      .then((rule) => {
        if (rule.state !== self.constantes.OK) {
          IpGameFirewall
            .pollRuleState(self.datas.selectedBlock, self.datas.selectedIp, ruleId)
            .then((rulePoll) => {
  ...
  $scope.$on('ips.gameFirewall.display.add', (event, rule) => {
    ...
    IpGameFirewall
      .pollRuleState(self.datas.selectedBlock, self.datas.selectedIp, rule.id)
      .then((rulePoll) => {
