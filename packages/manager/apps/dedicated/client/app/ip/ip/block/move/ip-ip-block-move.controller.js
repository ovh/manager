angular.module('Module.ip.controllers').controller('IpMoveIpBlockCtrl', ($scope, $q, $translate, Ip, Alerter) => {
  ...
  function init() {
    ...
    queue.push(
      Ip.getIpMove($scope.data.ipBlock.ipBlock).then((result) => {
        ...
        $scope.ipDestinations.push({ service: $translate.instant('ip_servicetype__PARK'), serviceType: '_PARK', nexthop: [] });
      }),
    );
  ...
  $scope.canMove = () => {
    const serviceNameChoosed = $scope.model.serviceName
      && $scope.model.serviceName.service
      && $scope.noTasksPending;
    const nextHopSelectedPCC = () => $scope.model.serviceName.serviceType === 'dedicatedCloud'
      && $scope.model.nexthop;
