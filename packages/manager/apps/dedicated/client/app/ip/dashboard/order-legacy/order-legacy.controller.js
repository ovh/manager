...
export default class {
  ...
  getServices() {
    ...
    return this.$q
      .all({
        servicesList: this.IpDashboardOrderLegacy.getServicesByType(),
  ...
  canServiceBeOrdered() {
    ...
    return this.IpDashboardOrderLegacy.checkIfAllowed(
      ...
      .then((serviceAllowed) => {
        ...
        return this.IpDashboardOrderLegacy.getServiceOrderableIp(
          this.$scope.model.service,
        );
      })
      .then((infos) => {
        ...
        if (get(infos, 'vpsInfos.model.version') === '2019v1') {
          return this.$state.go('app.ip.dashboard.order', {
            serviceNameToOrder: this.$scope.model.service.serviceName,
          });
        }
        ...
        if (this.$scope.model.service.serviceType === 'PCC') {
          return this.DedicatedCloud.getDescription(
            ...
            this.$scope.model.service.usesOrderLegacy = generation !== '2.0';
  ...
  loadOrderForm() {
    ...
    queue.push(
      this.IpDashboardOrderLegacy.getAvailableCountries(
        this.$scope.model.service,
      )
        .then((countries) => {
        ...
        .catch((error) => {
          if (
            ...
            !this.$scope.model.service.usesOrderLegacy
          ) {
            return null;
          }
    ...
    if (this.$scope.model.service.serviceType === 'PCC') {
      ...
      queue.push(
        this.IpDashboardOrderLegacy.checkIfCanadianServer(
          this.$scope.model.service.serviceName,
  ...
  orderFormValid() {
    ...
    switch (this.$scope.model.service.serviceType) {
      case 'DEDICATED':
        ...
        return true;
      case 'PCC':
        if (this.$scope.model.service.usesOrderLegacy) {
          return (
  ...
  loadPrices(durations) {
    ...
    const queue = durations.map((duration) =>
      this.IpDashboardOrderLegacy.getOrderForDuration(
        this.$scope.model.service,
        this.$scope.model.params,
        duration,
      ).then((details) => {
  ...
  getDurations() {
    ...
    if (
      ...
      queue.push(
        this.IpDashboardOrder.fetchPrices(
          this.$scope.model.service.serviceName,
          this.$scope.model.params.size,
        ).then((prices) => {
      ...
      queue.push(
        this.IpDashboardOrderLegacy.getOrder(
          this.$scope.model.service,
          this.$scope.model.params,
        ).then((durations) => {
    ...
    if (this.$scope.model.service.serviceType === 'DEDICATED') {
      ...
      if (needProfessionalUse) {
        queue.push(
          this.IpDashboardOrderLegacy.getProfessionalUsePrice(
            this.$scope.model.service.serviceName,
          ).then((price) => {
  ...
  confirmOrder() {
    ...
    return this.confirmOrderLegacy();
  }

  confirmOrderLegacy() {
    this.$scope.loading.validation = true;

    return this.IpDashboardOrderLegacy.postOrder(
      this.$scope.model.service,
      this.$scope.model.params,
      ...
      .then((order) => {
        ...
        return this.goBack(
          {
            message: {
              text: this.$translate.instant('ip_order_finish_success', {
                t0: order.url,
                t1: order.orderId,
              }),
              data: { idTask: order.orderId, state: 'OK' },
            },
          },
          { reload: true },
        );
