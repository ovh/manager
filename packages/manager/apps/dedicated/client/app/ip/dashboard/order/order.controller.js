...
import { IP_LOCATION_GROUPS, PRODUCT_TYPES } from './order.constant';
...
export default class IpOrderController {
      ...
      return ipCountryAvailablePromise
        .then((countries) => {
          if (countries.length > 0) {
            const ipOffersByRegion = IpOrderController.getRegionsOffers(
              countries,
            );
            ...
            this.ipOffers = IpOrderController.filterOfferDetailsFromServiceName(
              ipOfferDetails,
              this.model.selectedService.serviceName,
            );
          }
  ...
  static filterOfferDetailsFromServiceName(offerDetails, serviceName) {
    return filter(offerDetails, {
      productRegion: IpOrderController.getRegionFromServiceName(serviceName),
    });
  }
