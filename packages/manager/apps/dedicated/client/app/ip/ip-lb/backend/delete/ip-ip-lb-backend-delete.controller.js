angular.module('Module.ip.controllers').controller('IplbBackendDeleteCtrl', ($scope, $rootScope, $translate, Iplb, Alerter) => {
  ...
  $scope.deleteBackend = function deleteBackend() {
    ...
    Iplb.deleteBackend($scope.data.selectedIplb.value, $scope.data.backend.backend)
      .then(
        ...
        (reason) => {
          Alerter.alertFromSWS($translate.instant('iplb_backend_delBackend_failure'), reason);
        },
      )
      .finally(() => {
        $scope.resetAction();
