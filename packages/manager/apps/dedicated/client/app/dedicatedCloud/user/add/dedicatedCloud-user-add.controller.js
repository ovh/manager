angular
  .module('App')
  .controller(
    'DedicatedCloudUserAddCtrl',
    ($scope, $stateParams, $translate, DedicatedCloud, Alerter) => {
      ...
      $scope.addUser = function addUser() {
        ...
        DedicatedCloud.addUser($stateParams.productId, $scope.newUser)
          .then(
            () => {
              Alerter.success(
                $translate.instant('dedicatedCloud_users_add_start'),
              );
            },
            (err) => {
              Alerter.alertFromSWS(
                $translate.instant('dedicatedCloud_users_add_error'),
                err,
              );
            },
          )
...
angular
  .module('App')
  .controller('DedicatedCloudUserAddFormCtrl', ($scope, DedicatedCloud) => {
    $scope.userForm = null;
    ...
    $scope.checkPassword = function checkPassword(password) {
      ...
      const passwordIsCorrect = DedicatedCloud.checkPassword(
