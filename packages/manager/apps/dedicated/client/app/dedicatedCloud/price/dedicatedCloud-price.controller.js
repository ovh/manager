...
angular.module('App').controller('DedicatedCloudNewPricesInformationCtrl', [
  ...
  function DedicatedCloudNewPricesInformationCtrl($scope) {
    $scope.loader = true;
    $scope.pricesInformationData = angular
      .copy($scope.currentActionData)
      .filter((resource) => resource.changed === true);

    $scope.pricesInformationIds = $scope.pricesInformationData.map(
      (pricesInformation) => pricesInformation.name,
    );
