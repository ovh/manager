...
angular
  .module('App')
  .controller(
    'DedicatedCloudSecurityOptionOrderCtrl',
    (
      $stateParams,
      $rootScope,
      $scope,
      ...
      function checkOption(optionName) {
        return DedicatedCloud.getOptionState(
          optionName,
          $stateParams.productId,
        ).then((result) => {
          if (result.error) {
            Alerter.alertFromSWS(
              $translate.instant('dedicatedCloud_options_check_error'),
              result.error,
            );
          }
      ...
      function checkTrustedIp() {
        return DedicatedCloud.getSecurityPolicies(
          $stateParams.productId,
          null,
          null,
          true,
      ...
      function loadUsers() {
        return DedicatedCloud.getUsers($stateParams.productId).then((ids) =>
          $q.all(
            ids.map((id) =>
              DedicatedCloud.getUserDetail($stateParams.productId, id),
            ),
          ),
        );
      }
      ...
      function canEnableOption() {
        return DedicatedCloud.isOptionToggable(
          $stateParams.productId,
          $scope.optionName,
          'disabled',
          false,
      ...
      $scope.checkCompliance = function checkCompliance() {
        function checkStep(step) {
          switch (step) {
            case 1:
              return $q.all([
                applyCheck(checkOption('nsx'), 'nsx'),
                applyCheck(checkOption('vrops'), 'vrops'),
              ]);
            case 2:
              return $q.all([
                applyCheck(checkRestrictedAccess(), 'restrictedAccess'),
                applyCheck(checkTrustedIp(), 'trustedIp'),
              ]);
            case 3:
      ...
      $scope.loadPrices = function loadPrices() {
        ...
        return DedicatedCloud.getSelected($stateParams.productId)
          .then((pcc) =>
            DedicatedCloud.fetchAllHostsPrices(
              $stateParams.productId,
          ...
          .catch((err) => {
            Alerter.alertFromSWS(
              $translate.instant('dedicatedCloud_options_load_prices_error'),
              err,
            );
            $scope.resetAction();
          })
      ...
      $scope.subscribeOption = function subscribeOption() {
        ...
        DedicatedCloud.enableOption($stateParams.productId, $scope.optionName)
          ...
          .catch((err) => {
            Alerter.alertFromSWS(
              $translate.instant('dedicatedCloud_options_order_activate_error'),
              err,
            );
          })
          .finally(() => {
