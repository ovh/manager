...
angular
  .module('App')
  .controller('DedicatedCloudSecurityCtrl', function DedicatedCloudSecurityCtrl(
    $rootScope,
    $stateParams,
    $scope,
    $uibModal,
    ...
    function initSelection() {
      if (
        ...
        for (i, l = $scope.policies.model.list.results.length; i < l; i += 1) {
          if (
            ~$scope.selectedPolicies.indexOf(
              $scope.policies.model.list.results[i].id,
            )
          ) {
    ...
    $scope.globalCheckboxPoliciesStateChange = function globalCheckboxPoliciesStateChange(
      state,
    ) {
      if (
        $scope.policies &&
        ...
        switch (state) {
          ...
          case 1:
            for (
              i, l = $scope.policies.model.list.results.length;
              i < l;
              i += 1
            ) {
              if (
          ...
          case 2:
            for (
              i, l = $scope.policies.model.fullNetworksList.length;
              i < l;
              i += 1
            ) {
              if (
                !~$scope.selectedPolicies.indexOf(
                  $scope.policies.model.fullNetworksList[i],
                )
              ) {
                $scope.selectedPolicies.push(
                  $scope.policies.model.fullNetworksList[i],
    ...
    self.addKms = function addKms() {
      ...
      kmsCreationModal.result.then(() => {
        ...
        $scope.loadInfo();
        $scope.setMessage(
          $translate.instant('dedicatedCloud_vm_encryption_kms_added'),
          {},
        );
      });
    };
    ...
    self.deleteKms = function deleteKms(kmsToDelete) {
      ...
      kmsDeletionModal.result.then(() => {
        ...
        $scope.loadInfo();
        $scope.setMessage(
          $translate.instant('dedicatedCloud_vm_encryption_kms_deleted'),
          {},
        );
      });
    };
    ...
    self.editKms = function editKms(kmsToEdit) {
      ...
      kmsEditionModal.result.then(() => {
        ...
        $scope.loadInfo();
        $scope.setMessage(
          $translate.instant('dedicatedCloud_vm_encryption_kms_edited'),
          {},
        );
      });
    };

    self.getVMEncryptionKMSList = function getVMEncryptionKMSList({
      offset,
      pageSize,
    }) {
      return DedicatedCloud.getVMEncryptionKMSList($stateParams.productId).then(
        (kmsIds) => ({
          data: kmsIds
    ...
    self.getVMEncryptionKMSDetail = function getVMEncryptionKMSDetail(id) {
      return DedicatedCloud.getVMEncryptionKMSDetail(
        $stateParams.productId,
        id,
      );
    };
