angular.module('Module.license').controller('LicenseSplaRevokeCtrl', ($scope, $translate, License, Alerter, $rootScope, $location, $timeout) => {
  $scope.model = {
  ...
  $scope.revokeSpla = () => {
    if (!$scope.model.deleting && !$scope.model.deleted) {
      ...
      License.splaRevoke($scope.model.license.serverServiceName, $scope.model.license.id).then(
        ...
        (err) => {
          $scope.resetAction();
          Alerter.alertFromSWS($translate.instant('license_spla_revoke_fail'), err.data);
        },
      );
    }
  };
