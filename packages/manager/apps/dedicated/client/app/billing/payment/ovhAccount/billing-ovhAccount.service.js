angular
  ...
  .service(
    'BillingOvhAccount',
    function BillingOvhAccount($http, $q, $cacheFactory) {
      const billingAccountCache = $cacheFactory(
        'UNIVERS_BILLING_OVHACCOUNT_LIST',
      );
      const self = this;
      ...
      this.creditOvhAccount = function creditOvhAccount(ovhAccountId, amount) {
        return $http
          .post(
            ['apiv6/me/ovhAccount', ovhAccountId, 'creditOrder'].join('/'),
            {
      ...
      this.retrieveMoney = (ovhAccountId, amount, bankAccountId) =>
        $http
          .post(
            ['apiv6/me/ovhAccount', ovhAccountId, 'retrieveMoney'].join('/'),
            {
              amount,
              bankAccountId,
            },
