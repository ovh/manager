...
angular.module('Billing.controllers').controller(
  'BillingHistoryBalanceCtrl',
  class BillingHistoryBalanceCtrl {
    constructor(
      $q,
      $state,
      $translate,
        ...
        .then(() => {
          this.Alerter.success(
            this.$translate.instant('billing_history_balance_pay_success', {
              trackHref: this.$state.href(
                'app.account.billing.payments.request',
              ),
            }),
            'billing_balance',
          );
              ...
              .then((response) => {
                ...
                this.paymentMethods = filter(
                  ...
                  ({ paymentType, status }) =>
                    ...
                    ['CANCELED_BY_CUSTOMER', 'CANCELING'].indexOf(status) ===
                      -1,
                );
