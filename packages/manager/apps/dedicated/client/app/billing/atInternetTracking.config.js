angular.module('Billing.services').run(['$rootScope', 'atInternet', 'AUTORENEW_EVENT', 'PAYMENT_EVENT', 'OVH_ACCOUNT_EVENT', 'FIDELITY_EVENT',
  function BillingServicesRun(
    ...
    $rootScope.$on(AUTORENEW_EVENT.DISABLE_AUTOMATIC_PAYMENT_FOR_DOMAINS, trackClickEvent);
    $rootScope.$on(AUTORENEW_EVENT.TERMINATE, (event, data) => trackServiceClickEvent(event, data));
    $rootScope.$on(
      AUTORENEW_EVENT.TERMINATE_AT_EXPIRATION,
      (event, data) => trackServiceClickEvent(event, data),
    );
    $rootScope.$on(
      AUTORENEW_EVENT.CANCEL_TERMINATE,
      (event, data) => trackServiceClickEvent(event, data),
    );
    $rootScope.$on(
      AUTORENEW_EVENT.ACTIVATE_AUTO_PAYMENT,
      (event, data) => trackServiceClickEvent(event, data),
    );
    $rootScope.$on(
      AUTORENEW_EVENT.DEACTIVATE_AUTO_PAYMENT,
      (event, data) => trackServiceClickEvent(event, data),
    );
