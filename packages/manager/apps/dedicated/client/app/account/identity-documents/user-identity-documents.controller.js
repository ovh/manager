    ...
    if (!this.form.$invalid && this.isFileExtensionsValid()) {
      ...
      this.getUploadDocumentsLinks(this.files.length)
        ...
        .then(({ data: { uploadLinks } }) =>
          this.tryToFinalizeProcedure(uploadLinks),
        )
        // If everything goes well, we update the view and send a tracking hit
        .then(() => {
          this.loading = false;
    ...
    return (
      this.$q
        ...
        .then(() => {
          ...
          return this.$http.post(`/me/procedure/identity/finalize`);
        })
    );
  }
