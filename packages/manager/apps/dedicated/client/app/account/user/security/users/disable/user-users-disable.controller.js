...
angular.module('UserAccount').controller('UserAccountUsersDisableCtrl', class UserAccountUsersDisableCtrl {
  ...
  disableUser() {
    ...
    this.usersService.disableUser(this.user)
      .then(() => {
        this.alerter.success(this.$translate.instant('user_users_disable_success_message', { login: this.user.login }), 'userUsers');
      })
      .catch((err) => {
        this.alerter.error(`${this.$translate.instant('user_users_disable_error_message')} ${_.get(err, 'message', err)}`, 'userUsers');
      })
      .finally(() => {
        this.loader = false;
