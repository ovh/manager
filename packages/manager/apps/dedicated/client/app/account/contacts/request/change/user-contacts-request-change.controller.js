...
angular.module('UserAccount').controller('UserAccount.controllers.contacts.request', [
  ...
  function UserAccountContactsRequestController(
    ...
    $scope.changeContactRequest = function changeContactRequest() {
      ...
      switch ($scope.action) {
        case 'ACCEPT':
          promise = Contacts.acceptRequest({ id: $scope.task.id, token: $scope.token.value });
          break;
        case 'REFUSE':
          promise = Contacts.refuseRequest({ id: $scope.task.id, token: $scope.token.value });
          break;
        case 'RESEND':
          promise = Contacts.resendRequest({ id: $scope.task.id });
          break;
        default:
          throw new Error(`Unsupported and unexpected contact request type ${$scope.action}`);
      }
