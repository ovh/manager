  ...
  .controller(
    ...
    ($scope, $http, $stateParams, $translate, Polling, Alerter, Housing) => {
      ...
      function init() {
        ...
        Housing.getFtpBackup($stateParams.productId)
          .then((data) => {
            ...
            $scope.ftpBackup.usage =
              (data.quota.usage / data.quota.value) * 100;
      ...
      function startEditPolling(ipBlock) {
        return Housing.getFtpBackupIpDetail(
          $stateParams.productId,
          ipBlock,
        ).then((backup) => {
          if (!backup.isApplied) {
            ...
            .then(
              ...
              (data) => {
                Alerter.alertFromSWS(
                  $translate.instant(
                    'housing_configuration_ftpbackup_set_fail',
                    { t0: $scope.ipbackupCurrentEdit.ipBlock },
                  ),
                  data,
                  alert,
