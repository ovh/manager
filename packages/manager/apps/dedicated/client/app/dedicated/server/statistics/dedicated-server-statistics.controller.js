  ...
  .controller(
    ...
    (
      ...
      $scope.loadBandwidthInformations = function loadBandwidthInformations() {
        $q.all([
          Server.getBandwidth($stateParams.productId)
            ...
            .then((data) =>
              Server.getBandwidthOption($stateParams.productId, data),
            )
            .then(
              (bandwidthOption) => ($scope.bandwidthOption = bandwidthOption),
            ),
          Server.getBandwidthVrackOption($stateParams.productId).then(
            (bandwidthVrackOption) =>
              ($scope.bandwidthVrackOption = bandwidthVrackOption),
          ),
          BandwidthVrackOrderService.getOrderableBandwidths(
            $stateParams.productId,
          ).then(
            (bandwidthVrackOptions) =>
              ($scope.bandwidthVrackOrderOptions = bandwidthVrackOptions),
          ),
          ServerTrafficService.getTraffic($stateParams.productId).then(
            (traffic) => ($scope.traffic = traffic.data),
          ),
          ServerOrderTrafficService.getOrderables($stateParams.productId).then(
            (trafficOrderables) =>
              ($scope.trafficOrderables = trafficOrderables.data),
          ),
          ServerOrderTrafficService.getOption($stateParams.productId).then(
            (trafficOption) => ($scope.trafficOption = trafficOption.data),
          ),
        ])
          .catch((data) =>
            Alerter.alertFromSWS(
              $translate.instant('server_bandwidth_loading_error'),
              data.data,
              'bandwithError',
            ),
          )
      ...
      $scope.canOrderMoreTraffic = () =>
        !$scope.server.isExpired &&
        $scope.server.canOrderQuota &&
        get($scope.trafficOrderables, 'length');

      $scope.isFullAgora = (commercialRange) =>
        $scope.pattern.test(commercialRange);

      $scope.$on(
        'dedicated.informations.bandwidth',
      ...
      function createChart(data) {
        $scope.series = [];
        $scope.data = [];
        $scope.labels = map(get(data, 'download.values'), (value) =>
          moment.unix(value.timestamp).calendar(),
        );
        $scope.series.push(
          $translate.instant('server_tab_STATS_legend_download'),
        );
        $scope.series.push(
          $translate.instant('server_tab_STATS_legend_upload'),
        );
        ...
        $q.all(promises)
          ...
          .catch((data) => {
            ...
            if (nameServer && data) {
              ...
              $scope.setMessage(
                $translate.instant('server_tab_STATS_loading_fail'),
                data.data,
              );
            }
          })
      ...
      $scope.removeHack = function removeHack() {
        Server.removeHack($stateParams.productId).then(
          ...
          (data) => {
            set(data, 'type', 'ERROR');
            $scope.setMessage(
              $translate.instant('server_remove_hack_fail'),
              data.data,
            );
          },
        );
