...
angular
  ...
  .controller(
    'AddSecondaryDnsCtrl',
    ($scope, $translate, Server, Alerter, IpRange, $stateParams) => {
      $scope.entry = {
        domain: '',
        ip: '',
      };
      ...
      $scope.loadIpList = function loadIpList() {
        ...
        Server.listIps($stateParams.productId).then(
          ...
          (err) => {
            ...
            set(err, 'type', 'ERROR');
            $scope.setMessage(
              $translate.instant('server_configuration_ips_cannotfetch'),
              err,
            );
          },
        );
      ...
      $scope.loadToken = function loadToken() {
        ...
        Server.getDomainZoneInformation(
          ...
          (data) => {
            ...
            Alerter.alertFromSWS(
              $translate.instant(
                'server_configuration_secondarydns_add_step2_error',
              ),
              data.data,
              'addTokenAlert',
            );
      ...
      $scope.addSecondaryDns = function addSecondaryDns() {
        ...
        Server.addSecondaryDns(
          ...
          () => {
            ...
            $scope.setMessage(
              $translate.instant(
                'server_configuration_secondarydns_add_success',
          ...
          (err) => {
            ...
            set(err, 'type', 'ERROR');
            $scope.setMessage(
              $translate.instant('server_configuration_secondarydns_add_fail'),
              err,
            );
          },
        );
