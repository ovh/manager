...
angular.module('controllers').controller('controllers.Server.Netboot', ($scope, Server, Alerter, $q, $stateParams, $translate) => {
  ...
  $scope.firstStepValidation = function () {
    return (
      $scope.currentNetboot.type === $scope.HARDDISK
            ...
            || ($scope.currentNetboot.type === $scope.NETWORK && $scope.currentNetboot.network !== '' && $scope.isValid.root)
    );
  };

  function rootDeviceValidation() {
    return regRootDevice1.test($scope.rootDevice.root)
  ...
  $scope.setNetboot = function () {
    ...
    const promiseList = [Server.setNetBoot(
    ...
    if ($scope.currentNetboot.type === $scope.RESCUE) {
      promiseList.push(Server.updateRescueMail(
        $stateParams.productId,
        netbootId,
