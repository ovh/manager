angular.module('App').controller('ServerIpMitigationCtrl', ($scope, $translate, Server) => {
  ...
  if ($scope.mitigationStatusAuto) {
    $scope.translations.wizardTitle = $translate.instant('server_configuration_mitigation_force_title');
    $scope.translations.wizardQuestion = $translate.instant('server_configuration_mitigation_force_question', { t0: $scope.selectedIpAndBlock.ip.ip });
  } else {
    $scope.translations.wizardTitle = $translate.instant('server_configuration_mitigation_auto_title');
      ...
      .then(
        (data) => {
          ...
          if (newMitigationStatus === 'AUTO') {
            ...
            $scope.setMessage($translate.instant('server_configuration_mitigation_force_success', { t0: $scope.selectedIpAndBlock.ip.ip }), data);
          }
        },
        (data) => {
          ...
          if (newMitigationStatus === 'AUTO') {
            ...
            $scope.setMessage($translate.instant('server_configuration_mitigation_force_failed', { t0: $scope.selectedIpAndBlock.ip.ip }), data);
          }
        },
      );
  };
