angular.module('App').controller('NasOrderValidationCtrl', ($scope, Nas, $rootScope, $translate, Alerter) => {
  ...
  $scope.load = function load() {
    ...
    if ($scope.choice.nasha) {
      Nas
        .getBCNasha($scope.choice.model, $scope.choice.datacenter, $scope.choice.duration)
        .then(getNasSuccess, getNasFail);
    } else {
      Nas
        .getBCNas($scope.choice.model, $scope.choice.protocol, $scope.choice.duration)
        .then(getNasSuccess, getNasFail);
    }
  };
  ...
  $scope.orderNas = function orderNas() {
    ...
    if ($scope.choice.nasha) {
      Nas
        .postBCNasha($scope.choice.model, $scope.choice.datacenter, $scope.choice.duration)
        .then(postNasSuccess, postNasFail);
    } else {
      Nas
        .postBCNas($scope.choice.model, $scope.choice.protocol, $scope.choice.duration)
        .then(postNasSuccess, postNasFail);
    }
  };
});
