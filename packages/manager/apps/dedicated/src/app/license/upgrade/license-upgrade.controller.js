    ...
    switch ($scope.license.type) {
      ...
      case 'WORKLIGHT':
        if ($scope.selected.options[$scope.license.type].version !== null) {
          opts.version = $scope.selected.options[$scope.license.type].version.value;
        }
        break;
      default:
        break;
  ...
  $scope.isSelectionValid = function () {
    ...
    if ($scope.license
      ...
      angular.forEach(moreOptions, (value) => {
        ...
        valid = valid && (value === null || !value.mandatory || (value.mandatory && value.value !== null && (value.shouldBeEqualsTo === undefined || value.shouldBeEqualsTo === value.value))); // eslint-disable-line
      });
    }
