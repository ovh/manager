...
angular.module('Module.license').controller('LicenseMigrateCtrl', ($scope, $translate, License, Alerter) => {
  ...
  function checkIpValidity() {
    ...
    return License.canLicenceBeMovedTo({
      ...
      (data) => {
        ...
        Alerter.alertFromSWS($translate.instant('license_migrate_fail'), data);
      },
    ).finally(() => {
  ...
  $scope.migrate = function migrate() {
    ...
    return License.migrate({
      ...
      (data) => {
        Alerter.alertFromSWS($translate.instant('license_migrate_success'), data.status);
      },
      (err) => {
        Alerter.alertFromSWS($translate.instant('license_migrate_fail'), err.data);
      },
    ).finally(() => {
      $scope.resetAction();
      $scope.loaders.migrate = false;
