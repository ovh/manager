angular.module('Module.license').controller('LicenseDirectAdminChangeOsCtrl', ($scope, $q, $translate, License, Alerter) => {
  $scope.model = {
  ...
  function getDirectAdminModels() {
    return License.getDirectAdminModels().then(({ models }) => models['license.DirectAdminOsEnum'].enum);
  }
  ...
  function isTaskPending() {
    return License.tasks($scope.model.license, 'changeOs').then(tasks => _.size(tasks) > 0);
  }
  ...
  function init() {
    $q
      ...
      .catch((err) => {
        $scope.exit(true);
        Alerter.alertFromSWS($translate.instant('license_directadmin_changeOs_loading_error'), err);
      })
      .finally(() => {
  ...
  $scope.changeOs = function () {
    if ($scope.model.selectedOs && !$scope.model.modifying) {
      ...
      License.changeOs($scope.model.license, $scope.model.selectedOs)
        .then(() => {
          Alerter.success($translate.instant('license_directadmin_changeOs_success'));
        })
        .catch((err) => {
          Alerter.alertFromSWS($translate.instant('license_directadmin_changeOs_fail'), err.message);
        })
        .finally(() => {
          $scope.exit(true);
        });
