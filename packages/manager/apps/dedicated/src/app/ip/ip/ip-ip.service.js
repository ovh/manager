...
angular.module('Module.ip.services').service('Ip', function ($rootScope, $http, $q, constants, IpRange, $location, OvhHttp) {
  ...
  this.checkTaskUnique = (ipBlock, fct) => {
    ...
    angular.forEach(['init', 'doing', 'todo'], (status) => {
      queue.push(
        $http
          .get(`${swsProxypassPath}/ip/${window.encodeURIComponent(ipBlock)}/task`, {
            params: {
              function: fct,
              status,
            },
  ...
  this.getServerModels = () => $http.get(`${swsProxypassPath}/dedicated/server.json`, { cache: true }).then((response) => {
    if (response && response.data && response.data.models) {
      return response.data.models;
    }
    return {};
  });
  this.getIpModels = () => $http.get(`${swsProxypassPath}/ip.json`, { cache: true }).then((response) => {
    if (response && response.data && response.data.models) {
      return response.data.models;
    }
    return {};
  });
  this.getOrderModels = () => $http.get(`${swsProxypassPath}/order.json`, { cache: true }).then((response) => {
    if (response && response.data && response.data.models) {
      return response.data.models;
    }
    return {};
  });
  this.getMeModels = () => $http.get(`${swsProxypassPath}/me.json`, { cache: true }).then((response) => {
    if (response && response.data && response.data.models) {
      return response.data.models;
    }
    return {};
  ...
  this.getServicesList = () => OvhHttp.get('/sws/products/services', {
    rootPath: '2api',
  });

  this.getIpsForIpBlock = (ipBlock, serviceName, serviceType) => $http
  ...
  /*= ==================================
        =            DESCRIPTION            =
        =================================== */
  this.editIpDescription = (block, description) => OvhHttp.put('/ip/{block}', {
    rootPath: 'apiv6',
    urlParams: {
      block,
    },
