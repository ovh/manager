...
export const RECORD_FORM_CONFIGS: Readonly<Record<string, RecordFormConfig>> = {
  ...
  CAA: {
    ...
    rows: [
      ...
      {
        fields: [
          { type: 'number', name: 'flags', labelKey: 'zone_page_form_label_caa_flags', required: true, readOnly: true, min: 0, max: 255, placeholder: '0' },
  ...
  DKIM: {
    ...
    subDomainHelperKey: 'zone_page_form_dkim_subdomain_helper',
    subDomainSuffix: '._domainkey',
    rows: [
      { fields: ['subdomain', 'ttl'] },
      {
        fields: [
  ...
  DMARC: {
    ...
    rows: [
      ...
      {
        fields: [
          { type: 'text', name: 'v', labelKey: 'zone_page_form_label_dmarc_v', required: true, readOnly: true },
