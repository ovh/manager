...
export type RecordFormConfig = {
  ...
  /** When true, the helper text below the subdomain field is hidden. */
  readonly subDomainHelperHidden?: boolean;
  /** i18n key for a custom helper text below the subdomain field. Overrides the default helper. */
  readonly subDomainHelperKey?: string;
  /** When true, the subdomain field is read-only (e.g. DMARC forces _dmarc). */
  readonly subDomainReadOnly?: boolean;
  /** Fixed suffix appended to the subdomain value on submit (e.g. "._domainkey" for DKIM). Also displayed visually. */
