...
export const DeleteEntryModal = ({
  ...
  const { addWarning, addSuccess, clearNotifications } = useNotifications();

  const { mutate: onDelete } = useMutation<
    void,
    ApiError,
    { serviceName: string; recordId: string }
  >({
    ...
    onSuccess: () => {
      clearNotifications();
      addSuccess(t('zone_page_delete_entry_modal_success'), true);
      onCloseCallback?.();
    },
    onError: (error: ApiError) => {
      clearNotifications();
      addWarning(
        t('zone_page_delete_entry_modal_error', {
          domain: `${record?.subDomain}.${record?.zone}`,
  ...
  return (
    ...
    >
      <Text preset={TEXT_PRESET.span}>
        Voulez-vous réellement supprimer l'entrée suivante de la zone DNS du
        domaine ?
      </Text>
      {record && (
      ...
      >
        ...
        <MessageBody>{t('zone_page_delete_entry_modal_warning')}</MessageBody>
      </Message>
    </Modal>
  );
};
