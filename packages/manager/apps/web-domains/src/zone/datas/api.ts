...
export const validateZoneRecord = async (
  ...
  const conflictingIds = excludeRecordId
    ? data.filter((id) => String(id) !== String(excludeRecordId))
    : data;
...
/**
 * Zone status (deployment state + errors/warnings).
 * GET /domain/zone/{serviceName}/status
 */
...
export const getZoneStatus = async (
  ...
  const { data } = await v6.get<ZoneStatus>(
    `/domain/zone/${serviceName}/status`,
  );
  return data;
};
