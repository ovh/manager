...
const DiffLineRenderer = ({ line, lineNumber }: DiffLineRendererProps) => {
  if (line.charDiffs && (line.type === 'added' || line.type === 'removed')) {
    return (
      ...
      >
        <span className="select-none text-right pr-3 pl-3 text-gray-400 w-12 shrink-0 border-r border-gray-200 bg-gray-100">
          {lineNumber}
        </span>
        <div
          className={`min-h-5 break-words px-4 flex-1 ${
  ...
  return (
    <div className={`font-mono text-sm flex ${getLineColor(line.type)}`}>
      <span className="select-none text-right pr-3 pl-3 text-gray-400 w-12 shrink-0 border-r border-gray-200 bg-gray-100">
        {lineNumber}
      </span>
      <div
        className={`min-h-7 break-words px-4 flex-1 ${getLineTextColor(
          line.type,
        )}`}
      >
        {line.content || '\u00A0'}
