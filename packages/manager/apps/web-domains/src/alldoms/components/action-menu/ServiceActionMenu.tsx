...
import { ShellContext, useNavigationGetUrl } from '@ovh-ux/manager-react-shell-client';

import { RENEW_URL, allDomManagerService } from '@/alldoms/constants';
import { ActionEnum, LifecycleCapacitiesEnum } from '@/alldoms/enum/service.enum';
...
export default function ServiceActionMenu({
  ...
  const renewUrl = `${RENEW_URL[user.ovhSubsidiary] || RENEW_URL.default}${serviceName}`;
  ...
  switch (whichAction) {
    ...
    case ActionEnum.OnlyRenew:
      items = [renewCGIAction, renewalAction, disableAction ? cancelTerminate : terminateAction];
      break;
    default:
      items = [
        renewCGIAction,
  ...
  return <ActionMenu id={id} isCompact variant={ODS_BUTTON_VARIANT.ghost} items={items} />;
}
