...
export default function ServiceDetailInformation({ currentState }: ServiceDetailInformationProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-y-3">
        ...
        <div>
          <OdsText>
            <strong>{t('allDom_page_detail_information_general_extensions')}</strong>
          </OdsText>
          <OdsText preset={ODS_TEXT_PRESET.span}>
            {currentState.extensions.map((extension) => `.${extension.toLowerCase()}`).join('; ')}
          </OdsText>
        </div>
      </div>
    </OdsCard>
