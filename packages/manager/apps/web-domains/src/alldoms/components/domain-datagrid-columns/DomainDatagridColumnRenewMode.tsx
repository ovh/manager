...
export default function DomainDatagridColumnRenewMode({
  ...
  const { data, isLoading } = useGetServiceInformation(serviceName, ServiceRoutes.Domain);
  ...
  let badgeColor =
    mode === ServiceInfoRenewMode.Automatic ? ODS_BADGE_COLOR.success : ODS_BADGE_COLOR.warning;

  if (lifecycle?.capacities.actions.includes(LifecycleCapacitiesEnum.TerminateAtExpirationDate)) {
