...
export const useCancelAllDomTerminate = (serviceName: string, domains: TDomainsInfo[]) => {
  ...
  const {mutate} =  useMutation({
    mutationFn: async () => {
      await updateService(serviceName, ServiceInfoUpdateEnum.Empty, ServiceRoutes.AllDom);
      await Promise.all(
        domains.map((domain) =>
          updateService(
            domain.name,
  ...
  return { cancelAllDomTerminate: mutate };
};
