...
import { useAllDomDatagridColumns } from '@/alldoms/hooks/alldom-datagrid/useAllDomDatagridColumns';
...
export default function ServiceList() {
  ...
  useEffect(() => {
    if (allDomList) {
      const services = allDomList.map((alldom) => {
        ...
        const service = serviceList.find((s) => s.resource.name === alldom.currentState.name);
  ...
  return (
    <BaseLayout header={header} message={notifications.length ? <Notifications /> : null}>
