...
describe('DsRecordsForm', () => {
  ...
  it('Display an error when keyTag input is empty', async () => {
    ...
    const keyTagField = keyTagInput.closest('[data-ods="form-field"]');
    ...
    await waitFor(() => {
      const errorElement = keyTagField.querySelector('[data-ods="form-field-error"]');
  ...
  it('Display the getPublicKeyError error', async () => {
    ...
    const publicKeyField = textarea.closest('[data-ods="form-field"]');
    ...
    await waitFor(() => {
      const errorElement = publicKeyField.querySelector('[data-ods="form-field-error"]');
      expect(errorElement).toBeInTheDocument();
      expect(errorElement).toHaveTextContent(
        'domain_tab_dsrecords_drawer_form_publicKey_error',
      );
