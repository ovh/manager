...
vi.mock('@/domain/components/ContactCards/HolderCards', () => ({
  default: () => <div data-testid="holder-card">Holder card component</div>,
}));

vi.mock('@/domain/components/ContactCards/NichandleCards', () => ({
  default: () => (
...
describe('ContactManagement Component', () => {
  ...
  beforeEach(() => {
    ...
    (useNichandleInformations as jest.Mock).mockReturnValue({
      nichandleInformations: mockNichandleInfo,
    });
  });
  ...
  it('should render HolderCards with correct props', () => {
    render(<ContactManagement />);

    expect(screen.getByTestId('holder-card')).toBeInTheDocument();
  });

  it('should render NichandleCards for each contact type', () => {
    render(<ContactManagement />);
