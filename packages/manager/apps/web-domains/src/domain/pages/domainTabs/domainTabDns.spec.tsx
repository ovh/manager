...
describe('DomainTabDnsWithError', () => {
  ...
  it('displays the content of DNS datagrid', () => {
  ...
  it('shows warning message when a DNS has ERROR status', () => {
  ...
  it('renders DNS in the expected order', () => {
    (computeDnsDetails as jest.Mock).mockReturnValue(dnsDatagridMockError);
...
describe('DomainTabDns', () => {
  ...
  it('renders the two buttons with correct size and variant', () => {
    const { getByText } = render(
      <DomainTabDns domainResource={serviceInfoDetail} />,
      { wrapper },
    );

    const modifyBtn = getByText('domain_dns_tab_button_modify_dns');
    const anycastBtn = getByText('domain_dns_tab_button_order_anycast');

    expect(modifyBtn.className).toMatch(/button--sm/);
    expect(anycastBtn.className).toMatch(/button--sm/);
    expect(anycastBtn.className).toMatch(/button--outline/);
  });

  it('does not render a warning message when no DNS has ERROR status', () => {
