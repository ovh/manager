...
import { HubResponse, LastOrder } from '@/models/hub';
...
export default defineComponent({
  async setup() {
    ...
    const lastOrderResponse = await axios.get<HubResponse>('/engine/2api/hub/lastOrder');
    const lastOrder: LastOrder = lastOrderResponse.data.data.lastOrder.data;
    const orderStatus = await axios.get(`/engine/apiv6/me/order/${lastOrder?.orderId}/status`);
    const orderStatusData = orderStatus.data;
    const status = computed(() =>
      orderStatusData === 'delivered' ? 'INVOICE_IN_PROGRESS' : 'custom_creation',
    );
    ...
    const isWaitingPayment = computed(() => orderStatusData === WAITING_PAYMENT_LABEL);
