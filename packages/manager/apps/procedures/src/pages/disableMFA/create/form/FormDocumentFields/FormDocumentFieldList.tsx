...
import { acceptFile, maxFileSize, maxFiles } from './formDocumentFieldValidation.constants';
...
export const FormDocumentFieldList: FunctionComponent<Props> = ({
  ...
  if (!legal)
    throw Error(`the legalform "default" does not exist in the document form configuration file`);

  const sub = legal[subsidiary] ?? legal.DEFAULT;
  if (!sub)
    throw Error(`the subsidiary "DEFAULT" does not exist in the document form configuration file`);
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <FormDocumentFieldItem
              ...
              tooltips={field.tooltips.map((tooltip) => tdoc(tooltip, { maxFiles }))}
              disabled={disabled}
            />
          )}
