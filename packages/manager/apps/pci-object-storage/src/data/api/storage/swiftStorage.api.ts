...
export const createSwiftStorage = async ({
  ...
  const storageData = await apiClient.v6.post<storages.Container>(`/cloud/project/${projectId}/storage/`, container);
  await editSwiftStorage({ projectId, containerId: storageData.id, data: {
    containerType
  }});
  return storageData;
}
