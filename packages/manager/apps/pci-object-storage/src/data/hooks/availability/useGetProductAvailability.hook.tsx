...
import { OptionsFor, useQueryImmediateRefetch } from '@/hooks/useImmediateRefetch';
...
export function useGetProductAvailability(
  ...
  const user = useUser();
  const ovhSubsidiary = user?.ovhSubsidiary as OvhSubsidiaryEnum ??  OvhSubsidiaryEnum.FR;
  const queryKey = [projectId, 'capabilities/productAvailability', ovhSubsidiary];
