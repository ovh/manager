...
export function useAvailableRegions({regions, availabilities, options = {
  ...
  const availableRegions: Region[] = useMemo(() => {
    ...
    const filteredRegions = uniqueRegions.filter(region => {
      ...
      if (options.filterServiceName) {
        ...
        if (
          projectRegion &&
          !projectRegion.services.some(service => service.name === options.filterServiceName)
        ) {
          return false;
        }
      }
    ...
    return regions.filter(r => filteredRegions.some(fr => fr.name === r.name));
