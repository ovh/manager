...
import { FilterComparator } from '@ovh-ux/manager-core-api/dist/types/filters';
import { TIndexedObject } from './useDatagridColumn';
import { TObject } from '@/api/data/container';

type TPaginatedObjects = {
  rows: (
    | TObject
    | {
        index: string;
...
interface S3ContainerContextType extends CommonContainerContextType {
  ...
  containerObjectsWithIndex: TIndexedObject[];
  containerObjects: TObject[];
  search: string | null;
  ...
  objectsColumns: DatagridColumn<TIndexedObject>[];
}
...
interface SwiftContainerContextType extends CommonContainerContextType {
  ...
  isPending: boolean;
  columns: DatagridColumn<TIndexedObject>[];
  paginatedObjects: TPaginatedObjects;
  ...
  handleSearch: () => void;
  removeFilter: (filter: FilterWithLabel) => void;
