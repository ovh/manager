...
import AddReplicationPage from './AddReplication.page';
import {
  useStorageEndpoint,
  useStorage,
...
const mockStorageReturn: UseStorageReturn = {
  isPending: false,
  storage: {
    id: 'container-id',
    name: 'test-container',
...
const mockContainerStorage: MockStorage = {
  id: 'container-id',
  name: 'test-container',
  region: 'GRA',
  s3StorageType: '1AZ',
...
const mockDestinationContainerStorage: MockStorage = {
...
describe('AddReplicationPage', () => {
  beforeEach(() => {
    ...
    vi.mocked(useStorage).mockReturnValue(mockStorageReturn);
    ...
    vi.mocked(useUpdateStorage).mockReturnValue({
      updateContainer: vi.fn(),
      addReplicationRule: vi.fn(),
      mutate: vi.fn(),
      mutateAsync: vi.fn(),
  ...
  it('should render form fields when data is loaded', async () => {
    const { container } = render(<AddReplicationPage />, {
      wrapper: replicationTestWrapper,
    });

    const breadcrumbItem = container.querySelector(
      '[label="containers/replication/add:pci_projects_project_storages_containers_replication_add_replication_rule_sub_title"]',
    );
    expect(breadcrumbItem).toBeInTheDocument();
  });

  it('should show error when priority is already taken', async () => {
    vi.mocked(useServerContainer).mockReturnValue({
      data: {
        ...mockContainer,
        replication: {
          rules: [
            {
              ...
              deleteMarkerReplication: STATUS_DISABLED,
            },
          ],
        },
      },
      ...
      return {
        ...
        Outlet: () => {
          ...
          const handlePriorityChange = (
            ...
            setPriority(value);
            if (value === '1') {
              setError('This priority is already used');
            } else {
              setError('');
            }
          };
          ...
          return (
            <div>
              <form>
                ...
                />
                {error && <div data-testid="priority-error">{error}</div>}
              </form>
            </div>
          );
        },
    ...
    const priorityInput = await screen.findByTestId(
      'replication-rule-priority-input',
    );
    fireEvent.change(priorityInput, { target: { value: '1' } });

    expect(await screen.findByTestId('priority-error')).toBeInTheDocument();
    expect(
      await screen.findByText(/This priority is already used/),
    ).toBeInTheDocument();
  });
});
