...
- **Nav derrière le flag `hpc-backup-licenses`** : l'entrée est gatée normalement dans
  `packages/manager/apps/container/src/container/nav-reshuffle/sidebar/navigation-tree/services/hostedPrivateCloud.ts`
  et `.../legacy/server-sidebar/universe/HostedPrivateCloudSidebar.tsx`. Ce flag (CFF) n'est déclaré nulle
  part dans le repo : il vient de la feature-availability côté serveur. Rien ne s'affichera dans le vrai
  nav tant qu'il n'y est pas déclaré — c'est le prérequis au test visuel, pas un contournement à écrire
  dans le code.
