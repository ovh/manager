...
const confirmLabel = `${NAMESPACES.ACTIONS}:confirm`;
const acceptLabel = 'domain_operations_foa_accept';
const rejectLabel = 'domain_operations_foa_reject';

/** Both gates of the page : an explicit outcome and the certification. */
const certifyAndPick = async (outcome: string) => {
  fireEvent.click(getRadio(outcome));
  fireEvent.click(getCheckbox());
  await waitFor(() => {
    expect(getButton(confirmLabel).disabled).toBe(false);
  });
};
...
describe('UpdateFoaComponent', () => {
  ...
  it('preselects no outcome, an irreversible answer is never a default', async () => {
    mockQueries();
    ...
    // an irreversible answer is never a default
    expect(getRadio(acceptLabel)).not.toBeChecked();
    expect(getRadio(rejectLabel)).not.toBeChecked();
  ...
  it('confirms only once an outcome is picked and the certification ticked', async () => {
    mockQueries();
    render(<UpdateFoaComponent />, { wrapper });

    expect(getButton(confirmLabel).disabled).toBe(true);
    ...
    fireEvent.click(getRadio(acceptLabel));
  ...
  it('leaves the confirmation locked when only an outcome is picked', async () => {
    mockQueries();
    render(<UpdateFoaComponent />, { wrapper });

    fireEvent.click(getRadio(rejectLabel));

    await waitFor(() => {
      expect(getRadio(rejectLabel)).toBeChecked();
    });
    expect(getButton(confirmLabel).disabled).toBe(true);
  });

  it('lays the actions out with the primary one last', () => {
    mockQueries();
    render(<UpdateFoaComponent />, { wrapper });
  ...
  it('validates every still pending foa of the task on accept', async () => {
    ...
    await certifyAndPick(acceptLabel);
    fireEvent.click(getButton(confirmLabel));
  ...
  it('skips the foas already answered and rejects with the reject choice', async () => {
    ...
    await certifyAndPick(rejectLabel);
    fireEvent.click(getButton(confirmLabel));
  ...
  it('is not reachable when every foa has already been answered', () => {
    mockQueries({
      foas: [{ id: 'foa-1', currentState: { choice: FoaChoiceEnum.Accept } }],
    });
    render(<UpdateFoaComponent />, { wrapper });
