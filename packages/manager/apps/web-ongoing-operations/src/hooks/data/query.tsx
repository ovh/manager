...
export const useScheduledTradeTask = (domainName: string, enabled = true) => {
  return useQuery<TDomainTaskV2 | null>({
    ...
    // The listing is already narrowed to the scheduled trades of the domain,
    // and a domain carries at most one : the first task is the one to answer
    queryFn: async () => (await getScheduledTradeTasks(domainName))[0] ?? null,
    enabled: !!domainName && enabled,
    staleTime: 0,
  });
};
