...
var UI = {
  ...
  start(callback) {
    const html = ''; let i; let sheet; let sheets; let llevels; let port; let
      autoconnect;
  ...
  togglePopupStatusPanel() {
    const psp = $D('noVNC_popup_status_panel');
  ...
  updateState(rfb, state, oldstate, msg) {
    let s; let sb; let c; let d; let cad; let vd; let
      klass;
    UI.rfb_state = state;
    switch (state) {
      case 'failed':
  ...
  updateVisualState() {
    const connected = UI.rfb_state === 'normal';
  ...
  updateDocumentTitle(rfb, name) {
    document.title = `${name} - noVNC`;
  },


  clipReceive(rfb, text) {
  ...
  connect() {
    let host; let port; let password; let
      path;
  ...
  clipSend() {
    const text = $D('noVNC_clipboard_text').value;
  ...
  setViewClip(clip) {
    let display; let cur_clip; let pos; let new_w; let
      new_h;
  ...
  setViewDrag(drag) {
    const vmb = $D('noVNC_view_drag_button');
    if (!UI.rfb) { return; }
  ...
  keyInputBlur() {
    ...
    UI.hideKeyboardTimeout = setTimeout(() => { UI.setKeyboard(); }, 100);
  },
  ...
  setBarPosition() {
    ...
    const vncwidth = $D('noVNC_screen').style.offsetWidth;
