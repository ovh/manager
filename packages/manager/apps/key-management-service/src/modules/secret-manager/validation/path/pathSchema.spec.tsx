...
import {
  PATH_INVALID_CHARACTERS_NOT_ALLOWED,
  PATH_INVALID_CONTAINS_TWO_CONSECUTIVE_SLASHES,
  PATH_INVALID_ENDS_WITH_SLASH,
  PATH_INVALID_STARTS_WITH_SLASH,
  PATH_VALID,
...
describe('PathSchema test suite', () => {
  ...
  it('should return the correct error message for a path with disallowed characters', () => {
    const path = PATH_INVALID_CHARACTERS_NOT_ALLOWED;
    const result = getSchemaParsingResult(path);
    expect(result.success).toBe(false);
    expect(result.error.issues[0].message).toBe(
