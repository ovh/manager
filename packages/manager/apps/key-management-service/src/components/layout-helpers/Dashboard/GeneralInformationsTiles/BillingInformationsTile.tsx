...
const BillingInformationsTile = ({
  ...
  // Generate the contact URL for managing contacts
  useEffect(() => {
    navigation
      .getURL('dedicated', '#/contacts/services', {})
      .then((url) => setContactUrl(url as string));
  }, [navigation]);
  ...
  const tileItems: DashboardTileBlockItem[] = [
    {
      id: 'creationDate',
      label: t('key_management_service_dashboard_field_label_creation_date'),
      value: (
        <TileValueDate
        ...
        <span>
          <OdsBadge
            ...
            label={
              ...
              t('key_management_service_dashboard_field_label_engagement_none')
            }
          />
        </span>
      ),
  ...
  if (availability?.[KMS_FEATURES.DISPLAY_CONTACTS])
    tileItems.push({
      id: 'contacts',
  ...
  return (
    <>
      <DashboardTile title={t('billing_informations')} items={tileItems} />
      <Outlet />
    </>
  );
};
