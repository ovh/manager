...
const LocalizationSelect = <TCustomData extends TCustomRegionItemData>({
  ...
  return (
    <FormField className={className}>
      <FormFieldLabel>{t('regions:manager_components_select_localization_label')}</FormFieldLabel>
      <Select items={regions} {...props}>
        <SelectControl
          className="h-[2.5em]"
          placeholder={t('regions:manager_component_select_localization_placeholder')}
          customItemRenderer={({ selectedItems }: SelectCustomItemRendererArg) => (
            <>
              {selectedItems[0] && (
                <Localization
                  name={t(selectedItems[0].label, {
                    micro: (selectedItems[0].customRendererData as TCustomData).regionId,
                  })}
                  countryCode={(selectedItems[0].customRendererData as TCustomData).countryCode}
                  deploymentMode={
                    (selectedItems[0].customRendererData as TCustomData).deploymentMode
                  }
                />
              )}
            </>
        ...
        <SelectContent
          ...
          customGroupRenderer={({ label }) => (
            <span>{t(`common:pci_instances_common_instance_continent_${label}`)}</span>
          )}
          customOptionRenderer={({ label, customData }: SelectCustomOptionRendererArg) => (
            <Localization
              name={t(label, {
