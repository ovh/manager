export const NotebookRoadmapLinks = [
  {
    label: 'Changelog',
    url:
      'https://github.com/orgs/ovh/projects/16/views/6?pane=info&sliceBy%5Bvalue%5D=AI+Notebooks',
  },
  {
    label: 'Roadmap',
    url:
      'https://github.com/orgs/ovh/projects/16/views/1?pane=info&sliceBy%5Bvalue%5D=AI+Notebooks',
  },
  {
    label: 'Feature Request',
    url:
      'https://github.com/ovh/public-cloud-roadmap/issues/new?assignees=&labels=&projects=&template=feature_request.md&title=',
  },
];

export const QpuRoadmapLinks = [
  {
    label: 'Changelog',
    url:
      'https://github.com/orgs/ovh/projects/16/views/6?sliceBy%5Bvalue%5D=Quantum+QPU',
  },
  {
    label: 'Roadmap',
    url:
      'https://github.com/orgs/ovh/projects/16/views/1?sliceBy%5Bvalue%5D=Quantum+QPU',
  },
  {
    label: 'Feature Request',
    url:
      'https://github.com/ovh/public-cloud-roadmap/issues/new?assignees=&labels=&projects=&template=feature_request.md&title=',
  },
];

export const EmulatorRoadmapLinks = [
  {
    label: 'Changelog',
    url:
      'https://github.com/orgs/ovh/projects/16/views/6?sliceBy%5Bvalue%5D=Quantum+Emulators',
  },
  {
    label: 'Roadmap',
    url:
      'https://github.com/orgs/ovh/projects/16/views/1?sliceBy%5Bvalue%5D=Quantum+Emulators',
  },
  {
    label: 'Feature Request',
    url:
      'https://github.com/ovh/public-cloud-roadmap/issues/new?assignees=&labels=&projects=&template=feature_request.md&title=',
  },
];

export const JobsRoadmapLinks = [
  {
    label: 'Changelog',
    url:
      'https://github.com/orgs/ovh/projects/16/views/1?pane=info&sliceBy%5Bvalue%5D=AI+Training',
  },
  {
    label: 'Roadmap',
    url:
      'https://github.com/orgs/ovh/projects/16/views/1?pane=info&sliceBy%5Bvalue%5D=AI+Training',
  },
  {
    label: 'Feature Request',
    url:
      'https://github.com/ovh/public-cloud-roadmap/issues/new?assignees=&labels=&projects=&template=feature_request.md&title=',
  },
];

export const DashboardRoadmapLinks = [
  {
    label: 'Changelog',
    url:
      'https://github.com/orgs/ovh/projects/16/views/6?pane=info&sliceBy%5Bvalue%5D=AI+Tools',
  },
  {
    label: 'Roadmap',
    url:
      'https://github.com/orgs/ovh/projects/16/views/1?pane=info&sliceBy%5Bvalue%5D=AI+Tools',
  },
  {
    label: 'Feature Request',
    url:
      'https://github.com/ovh/management-security-operations-roadmap/issues/new',
  },
];
