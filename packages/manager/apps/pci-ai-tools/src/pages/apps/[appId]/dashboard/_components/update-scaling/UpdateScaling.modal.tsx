...
const UpdateScaling = () => {
  ...
  const form = useForm<FullScalingFormValues>({
    resolver: zodResolver(baseScalingSchema),
    defaultValues: {
      autoScaling: currentScaling.autoScaling ?? false,
      replicas: currentScaling.replicas ?? 1,
      ...getInitialValues(currentScaling),
    },
    mode: 'onChange',
  });
  ...
  const onSubmit = form.handleSubmit((formValues) => {
    ...
    const scalingInfo: ai.app.ScalingStrategyInput = formValues.autoScaling
      ...
      : { fixed: { replicas: formValues.replicas || 1 } };
