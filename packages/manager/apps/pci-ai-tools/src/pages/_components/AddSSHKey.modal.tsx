...
const AddSSHKey = () => {
  ...
  const getPopupTrackingName = (action: 'confirm' | 'cancel') => {
    if (isQpu) {
      return TRACKING.qpus.popup.configureSshKeyClick(action);
    }
    if (isQuantum) {
      return TRACKING.emulators.popup.configureSshKeyClick(action);
    }
    return null;
  };
  const getBannerTrackingName = (
    ...
    if (isQpu) {
      return TRACKING.qpus.banner.configureSshKeyBanner(type, status);
    }
    if (isQuantum) {
      return TRACKING.emulators.banner.configureSshKeyBanner(type, status);
    }
    return null;
  };
