...
import { TClusterPlan, TClusterPlanEnum } from '@/types';
  ...
  const plans: TPlan[] = [
    {
      ...
      code: codes.find((code) => code.includes(TClusterCodePlanEnum.FREE)) ?? null,
    },
    {
      ...
      code: codes.find((code) => code.includes(TClusterCodePlanEnum.STANDARD)) ?? null,
    },
  ].map((plan) => ({
    ...plan,
