...
export default function EditNetworkPage() {
  ...
  const isIpValid = gatewayForm.mode === ModeEnum.CUSTOM ? !!gatewayForm.ip : true;

  const hasLoadBalancersChanges = kubeDetail?.loadBalancersSubnetId !== loadBalancerSubnet?.id;

  const hasChanges = hasGatewayChanges || hasLoadBalancersChanges;

  const shouldWarnLoadBalancerSubnet = loadBalancerSubnet && !loadBalancerSubnet.gatewayIp;
  ...
  const onSubmit = async () => {
    ...
    editNetwork(
      ...
      hasGatewayChanges && {
        ...
        defaultVrackGateway: gatewayForm.mode === ModeEnum.CUSTOM ? gatewayForm.ip : '',
      },
    )
      .then(async () => {
  ...
  return (
    ...
    >
      ...
      <OsdsButton
        slot="actions"
        color={ODS_THEME_COLOR_INTENT.primary}
        disabled={isPending || isSubmitting || !hasChanges || !isIpValid || undefined}
        onClick={onSubmit}
