...
describe('Considering CartContent component', () => {
  test('Should return expected items', () => {
    render(
      <CartContent
        ...
        renderCartItem={({ item, isExpanded }) => (
          <CartItem key={item.title} value={item.title} data-testid={item.title}>
            <p>{`${item.name} ${isExpanded(item.id) ? 'expanded' : 'collapsed'}`}</p>
          </CartItem>
        )}
      />,
    );
    ...
    const instanceElement = screen.getByTestId(`cart-item-${instanceItemTitle}`);
