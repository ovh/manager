...
export type RegionListProps = {
  ...
  render: (region: TLocation, isSelected: boolean) => JSX.Element | string;
  isUnselectableRegion?: boolean;
};

export const regionContainerClassName =
...
export function RegionList({
  ...
  const isDisabledRegion = (region: TLocation) =>
    isUnselectableRegion && !region.enabled;
    ...
    <ul className={regionContainerClassName}>
      {regions.map((region) => (
        <li className="w-full px-1" key={region.name}>
          <OsdsTile
            disabled={isUnselectableRegion || undefined}
            className={clsx(
              regionTile,
              region.name === selectedRegion && regionTileSelected,
            )}
