...
export const useKubeNetwork = ({
  projectId,
  kubeId,
}: Readonly<UseKubeNetworkProps>) => {
  const { data: kubeDetail, isPending: isKubePending } = useKubeDetail(
    projectId,
    kubeId,
  );
  ...
  const kubeSubnet = useMemo(
    () =>
      isPending
        ? null
        : availableSubnets.find((sub) => sub.id === kubeDetail.nodesSubnetId),
