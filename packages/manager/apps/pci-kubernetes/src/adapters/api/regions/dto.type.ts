...
export type TOpenstackRegionStatusDTO = 'DOWN' | 'MAINTENANCE' | 'UP';

export type TServiceStatusDTO = 'DOWN' | 'UP';
export type TServiceDTO = {
  endpoint: string;
  name: string;
  status: TServiceStatusDTO;
};
...
export type TRegionDTO = {
  ...
  countryCode: TCountryCodeDTO;
  services: TServiceDTO[];
  status: TOpenstackRegionStatusDTO;
  ipCountries: IpCountryDTO[];
};

export type TCountryCodeDTO =
