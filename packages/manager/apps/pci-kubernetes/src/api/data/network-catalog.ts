...
export type TNetworkCatalog = {
  ipModels: TIpModel[];
  routersModels: TRouterModel[];
};

export const getNetworkCatalog = async (projectId: string): Promise<TNetworkCatalog> => {
  const { data } = await v6.get<TNetworkCatalog>(`/cloud/project/${projectId}/catalog/network`);
