...
import { productAvailabilityQueryKey } from '@/adapters/tanstack/productAvailability/queryKey';
import { TRegions } from '@/domain/entities/regions';
import { useParam } from '@/hooks/useParam';
import { TSelectOptions } from '@/types/query';

import { getAvailabitilityRegions } from '../data/available-regions';
...
export const useAvailabilityRegions = <TData>({ select }: TSelectOptions<TRegions, TData>) => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      getAvailabitilityRegions(projectId, {
        addonFamily: 'mks',
        ovhSubsidiary: me?.ovhSubsidiary ?? '',
      }),
    enabled: !!me,
