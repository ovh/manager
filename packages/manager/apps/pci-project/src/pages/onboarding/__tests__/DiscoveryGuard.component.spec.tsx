...
import { TEligibility, TEligibilityPaymentMethod } from '@/data/models/Eligibility.type';
...
const mockEligibility = (
  ...
  const { paymentMethodsAuthorized = [], isLoading = false, isError = false } = opts;
  return {
...
describe('DiscoveryGuard', () => {
  ...
  it('shows spinner when loading eligibility', () => {
    vi.mocked(useEligibility).mockReturnValue(mockEligibility({ isLoading: true }));
  ...
  it('renders children when not loading or redirecting', async () => {
    ...
    const { getByText, queryByTestId } = render(<DiscoveryGuard>children</DiscoveryGuard>, {
      wrapper,
    });
    await waitFor(() => {
