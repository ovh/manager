...
import { CartProductOption, CartProductType, OrderedProduct } from '@/data/models/Cart.type';
import {
  useIsAValidHdsSupportLevel,
  useIsHdsFeatureAvailabilityEnabled,
} from '@/hooks/use-hds/useHds';

import { UseHdsManagementProps, useHdsManagement } from '../useHdsManagement';
...
describe('useHdsManagement', () => {
  ...
  const mockAddItemMutationResult = (
    isPending = false,
  ): UseMutationResult<OrderedProduct, ApiError, AddItemToCartParams, unknown> =>
    ({
      mutate: vi.fn(),
      mutateAsync: vi.fn(),
      isPending,
      ...
      isPaused: false,
      submittedAt: 0,
    }) as UseMutationResult<OrderedProduct, ApiError, AddItemToCartParams, unknown>;
  ...
  beforeEach(() => {
    ...
    vi.mocked(useGetHdsAddonOption).mockReturnValue(mockQueryResult(mockHdsAddonOption));
    vi.mocked(useAddItemToCart).mockReturnValue(mockAddItemMutationResult());
    vi.mocked(useRemoveItemFromCart).mockReturnValue(mockRemoveItemMutationResult());
  });
  ...
  describe('isHdsPending', () => {
    ...
    it('should be true when adding HDS', () => {
      vi.mocked(useAddItemToCart).mockReturnValue(mockAddItemMutationResult(true));
    ...
    it('should be true when removing HDS', () => {
      vi.mocked(useRemoveItemFromCart).mockReturnValue(mockRemoveItemMutationResult(true));
  ...
  describe('handleHdsToggle', () => {
    it('should update form state and add HDS to cart when checking HDS', () => {
      ...
      const handleHdsToggleFn = result.current.handleHdsToggle as unknown as (
        checked: boolean,
      ) => void;
      handleHdsToggleFn(true);

      // Should update form state
      expect(mockSetForm).toHaveBeenCalledWith(expect.any(Function) as (prev: unknown) => unknown);
      const formUpdater = mockSetForm.mock.calls[0]?.[0] as
        | ((prev: unknown) => unknown)
        | undefined;
      expect(formUpdater).toBeDefined();
    ...
    it('should use fallback values when hdsAddonOption is missing price info', () => {
      ...
      vi.mocked(useGetHdsAddonOption).mockReturnValue(mockQueryResult(incompleteHdsOption));
  ...
  describe('mutation callbacks', () => {
    it('should handle successful HDS addition', () => {
      let onSuccessCallback: ((item: OrderedProduct) => void) | undefined;
  ...
  describe('return values', () => {
    it('should provide all expected return values', () => {
      ...
      expect(result.current).toEqual({
        shouldDisplayHdsSection: expect.any(Boolean) as boolean,
        isHdsPending: expect.any(Boolean) as boolean,
        handleHdsToggle: expect.any(Function) as (checked: boolean) => void,
      });
    });
  });
});
