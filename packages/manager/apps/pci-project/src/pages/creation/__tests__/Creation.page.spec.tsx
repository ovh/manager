...
const mockShellContext = {
  ...
  shell: {
    navigation: {
      getURL: vi.fn((appName: string, path: string) => Promise.resolve(`/${appName}${path}`)),
    },
  },
...
vi.mock('react-router-dom', async () => {
  ...
  return {
    ...
    useParams: () => mockUseParams() as { projectId?: string },
    useSearchParams: () =>
      [new URLSearchParams(), mockSetSearchParams] as [
        URLSearchParams,
...
vi.mock('@/data/hooks/useCart', () => ({
  useCreateAndAssignCart: () => mockUseCreateAndAssignCart() as unknown,
  useGetCart: () => mockUseGetCart() as unknown,
  useGetProjectItem: () => mockuseGetProjectItem() as unknown,
  useOrderProjectItem: () => mockUseOrderProjectItem() as unknown,
  useAttachConfigurationToCartItem: () => mockUseAttachConfigurationToCartItem() as unknown,
  useDeleteConfigurationItemFromCart: () => mockUseDeleteConfigurationItemFromCart() as unknown,
...
vi.mock('@/pages/creation/hooks/useConfigForm', () => ({
  useConfigForm: () => mockUseConfigForm() as unknown,
}));

// Mock useStepper hook
const mockUseStepper = vi.fn();

vi.mock('@/pages/creation/hooks/useStepper', () => ({
  Step: {
    Config: 0,
    Payment: 1,
    CreditConfirmation: 2,
  },
  useStepper: () => mockUseStepper() as unknown,
}));

// Mock useProjectCreation hook
const mockUseProjectCreation = vi.fn();

vi.mock('@/pages/creation/hooks/useProjectCreation', () => ({
  useProjectCreation: () => mockUseProjectCreation() as unknown,
}));

// Mock useWillPayment hook
const mockUseWillPayment = vi.fn();

vi.mock('@/pages/creation/hooks/useWillPayment', () => ({
  useWillPayment: () => mockUseWillPayment() as unknown,
}));

// Mock useAntiFraud hook
const mockUseAntiFraud = vi.fn();

vi.mock('@/pages/creation/hooks/useAntiFraud', () => ({
  default: () => mockUseAntiFraud() as unknown,
}));
...
vi.mock('@/hooks/payment/usePaymentRedirect', () => ({
  usePaymentRedirect: () => mockUsePaymentRedirect() as unknown,
}));
...
vi.mock('@/pages/creation/steps/ConfigStep', () => ({
  default: () => <div data-testid="config-step">Config Step</div>,
}));

vi.mock('@/pages/creation/steps/PaymentStep', () => ({
  default: () => <div data-testid="payment-step">Payment Step</div>,
}));

vi.mock('@/pages/creation/components/credit/CreditConfirmation.component', () => ({
  default: () => <div data-testid="credit-confirmation">Credit Confirmation</div>,
}));
