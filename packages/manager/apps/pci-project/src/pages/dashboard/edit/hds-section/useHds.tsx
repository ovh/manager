...
export const useIsHdsFeatureAvailabilityEnabled = (): boolean => {
  ...
  return availability?.[FEATURE_AVAILABILITY.HDS] || false;
};
 ...
 * 1. Billing plan code matching PCI_HDS_ADDON.planCodeScope
  ...
  useQuery({
    ...
    select: (serviceOptions: TServiceOption[]) => {
      const hasCertification = (option: TServiceOption): boolean => {
        const hasPlanCode = option.billing?.plan?.code.match(
