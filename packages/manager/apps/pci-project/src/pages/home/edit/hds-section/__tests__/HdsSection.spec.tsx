...
import { Cart, CartSummary, PaymentMean } from '@/data/models/Cart.type';
import { TCartServiceOption } from '@/data/models/Service.type';
...
vi.mock('@/hooks/use-checkout/useCheckout', () => ({
...
function mockMutationResult(options: Partial<HdsMutationResult> = {}): HdsMutationResult {
  return {
    mutate: vi.fn(),
    isPending: false,
    ...options,
...
describe('HdsSection', () => {
  ...
  it('renders the title and HdsOption', () => {
    ...
    vi.mocked(useHdsModule.usePrepareHdsCart).mockReturnValue(mockQueryResult<Cart>(mockCart));
    vi.mocked(useCartModule.useGetCartSummary).mockReturnValue(
      mockQueryResult<CartSummary>(mockCartSummary),
    );
    vi.mocked(useCheckoutWithFidelityAccount).mockReturnValue(mockMutationResult());

    render(<HdsSection project={mockProject} />, { wrapper: createWrapper() });

    expect(screen.getByText('pci_projects_project_edit_hds_title')).toBeInTheDocument();
  ...
  it('disables HdsOption if already certified', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('hds-checkbox')).toHaveAttribute('is-disabled', 'true');
    });
  });
  ...
  it('shows contracts section when HDS is checked and not certified', async () => {
    ...
    fireEvent(hdsCheckbox, new CustomEvent('odsChange', { detail: { checked: true } }));
  ...
  it('disables validate button unless contracts are checked', async () => {
    ...
    fireEvent(contractsCheckbox, new CustomEvent('odsChange', { detail: { checked: true } }));
  ...
  it('shows skeleton in contracts section when contracts are loading', async () => {
    ...
    vi.mocked(useHdsModule.usePrepareHdsCart).mockReturnValue(mockQueryResult<Cart>(mockCart));
    vi.mocked(useCartModule.useGetCartSummary).mockReturnValue(
      mockQueryResult<CartSummary | undefined>(undefined, true) as unknown as UseQueryResult<
        CartSummary,
        Error
      >,
    );
