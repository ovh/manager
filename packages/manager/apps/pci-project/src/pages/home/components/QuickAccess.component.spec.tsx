...
describe('QuickAccess', () => {
  ...
  it('should render all quick access items when all features are available', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('pci_projects_home_quick_access'),
      ).toBeInTheDocument();
    });
    ...
    expect(
      screen.getByText('pci_projects_home_kubernetes'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('pci_projects_home_object_storage'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('pci_projects_home_block_storage'),
    ).toBeInTheDocument();
  ...
  it('should filter out items when features are not available', async () => {
    ...
    expect(
      screen.queryByText('pci_projects_home_kubernetes'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('pci_projects_home_block_storage'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('pci_projects_home_database'),
    ).not.toBeInTheDocument();
  });
  ...
  it('should render no items when no features are available', async () => {
    ...
    expect(
      screen.queryByText('pci_projects_home_instances'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('pci_projects_home_kubernetes'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('pci_projects_home_object_storage'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('pci_projects_home_block_storage'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('pci_projects_home_network'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('pci_projects_home_database'),
    ).not.toBeInTheDocument();
