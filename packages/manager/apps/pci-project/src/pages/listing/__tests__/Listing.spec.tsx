...
const {
  mockGetDatagridColumns,
  mockUseRedirectAfterProjectSelection,
  mockGetProjectsWithServices,
} = vi.hoisted(() => ({
  mockGetDatagridColumns: vi.fn<() => unknown[]>(),
  mockUseRedirectAfterProjectSelection: vi.fn<() => unknown>(),
  mockGetProjectsWithServices: vi.fn<() => Promise<unknown>>(),
}));

vi.mock('./datagrid-columns', () => ({
  getDatagridColumns: mockGetDatagridColumns,
}));

vi.mock('@/hooks/useRedirectAfterProjectSelection', () => ({
  __esModule: true,
  default: mockUseRedirectAfterProjectSelection,
}));

vi.mock('@/data/api/projects-with-services', () => ({
  getProjectsWithServices: (): Promise<unknown> => mockGetProjectsWithServices(),
  projectsWithServiceQueryKey: (): string[] => ['projects-with-services'],
}));
...
describe('listing page', () => {
  beforeEach(() => {
    ...
    // Set default mocks
    mockUseRedirectAfterProjectSelection.mockReturnValue({
      redirect: vi.fn(),
  ...
  it('displays datagrid skeleton while main request is loading', () => {
    ...
    (
      vi.mocked(useTrustedZoneBanner) as {
        mockReturnValue: (value: { isBannerVisible: boolean; isLoading: boolean }) => void;
      }
    ).mockReturnValue({
    ...
    mockGetDatagridColumns.mockReturnValue([
      {
        id: 'description',
        label: 'Description',
  ...
  it('displays create project button when trusted zone is not visible and not loading', () => {
    ...
    expect(screen.getByTestId('listing_create-project_button')).toBeInTheDocument();
  });
  ...
  it('does not display create project button when trusted zone is visible', () => {
    ...
    expect(screen.queryByTestId('listing_create-project_button')).not.toBeInTheDocument();
  });
