...
const PaymentMethods: React.FC<PaymentMethodsProps> = ({
  ...
  const internalHandlePaymentMethodChange = (method: TPaymentMethod) => {
  ...
  useEffect(() => {
    if (defaultPaymentMethod) {
      internalHandlePaymentMethodChange(defaultPaymentMethod);
      internalHandleSetAsDefaultChange(defaultPaymentMethod.default || false);
    }
        ...
        submitPaymentMethod: async () => {
          return new Promise<void>((resolve, reject) => {
            if (handlePaymentMethodChallenge && paymentChallengeRef.current) {
              return paymentChallengeRef.current
                .submitChallenge()
                .then((status) => {
                  switch (status) {
                    case 'deactivated':
                      ...
                      queryClient.invalidateQueries({
                        queryKey: paymentMathodQueryKey(
                          defaultPaymentMethodsParams,
                        ),
                      });
                      queryClient.invalidateQueries({
                        queryKey: eligibilityQueryKey(),
                      });
                      return reject(new Error('payment_method_deactivated'));
                    case 'retry':
                      return reject(new Error('challenge_retry'));
                    default:
