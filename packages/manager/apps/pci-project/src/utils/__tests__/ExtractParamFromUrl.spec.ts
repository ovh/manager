...
describe('extractParamFromUrl', () => {
  ...
  it('should handle URL encoded values', () => {
    ...
    expect(extractParamFromUrl({ url, paramName: 'message' })).toBe('hello world');
  });
  ...
  it('should return null for invalid inputs', () => {
    expect(extractParamFromUrl({ url: '', paramName: 'id' })).toBeNull();
    expect(extractParamFromUrl({ url: 'https://example.com', paramName: '' })).toBeNull();
    expect(extractParamFromUrl({ url: 'not-a-url', paramName: 'id' })).toBeNull();
  });
  ...
  it('should handle payment callback URLs', () => {
    const url = 'https://app.example.com/callback?paymentMethodId=pm_123#/dashboard?tab=payments';
    expect(extractParamFromUrl({ url, paramName: 'paymentMethodId' })).toBe('pm_123');
