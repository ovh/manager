...
import { getOrderDetailWithExtension, getOrderDetails, getOrderFollowUp } from '../order';

describe('order API', () => {
  // Mock data factories
  const createMockOrderDetail = (overrides: Partial<TOrderDetail> = {}): TOrderDetail => ({
    cancelled: false,
  ...
  const createMockFollowUpData = (additionalItems: TOrderFollowUp[] = []): TOrderFollowUp[] => [
    {
  ...
  // Helper functions
  const mockV6Get = vi.mocked(v6).get as ReturnType<typeof vi.fn>;
  ...
  const mockSequentialApiCalls = (successData: unknown[], error?: Error) => {
    if (error) {
      mockV6Get.mockResolvedValueOnce({ data: successData[0] }).mockRejectedValueOnce(error);
    } else {
      successData.forEach((data) => {
        mockV6Get.mockResolvedValueOnce({ data });
      });
    }
  };
  ...
  describe('getOrderFollowUp', () => {
    it('should fetch order follow up successfully', async () => {
      ...
      expect(mockV6Get).toHaveBeenCalledWith('/me/order/123/followUp');
  ...
  describe('getOrderDetails', () => {
    it('should fetch order detail IDs successfully', async () => {
      ...
      expect(mockV6Get).toHaveBeenCalledWith('/me/order/456/details');
  ...
  describe('getOrderDetailWithExtension', () => {
    it('should fetch order detail with extension successfully', async () => {
      ...
      const result = await getOrderDetailWithExtension(mockOrderId, mockDetailId);

      expect(mockV6Get).toHaveBeenCalledWith('/me/order/789/details/111');
      expect(mockV6Get).toHaveBeenCalledWith('/me/order/789/details/111/extension');
      expect(result).toEqual({
        ...mockOrderDetail,
    ...
    it('should handle API error for order detail', async () => {
      ...
      await expect(getOrderDetailWithExtension(mockOrderId, mockDetailId)).rejects.toThrow(
