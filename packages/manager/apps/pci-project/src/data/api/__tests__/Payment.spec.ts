...
describe('payWithRegisteredPaymentMean', () => {
  ...
  it('should call v6.post with correct endpoint and payload', async () => {
    ...
    expect(mockedV6Post).toHaveBeenCalledWith('me/order/123/payWithRegisteredPaymentMean', payload);
  });

  it('should throw error when API call fails', async () => {
    mockedV6Post.mockRejectedValue(new Error('API Error'));

    await expect(payWithRegisteredPaymentMean(123, { paymentMean: 'card' })).rejects.toThrow(
      'API Error',
    );
  });
});
