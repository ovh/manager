...
import {
  ...
  TEligibilityVoucher,
} from '@/data/models/Eligibility.type';

import { checkVoucherEligibility, getEligibility } from '../eligibility';

vi.mock('@ovh-ux/manager-core-api');

const mockV6 = vi.mocked(v6).get as ReturnType<typeof vi.fn>;
...
describe('eligibility API', () => {
  ...
  describe('checkVoucherEligibility', () => {
    it('should call the correct API endpoint with voucher parameter', async () => {
      ...
      expect(mockV6).toHaveBeenCalledWith('/cloud/eligibility?voucher=VOUCHER123');
    });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(checkVoucherEligibility('INVALID_VOUCHER')).rejects.toThrow('VOUCHER_INVALID');
    });
