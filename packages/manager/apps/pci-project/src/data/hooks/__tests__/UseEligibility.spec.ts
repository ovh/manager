...
import { eligibilityQueryKey, useCheckVoucherEligibility, useEligibility } from '../useEligibility';
...
describe('useCheckVoucherEligibility', () => {
  ...
  it('should call onSuccess when voucher eligibility check succeeds', async () => {
    ...
    const { result } = renderHook(() => useCheckVoucherEligibility({ onSuccess, onError }), {
      wrapper: createWrapper(),
    });
    ...
    (
      result.current as unknown as {
        mutate: (voucher: string) => void;
      }
    ).mutate('VALID_VOUCHER_CODE');

    await waitFor(() => {
      expect(result.current.isSuccess).toBe(true);
    });

    expect(mockCheckVoucherEligibility).toHaveBeenCalledWith('VALID_VOUCHER_CODE');
  ...
  it('should call onError when voucher eligibility check fails', async () => {
    ...
    ).mutate('INVALID_VOUCHER_CODE');

    await waitFor(() => {
      expect(result.current.isError).toBe(true);
    });

    expect(mockCheckVoucherEligibility).toHaveBeenCalledWith('INVALID_VOUCHER_CODE');
    expect(onError).toHaveBeenCalledWith(mockError, 'INVALID_VOUCHER_CODE', undefined);
  ...
  it('should handle voucher with zero credit value', async () => {
    ...
    const { result } = renderHook(() => useCheckVoucherEligibility({ onSuccess }), {
      wrapper: createWrapper(),
    });
