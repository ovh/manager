...
describe('useCart hooks', () => {
  ...
  describe('getCartSummaryQueryKey', () => {
    it('should return correct query key for cartId', () => {
      expect(getCartSummaryQueryKey('cart-1')).toEqual(['/order/cart/cart-1/summary']);
    });
  });
  ...
  describe('useCreateCart', () => {
    it('should create cart and order project successfully', async () => {
      ...
      vi.mocked(getPublicCloudOptions).mockResolvedValueOnce(mockCartProductOptions);
      ...
      const { result } = renderHook(
        () => useCreateCart(OvhSubsidiary.FR, PlanCode.PROJECT_2018, 'Test project'),
        { wrapper: createWrapper() },
      );
      ...
      expect(getPublicCloudOptions).toHaveBeenCalledWith('cart-1', PlanCode.PROJECT_2018);
      expect(orderCloudProject).toHaveBeenCalledWith('cart-1', PlanCode.PROJECT_2018);
      expect(attachConfigurationToCartItem).toHaveBeenCalledWith('cart-1', 123, {
        label: 'description',
        value: 'Test project',
      });
  ...
  describe('useOrderProjectItem', () => {
    it('should order project item successfully', async () => {
      ...
      expect(orderCloudProject).toHaveBeenCalledWith('cart-1', PCI_PROJECT_ORDER_CART.planCode);
  ...
  describe('useRemoveItemFromCart', () => {
    it('should remove item from cart successfully', async () => {
      ...
      const { result } = renderHook(() => useRemoveItemFromCart({ onSuccess }), {
        wrapper: createWrapper(),
      });
  ...
  describe('useAttachConfigurationToCartItem', () => {
    it('should attach configuration to cart item successfully', async () => {
      ...
      const { result } = renderHook(() => useAttachConfigurationToCartItem({ onSuccess }), {
        wrapper: createWrapper(),
      });
  ...
  describe('useGetHdsAddonOption', () => {
    it('should get HDS addon option when cartId is provided', async () => {
      ...
      expect(getPublicCloudOptions).toHaveBeenCalledWith('cart-1', PCI_PROJECT_ORDER_CART.planCode);
  ...
  describe('useDeleteConfigurationItemFromCart', () => {
    it('should delete configuration from cart item successfully', async () => {
      vi.mocked(deleteConfigurationItemFromCart).mockResolvedValueOnce(undefined);
      const onSuccess = vi.fn();

      const { result } = renderHook(() => useDeleteConfigurationItemFromCart({ onSuccess }), {
        wrapper: createWrapper(),
      });
      ...
      (
        result.current as unknown as {
          mutate: (variables: { cartId: string; itemId: number; configurationId: number }) => void;
        }
      ).mutate({
        cartId: 'cart-1',
        itemId: 123,
      ...
      expect(deleteConfigurationItemFromCart).toHaveBeenCalledWith('cart-1', 123, 456);
      expect(onSuccess).toHaveBeenCalled();
    });

    it('should handle delete configuration errors', async () => {
      const mockError = new Error('API Error');
      vi.mocked(deleteConfigurationItemFromCart).mockRejectedValueOnce(mockError);
      const onError = vi.fn();

      const { result } = renderHook(() => useDeleteConfigurationItemFromCart({ onError }), {
        wrapper: createWrapper(),
      });
    ...
    it('should work without onSuccess and onError callbacks', async () => {
      ...
      const { result } = renderHook(() => useDeleteConfigurationItemFromCart(), {
        wrapper: createWrapper(),
      });
  ...
  describe('useGetCartConfiguration', () => {
    ...
    it('should fetch and select configuration by label when cartId and itemId are provided', async () => {
      ...
      const { result } = renderHook(() => useGetCartConfiguration('description', 'cart-1', 123), {
        wrapper: createWrapper(),
      });
    ...
    it('should not fetch when cartId or itemId is not provided', () => {
      const { result } = renderHook(() => useGetCartConfiguration('description', undefined, 123), {
        wrapper: createWrapper(),
      });
    ...
    it('should return undefined when no matching configuration found', async () => {
      ...
      const { result } = renderHook(() => useGetCartConfiguration('nonexistent', 'cart-1', 123), {
        wrapper: createWrapper(),
      });
