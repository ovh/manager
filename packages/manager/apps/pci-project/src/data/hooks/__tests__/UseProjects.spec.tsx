...
import { getDefaultProject, removeProject, unFavProject } from '@/data/api/projects';
...
describe('useProjects hooks', () => {
    ...
    it('should call removeProject when project is not default', async () => {
      ...
      await (
        result.current as unknown as {
          mutateAsync: (p: typeof params) => Promise<void>;
        }
      ).mutateAsync(params);
  ...
  describe('useIsDefaultProject', () => {
    it('should return true when projectId matches default project', async () => {
      ...
      const { result } = renderHook(() => useIsDefaultProject('test-project-123'), {
        wrapper: createWrapper(),
      });
    ...
    it('should return false when projectId does not match default project', async () => {
      ...
      const { result } = renderHook(() => useIsDefaultProject('different-project'), {
        wrapper: createWrapper(),
      });
