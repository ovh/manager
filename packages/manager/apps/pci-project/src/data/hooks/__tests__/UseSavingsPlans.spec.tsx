...
import { useHasActiveOrPendingSavingsPlan, useSavingsPlans } from '../useSavingsPlans';
...
describe('useSavingsPlans hooks', () => {
  ...
  describe('useSavingsPlans', () => {
    it('should fetch savings plans when feature is available', async () => {
      ...
      const { result } = renderHook(() => useSavingsPlans('service-123', true), {
    ...
    it('should not fetch savings plans when feature is not available', async () => {
      const { result } = renderHook(() => useSavingsPlans('service-123', false), {
        wrapper: createWrapper(),
      });
  ...
  describe('useHasActiveOrPendingSavingsPlan', () => {
    it('should return true when there is an active savings plan', async () => {
      ...
      const { result } = renderHook(() => useHasActiveOrPendingSavingsPlan('service-123', true), {
        wrapper: createWrapper(),
      });
    ...
    it('should not fetch savings plans when feature is not available', async () => {
      const { result } = renderHook(() => useHasActiveOrPendingSavingsPlan('service-123', false), {
        wrapper: createWrapper(),
      });
    ...
    it('should not fetch savings plans when serviceId is empty', async () => {
      const { result } = renderHook(() => useHasActiveOrPendingSavingsPlan('', true), {
        wrapper: createWrapper(),
      });
