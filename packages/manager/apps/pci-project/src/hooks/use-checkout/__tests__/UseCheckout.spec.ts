...
describe('useCheckoutWithFidelityAccount', () => {
  ...
  it('useCheckoutWithFidelityAccount calls payment API and triggers onSuccess', async () => {
    ...
    vi.mocked(paymentApi.payWithRegisteredPaymentMean).mockResolvedValue(undefined);

    const onSuccess = vi.fn();
    const onError = vi.fn();
    const { result } = renderHook(() => useCheckoutWithFidelityAccount({ onSuccess, onError }), {
      wrapper: createWrapper(),
    });

    await (
      result.current as unknown as {
        mutateAsync: (params: { cartId: string }) => Promise<void>;
      }
    ).mutateAsync({ cartId: 'cart-1' });
