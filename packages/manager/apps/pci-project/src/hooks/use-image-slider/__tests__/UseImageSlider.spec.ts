...
describe('useImageSlider', () => {
  ...
  it('should advance to next image after interval', () => {
    ...
    (act as unknown as (cb: () => void) => void)(() => {
      void vi.advanceTimersByTime(SLIDE_ANIMATION_INTERVAL);
    });
  ...
  it('should loop back to first image after reaching the end', () => {
    ...
    (act as unknown as (cb: () => void) => void)(() => {
      void vi.advanceTimersByTime(SLIDE_ANIMATION_INTERVAL * (SLIDE_IMAGES.length - 1));
    });

    expect(result.current.currentImage).toBe(SLIDE_IMAGES[SLIDE_IMAGES.length - 1]);
