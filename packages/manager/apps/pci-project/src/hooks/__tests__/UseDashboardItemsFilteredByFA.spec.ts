...
import { DashboardItem, DashboardItemConfig } from '@/data/types/Dashboard.type';
...
describe('useDashboardItemsFilteredByFA', () => {
  ...
  it('should return all items when no feature flags are present', () => {
    ...
    const { result } = renderHook(() => useDashboardItemsFilteredByFA(itemsWithoutFlags));
  ...
  it('should return all items when all features are available', () => {
    ...
    const { result } = renderHook(() => useDashboardItemsFilteredByFA(mockItems));

    expect(result.current).toEqual(mockItems);
    expect(mockUseFeatureAvailability).toHaveBeenCalledWith(['feature1', 'feature2'], {
      enabled: true,
    });
  });
  ...
  it('should work with DashboardItemConfig items', () => {
    ...
    const { result } = renderHook(() => useDashboardItemsFilteredByFA(mockConfigItems));
  ...
  it('should handle items with empty feature flags', () => {
    ...
    const { result } = renderHook(() => useDashboardItemsFilteredByFA(itemsWithEmptyFlags));
