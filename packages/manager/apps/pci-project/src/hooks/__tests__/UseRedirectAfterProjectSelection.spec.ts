...
describe('useRedirectAfterProjectSelection', () => {
  ...
  it('should return no redirect required when target is invalid', () => {
    mockUseSearchParams.mockReturnValue([new URLSearchParams('target={"invalid": true}'), vi.fn()]);
  ...
  it('should handle external navigation correctly', () => {
    ...
    expect(mockAssign).toHaveBeenCalledWith('https://external.com/test-project-id');
  });
  ...
  describe('redirectUrl function', () => {
    ...
    it('should handle redirect URL with parameters correctly', async () => {
      ...
      const mockGetURL = vi
        .fn()
        .mockResolvedValue('/public-cloud/test-path?key=value&projectId=test-project-id');
      if (shellContext.shell) {
        shellContext.shell.navigation.getURL = mockGetURL;
      }
      ...
      expect(url).toBe('/public-cloud/test-path?key=value&projectId=test-project-id');
    });
  });
});
