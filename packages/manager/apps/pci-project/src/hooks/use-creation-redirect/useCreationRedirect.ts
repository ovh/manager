...
export default function useCreationRedirect() {
  const { data: shouldRedirectToIncreaseQuota, isLoading: isQuotaLoading } =
    useIsAskIncreaseProjectsQuota();

  const { data: discoveryProject, isPending: isDiscoveryPending } = useDiscoveryProject();

  const shouldBlockCreation = shouldRedirectToIncreaseQuota || !!discoveryProject;
