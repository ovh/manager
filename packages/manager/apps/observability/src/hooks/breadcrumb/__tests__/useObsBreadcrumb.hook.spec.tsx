...
vi.mock('@ovh-ux/manager-react-shell-client', async () => {
  ...
  return {
    ShellContext: ReactModule.createContext({
      shell: {
        navigation: {
          getURL: vi.fn().mockResolvedValue('/observability'),
        },
      },
      environment: {},
    }),
  ...
  routes: [
    ...
    },
    {
      ...
      items: [
        { labelKey: 'breadcrumb:metrics', path: '/metrics' },
        { labelKey: 'breadcrumb:tenants', path: '/metrics/tenants' },
        { useSegmentAsLabel: true, path: '/metrics/tenants/:resourceName/:tenantId' },
...
describe('useObsBreadcrumb', () => {
  ...
  describe('route matching', () => {
    ...
    it('should match route with dynamic parameters', () => {
      ...
      expect(result.current[3]?.label).toBe('my-tenant-id'); // useSegmentAsLabel
    });
  ...
  describe('translations', () => {
    it('should translate label keys', () => {
      ...
      expect(mockT).toHaveBeenCalledWith('breadcrumb:metrics');
      expect(mockT).toHaveBeenCalledWith('breadcrumb:tenants');
      expect(result.current[1]?.label).toBe('Metrics');
      expect(result.current[2]?.label).toBe('Tenants');
    });

    it('should fallback to key suffix when translation not found', () => {
      ...
      // Should extract 'metrics' from 'breadcrumb:metrics'
      expect(result.current[1]?.label).toBe('metrics');
    });
  });

  describe('useSegmentAsLabel', () => {
    it('should use URL segment as label when configured', () => {
      const { result } = renderHook(() => useObsBreadcrumb({ config: testConfig }), {
  ...
  describe('href generation', () => {
    it('should generate correct hrefs for items with paths', () => {
      const { result } = renderHook(() => useObsBreadcrumb({ config: testConfig }), {
        wrapper: createWrapper(['/metrics/tenants']),
      });

      expect(result.current[1]?.href).toBe('/metrics');
      expect(result.current[2]?.href).toBe(''); // Last item has no path
    });

    it('should replace dynamic parameters in href', () => {
      const { result } = renderHook(() => useObsBreadcrumb({ config: testConfig }), {
        wrapper: createWrapper(['/metrics/tenants/my-resource/my-tenant/edit']),
      });

      const tenantItem = result.current[3];
      expect(tenantItem?.href).toBe('/metrics/tenants/my-resource/my-tenant');
    });
  });
