...
const mapToMRCTrackinPageParams =
  (pageName?: string) =>
  ({ type }: TrackingBannerParams): MRCTrackinPageParams => ({
    pageType: type ? (`banner-${type}` as PageType) : undefined,
    pageName,
  });

export function withTrackBanner<CallBack extends (...args: unknown[]) => unknown>(
...
export function useTrackBanner<CallBack extends (...args: unknown[]) => unknown>(
  trackingParams:
    | TrackingBannerParams
    | ((...params: Parameters<CallBack>) => TrackingBannerParams),
  callback?: CallBack,
  ...
  return useMemo(
    () => withTrackBanner(trackPage, pageTracking?.pageName ?? '', trackingParams, callback),
