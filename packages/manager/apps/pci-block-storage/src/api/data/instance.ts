...
export type TInstanceDTO = Pick<TInstance, 'id' | 'status' | 'name' | 'availabilityZone'>;

export async function getInstancesByRegion(
  projectId: string,
  regionId: string,
): Promise<TInstance[]> {
  const instancesDTOs = (await commonGetInstancesByRegion(projectId, regionId)) as TInstanceDTO[];
