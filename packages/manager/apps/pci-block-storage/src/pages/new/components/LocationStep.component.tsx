...
const PriceDisplay: React.FC<PriceDisplayProps> = ({ data, volumeCatalog }) => {
  ...
  const formattedPrice = useMemo(
    () =>
      getFormattedCatalogPrice(
        convertHourlyPriceToMonthly(
          getGroupLeastPrice(data, volumeCatalog.regions, volumeCatalog.models),
        ),
      ),
    [
      data,
  ...
  const deployments = useMemo(
    () =>
      volumeCatalog?.filters.deployment.map<TDeployment>((d) => ({
        ...
        price: <PriceDisplay data={d} volumeCatalog={volumeCatalog} />,
      })) || [],
    [volumeCatalog, regions],
  );
