...
> Universe: **Baremetal / Hub**

## Overview

This application is a single-page React app integrated into the OVHcloud Manager ecosystem.  
It manages **NAS-HA (Network Attached Storage - High Availability)** services with complete CRUD operations for partitions, access control (ACL), and snapshot policies.

**Migration Status**: ✅ Fully migrated from AngularJS (`@ovh-ux/manager-nasha`) to React with **99% functional parity**.

### Key Features

- ✅ **Service Management**: List, view, and configure NAS-HA services
- ✅ **Partition CRUD**: Create, read, update (size/description), delete partitions
- ✅ **ZFS Options**: Advanced partition configuration (atime, recordsize, sync)
- ✅ **Access Control (ACL)**: Manage IP-based access permissions
- ✅ **Snapshot Policies**: 
  - Create and delete custom snapshots
  - **NEW (2025-11-24)**: Enable/disable automatic snapshot types
- ✅ **Task Tracking**: Real-time monitoring of asynchronous operations
- ✅ **Order Integration**: Module Federation with ConfigoNasHa for new service orders
- ✅ **Metrics & Monitoring**: Storage usage visualization

### Architecture

The app is fully **flavor-driven**, meaning routing, tracking, and API configuration are centralized in [`src/App.constants.ts`](src/App.constants.ts).  
