...
describe('pollOnNewOkms', () => {
  ...
  const expirationInMs = 60_000; // 1 minute

  beforeEach(() => {
    vi.clearAllMocks();
    // Reset store to initial state using the public API
    clearPendingOrder();
  ...
  describe('when the order has expired', () => {
    it('should show warning and clear the pending order', async () => {
      // GIVEN - Expired order
  ...
  describe('when a new OKMS is found', () => {
    it('should clear pending order, invalidate queries, show success notification, and call the onSuccess callback', async () => {
