...
import { PATH_MAX_CHAR, PATH_MIN_CHAR, useSecretPathSchema } from './pathSchema';
...
describe('PathSchema test suite', () => {
  ...
  it('should return the correct error message for a path with disallowed characters', () => {
    ...
    const result = getSchemaParsingResult(path);
    expect(result.success).toBe(false);
    expect(result.error.issues[0].message).toBe(labels.secretManager.error_path_allowed_characters);
  });
  ...
  it.each([
    ...
    MOCK_PATH_INVALID_ENDS_WITH_SLASH,
    MOCK_PATH_INVALID_CONTAINS_TWO_CONSECUTIVE_SLASHES,
  ])('should return the correct error message for an incorrect structured path: %s', (path) => {
    const result = getSchemaParsingResult(path);
    expect(result.success).toBe(false);
    expect(result.error.issues[0].message).toBe(labels.secretManager.error_path_structure);
  });
  ...
  it('should return the correct error message for a path that is too long', () => {
    ...
    expect(result.error.issues[0].message).toBe(
      labels.common.form.error_max_chars.replace('{{ value }}', PATH_MAX_CHAR.toString()),
    );
  });
});
