...
import { SHOW_VALUE_TEST_IDS, ShowValueLink } from './ShowValueLink.component';
...
const renderComponent = async () => {
  ...
  return render(<ShowValueLink secret={SECRET_MOCK} />, { wrapper });
};

describe('ShowValueLink test suite', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('when data is loading', () => {
    it('should display a skeleton', async () => {
      ...
      // THEN
      expect(screen.getByTestId(SHOW_VALUE_TEST_IDS.skeleton)).toBeVisible();
    });
  });
  ...
  describe('When okms is loaded', () => {
    it('should be enabled when OKMS domain status is OK, with correct parameters', async () => {
      ...
      // THEN
      const link = screen.getByTestId(SHOW_VALUE_TEST_IDS.showValueLink);

      await waitFor(() => {
        expect(link).not.toHaveAttribute('is-disabled');
      });
      expect(link).toHaveAttribute('label', labels.secretManager.reveal_secret);
