...
import {
  UseDeactivateVersionParams,
  useDeactivateVersionItem,
} from './useDeactivateVersionMenuItem';
...
const deactivateMenuItem: ActionMenuItemProps = {
  id: MOCKED_ID,
...
const renderTestHook = async (values: UseDeactivateVersionParams) => {
  const wrapper = await testWrapperBuilder().withI18next().withQueryClient().build();

  return renderHook(() => useDeactivateVersionItem(values), { wrapper });
};

describe('useDeactivateVersionMenuItem test suite', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  const testCases: TestCase[] = [
    { version: versionActiveMock, returned: deactivateMenuItem },
