...
import { SavingsPlanCellData } from './Table.type';
import './Table.scss';
import { SavingsPlanStatus } from '@/data/api/api.type';

export default function LinkService({ cell, row }: Readonly<SavingsPlanCellData>) {
  const navigate = useNavigate();
  const label = cell.renderValue();
  const path = row.original.id;
  const isReady = row.original.status === SavingsPlanStatus.ACTIVE;
  return (
    <OsdsText
