...
import {
  ...
  DomainDiagnosisTestMXError,
  DomainDiagnosisTestResult,
  DomainDiagnosisTestSPFError,
  ...
  return (
    <div className="flex flex-col gap-4 md:w-1/2" data-testid={`tab-content-${recordType}`}>
      <OdsText preset={ODS_TEXT_PRESET.paragraph}>
        ...
        <StatusBadge
          className="ml-4"
          status={isDkimDisabled ? DomainDiagnosisTestStatusEnum.DISABLED : diagnostic.status}
        />
      </OdsText>
      {!isOk && (
      ...
      <OdsText preset={ODS_TEXT_PRESET.paragraph}>
        <Trans
          ...
          i18nKey={`zimbra_domain_diagnostic_information_message_${recordType.toLowerCase()}_${
            isDkimDisabled ? 'disabled' : diagnostic?.status.toLowerCase()
          }`}
          values={{
            recordType,
