...
export const GeneralInformations = () => {
  ...
  const itemsStatus = useMemo(() => {
    return [
        ...
        : []),
      {
        ...
        value: platformUrn && (
          <Text className="w-full flex" urn={platformUrn} iamActions={[IAM_ACTIONS.task.get]}>
            <OngoingTasks />
          </Text>
        ),
      },
  ...
  return (
    <GridLayout>
      <Tile.Root title={t(`${NAMESPACES.STATUS}:status`)}>
        {itemsStatus.map((item, index) => (
          <Tile.Item.Root key={index}>
            <Tile.Item.Term label={item.label}></Tile.Item.Term>
            <Tile.Item.Description>{item.value}</Tile.Item.Description>
          </Tile.Item.Root>
        ))}
      </Tile.Root>
      <Tile.Root title={t('zimbra_dashboard_tile_serviceConsumption_title')}>
      ...
      <Tile.Root title={t('common:useful_links')}>
