...
const diagnosticHasError = (diagnostic: DnsDiagnostic) => {
  return (
    diagnostic &&
...
const TabContent = ({
  ...
  const { t } = useTranslation('domains/diagnostic');
  const isOvh = false;
  const hasError = diagnosticHasError(diagnostic);
...
export default function DomainDiagnostics() {
  ...
  const { data: domain, isFetching, isError, refetch } = useDomainDiagnostic({
    domainId,
  });
  ...
  const tabsList: TabItemProps[] = [
    {
      ...
      title: (
        <TabTitle
          ...
          hasError={diagnosticHasError(domain?.diagnostic.mx)}
        />
      ),
    ...
    },
    {
      ...
      component: (
        <TabContent diagnostic={null} recordType={DnsRecordType.SRV} />
      ),
    },
    {
      ...
      title: (
        <TabTitle
          ...
          hasError={diagnosticHasError(domain?.diagnostic.spf)}
        />
      ),
    ...
    },
    {
      ...
      title: (
        <TabTitle
          ...
          hasError={diagnosticHasError(domain?.diagnostic.dkim)}
        />
      ),
