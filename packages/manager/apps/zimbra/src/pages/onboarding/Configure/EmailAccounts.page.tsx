...
import InlineEmailAccountFormItem from '@/components/InlineEmailAccountFormItem/InlineEmailAccountFormItem.component';
...
export const ConfigureEmailAccounts: React.FC = () => {
  ...
  const { data: emailAccounts, isLoading: isLoadingAccounts } = useAccounts({
    gcTime: 0,
  });
  ...
  const domain = useMemo(() => domains?.[0]?.currentState?.name || '', [
    domains,
  ]);
          ...
          <OdsButton
            ...
            onClick={() => {
              if (accounts.length === 0) {
                ...
                append({ domain, offer: ZimbraOffer.STARTER });
              }
            }}
