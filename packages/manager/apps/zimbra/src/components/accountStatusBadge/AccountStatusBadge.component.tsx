...
import { ResourceStatus, CurrentAccountStatus } from '@/data/api';
...
const CurrentAccountStatusLabels: Record<
  keyof typeof CurrentAccountStatus,
  string
> = {
...
const CurrentAccountTooltipContent: Record<
  keyof typeof CurrentAccountStatus,
  string
> = {
...
export const AccountStatusBadge: React.FC<EmailAccountItem> = (props) => {
  ...
  if (
    props.status === ResourceStatus.READY &&
    props.detailedStatus.length > 0
  ) {
  ...
  return (
    <BadgeStatus
      data-testid={`${props.id}_${props.status}`}
      status={props.status}
    ></BadgeStatus>
  );
};
