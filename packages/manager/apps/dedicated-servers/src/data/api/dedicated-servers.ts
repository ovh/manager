...
): Promise<any> => apiClient.v6.get('/dedicated/server', { data: params });
...
export const getdedicatedServerServiceQueryKey = (
  params: GetdedicatedServerServiceParams,
) => [`get/dedicated/server/${params.serviceName}`];

/**
 * Server informations : Get this object properties
 */
export const getdedicatedServerService = async (
  params: GetdedicatedServerServiceParams,
): Promise<any> => apiClient.v6.get(`/dedicated/server/${params.serviceName}`);

/**
 *  Get listing with iceberg V6
 */
export const getListingIcebergV6 = async <T>({
  pageSize,
  page,
}: {
  pageSize: number;
