...
const catalog: TNetworkCatalog = {
  ipModels: [
    {
      type: 'PublicIP',
      pricings: [pricing(['GRA11', 'SBG5'], 340000), pricing(['WAW1'], 410000)],
    },
...
const differentlyCasedCatalog: TNetworkCatalog = {
  ipModels: [
    {
      type: 'publicip' as TPublicIpType,
      pricings: [pricing(['gra11'], 340000)],
    },
  ],
};
  ...
  ({
    ...
    describe('when reading the public ip hourly price', () => {
      it('returns the price the ip model carries in that region', () => {
        expect(getPublicIpHourlyPrice(catalog, type, region)).toEqual(
          expectedPrice,
        );
      });
    });
...
describe('given a catalog casing its ip model and regions differently', () => {
  describe('when reading the public ip hourly price', () => {
    it('still finds the price', () => {
      expect(
        getPublicIpHourlyPrice(differentlyCasedCatalog, 'PublicIP', 'GRA11'),
      ).toEqual(340000);
    });
  });
});
