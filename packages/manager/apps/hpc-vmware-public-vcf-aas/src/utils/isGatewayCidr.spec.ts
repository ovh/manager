...
import { getAddressFromCIDR, getNetworkAddressFromCIDR, isGatewayCidr } from './isGatewayCidr';
...
it.each([
  ...
  (network, exitType, expected) => {
    expect(getNetworkAddressFromCIDR(network).pipe(Effect.runSyncExit)).toStrictEqual(
...
] as const)('getNetworkAddressFromCIDREffect with %s should return (%s)', (network, expected) => {
...
] as const)('isGatewayCidr with %s should return Effect.%s(%s)', (network, exitType, expected) => {
  expect(isGatewayCidr(network).pipe(Effect.runSyncExit)).toStrictEqual(Exit[exitType](expected));
});
