...
import { OdsButton, OdsIcon, OdsMessage, OdsProgressBar, OdsText } from '@ovhcloud/ods-components/react';
...
export const Step2Polling = ({ serverData, onBackToStep1 }: Step2PollingProps) => {
  ...
  const tenantQuery = useTunnelTenantPolling(phase === 'polling-tenant');
  ...
  if (isLocked) {
    return (
      <div className="opacity-60 pointer-events-none flex flex-col gap-3" aria-disabled="true">
      ...
      <div className="flex flex-col gap-2">
        ...
        {showProgress && (
          <div className="flex flex-col gap-2">
            ...
            {phase === 'ready' ? (
              <OdsProgressBar data-testid="progress-bar-full" value={100} max={100} style={{ width: '100%', display: 'block' }} />
            ) : (
              <div
                className="relative w-full h-2 rounded-[8px] overflow-hidden bg-[var(--ods-color-neutral-100)]"
                role="progressbar"
                aria-label={t('tunnel:step2_progress_label')}
                aria-busy
              >
                <div data-testid="progress-bar-indeterminate" className="absolute top-0 left-0 h-full w-[40%] rounded-[8px] bg-[var(--ods-color-primary-500)] animate-ba-step2-slide motion-reduce:animate-none motion-reduce:w-full motion-reduce:opacity-40" />
              </div>
            )}
            {phase !== 'ready' && (
        ...
        {phase === 'ready' && (
          <div className="flex items-center gap-3 mt-4">
            <OdsIcon name={ODS_ICON_NAME.check} className="text-[var(--ods-color-success-500)] text-[20px] shrink-0" />
