...
const { triggerDownloadMock, refetchMock, agentHookState } = vi.hoisted(() => ({
  ...
  refetchMock: vi.fn(),
  agentHookState: {
    data: undefined as { linuxUrl: string; windowsUrl: string } | undefined,
    isPending: false,
    isError: false,
  },
}));
...
describe('AgentInstallationPanel', () => {
  ...
  // TC-TNL-77 / TC-TNL-83 — selecting Linux populates curl + wget terminal commands
  it('renders Linux curl + wget commands after selecting Linux', async () => {
    ...
    expect(screen.getByText(`curl -O "${AGENT_LINKS.linuxUrl}"`)).toBeInTheDocument();
    expect(screen.getByText(`wget "${AGENT_LINKS.linuxUrl}"`)).toBeInTheDocument();
  });
  ...
  it('disables download and shows the unavailable message when the URL is empty', async () => {
    agentHookState.data = { linuxUrl: '', windowsUrl: AGENT_LINKS.windowsUrl };
    const { container } = await renderPanel();
    await selectOs(container, 'LINUX');
