...
} from '@/utils/defaultApplicationServers';
...
// TODO: remove these 3 lines (testing purposes) and initialize with formContext
const testServiceName = 'pcc-145-239-132-226';
const testVdcId = 869;
const testClusterId = 1006;
...
export default function InstallationStepServerConfig() {
  ...
  const {
    ...
    datacenterId: testVdcId.toString(),
  ...
  const hasReachMaximumVM = useMemo(
    () => applicationServers.length >= SERVER_CONFIG_LIMITS.vmMax,
    [applicationServers.length],
  );
          ...
          >
            <div className="flex flex-col gap-y-4 max-w-5xl">
              ...
              <RhfField
                ...
                helperMessage={
                  formState.errors?.applicationServers?.[index]?.instanceNumber
                    ?.message === 'duplicateInstance'
                    ? t('server_config_error_instance_number_duplicate')
                    : t('server_config_helper_instance_number')
                }
              >
                <RhfField.Label>
                  {t('server_config_input_instance_number')}
