  ...
  .controller(
      ...
      $scope.getAvailableOffers = function getAvailableOffers() {
        ...
        $q.all([
          ...
          .then((data) => {
            ...
            $scope.mxplanEnum = filter(
              models['hosting.web.order.MxPlanEnum'].enum,
              (mxEnum) => mxEnum !== 'delete',
            );
          })
          .catch(() => {
            $scope.availableOffers = [];
    ...
    ============================== */
      $scope.validateEmailDecision = function validateEmailDecision() {
        ...
        const keepsEmailsWithNewPlan =
          $scope.emails.keep && !isEmpty($scope.model.mxplan);
    ...
    ============================== */
      ...
      $scope.loadContracts = function loadContracts() {
        ...
        const mustDeleteEmails =
          !$scope.emails.keep || isEmpty($scope.emails.data);
        if (mustDeleteEmails) {
          mxplan = 'delete';
        }
        ...
        hostingChangeDomain
          ...
          .then((durations) => {
            const priceAndContractPromises = map(durations, (duration) =>
              hostingChangeDomain
                ...
                .catch((err) =>
                  Alerter.alertFromSWS(
                    $translate.instant('hosting_order_upgrade_error'),
                    err,
                    $scope.alerts.main,
                  ),
                )
      ...
      $scope.orderUpgrade = function orderUpgrade() {
        ...
        hostingChangeDomain
          ...
          .then((order) => {
            ...
            Alerter.success(
              $translate.instant('hosting_order_upgrade_success', {
                t0: order.url,
                t1: order.orderId,
              }),
              $scope.alerts.main,
