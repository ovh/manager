...
angular.module('App').controller(
  ...
  class SqlDatabaseOrderCtrl {
    ...
    load() {
      ...
      return this.PrivateDatabase.getOrderModels()
        .then((models) => {
          const dbaasName = get(models, 'hosting.PrivateDatabase.OfferEnum')
            .enum[1];
          const sqlName = get(models, 'hosting.PrivateDatabase.OfferEnum')
            .enum[0];
          ...
          return this.$q
            .all({
              dbaasOrderCapacities: this.PrivateDatabase.getAvailableOrderCapacities(
                dbaasName,
              ),
              privateOrderCapacities: this.PrivateDatabase.getAvailableOrderCapacities(
                sqlName,
              ),
              domainNames: this.Hosting.getHostings(),
              dbPack: this.HostingOptionOrder.getOrderEnums(
              ...
              this.data = [
                {
                  ...
                  tooltips: {
                    rams: {
                      min: this.convertBytesSize(
                        min(result.dbaasOrderCapacities.ram),
                      ),
                      max: this.convertBytesSize(
                        max(result.dbaasOrderCapacities.ram),
                      ),
                    },
                  },
                },
                {
                  ...
                  tooltips: {
                    rams: {
                      min: this.convertBytesSize(
                        min(result.privateOrderCapacities.ram),
                      ),
                      max: this.convertBytesSize(
                        max(result.privateOrderCapacities.ram),
                      ),
                    },
                  },
                },
        ...
        .then(({ domainNames }) =>
          this.$q
            .all(
              map(domainNames, (domainName) =>
                this.Hosting.getHosting(domainName),
              ),
            )
            ...
            .then((hostings) => {
              ...
              find(this.data, { key: 'start' }).hostings = map(
                hostings,
                (hosting) => ({
              ...
              find(this.data, { key: 'premium' }).hostings = angular.copy(
                find(this.data, { key: 'start' }).hostings,
              );
    ...
    getDurationspremium(data) {
      ...
      return this.PrivateDatabase.orderDuration(version, ram)
        .then((durations) => {
          // eslint-disable-next-line no-param-reassign
          data.durations = map(durations, (duration) => ({
            duration,
            details: {},
          }));
    ...
    getDurationsstart(data) {
      ...
      return this.HostingOptionOrder.getSqlPersoAllowedDurations(
        hosting,
        startDbVersion,
      )
        .then((durations) => {
      ...
      return this.$q
        .all(
          map(durations, (duration) =>
            this.PrivateDatabase.orderPrice(version, ram, duration).then(
              (details) => {
    ...
    getPricesstart(data, durations) {
      ...
      return this.$q
        .all(
          map(durations, (duration) =>
            this.HostingOptionOrder.getSqlPersoPrice(
              hosting,
              startDbVersion,
              duration,
            ).then((details) => {
    ...
    generateBcpremium() {
      ...
      return this.PrivateDatabase.orderPrivateDatabase(
        this.model.version,
        this.model.ram,
        this.model.duration,
    ...
    generateBcdbaas() {
      ...
      return this.PrivateDatabase.orderDBaaS(
        this.model.version,
        this.model.ram,
        this.model.duration,
        this.model.datacenter,
    ...
    generateBcstart() {
      ...
      return this.HostingOptionOrder.orderSqlPerso(
        this.model.hosting,
        this.model.dbPack,
        this.model.duration,
      )
    ...
    setDatacenter() {
      if (
        isEmpty(this.selectedHosting) ||
        this.selectedHosting === this.noHostValue
      ) {
        return;
      }
    ...
    openBc() {
      this.atInternet.trackOrder({
        name: `[sql-${this.model.type}]::${this.model.version ||
          this.model.dbPack}[${this.model.version || this.model.dbPack}]`,
