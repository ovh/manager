  ...
  class MXPlanOrderCtrl {
    ...
    initialLoad() {
      ...
      return this.Domain.getDomains()
        ...
        .catch(() =>
          this.Alerter.error(
            this.$translate.instant('mxPlan_order_step1_error'),
    ...
    fetchOffers() {
      ...
      return this.MXPlan.getOrderModels(this.selectedOrder.config.domain)
        .then((response) => {
          this.list.offers = response;
        })
        .catch((err) => {
    ...
    getDurations() {
      ...
      return this.MXPlan.orderDuration(
        this.selectedOrder.config.domain,
        this.selectedOrder.config.offer,
      )
        .then((durations) => {
        ...
        .catch((err) =>
          this.Alerter.alertFromSWS(
            this.$translate.instant('mxPlan_order_step2_duration_fail'),
            err,
    ...
    loadPrices(durations) {
      ...
      const queue = map(durations, (duration) =>
        this.MXPlan.orderPrice(
          this.selectedOrder.config.domain,
          this.selectedOrder.config.offer,
          duration,
        ).then((details) => {
      ...
      return this.$q
        ...
        .catch((err) =>
          this.Alerter.alertFromSWS(
            this.$translate.instant('mxPlan_order_step2_price_fail'),
            err,
            this.$scope.alerts.order,
          ),
        )
    ...
    generateBc() {
      ...
      this.MXPlan.orderMxPlan(
        ...
        .catch((err) =>
          this.Alerter.alertFromSWS(
            this.$translate.instant('mxPlan_order_step3_fail'),
            err,
            this.$scope.alerts.order,
          ),
        )
