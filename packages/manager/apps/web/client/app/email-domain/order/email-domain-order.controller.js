...
export default class MXPlanOrderCtrl {
  ...
  getProductTaxAmount({ planCode }) {
    ...
    const price = find(pricings, ({ capacities }) => includes(capacities, 'installation'));
    return price.tax;
  }
  ...
  onDomainChange() {
    ...
    return this.deleteItem(this.item)
      .catch((error) => this.Alerter.error(
        this.$translate.instant('mxPlan_order_checkout_error', { message: get(error, 'data.message') }),
      ))
      .finally(() => {
  ...
  loadValidation() {
    ...
    return this.addConfiguration(this.item, this.model.product, this.model.selectedDomain)
      .then(({ item }) => {
        this.item = item;
  ...
  validateCheckout() {
    ...
    return this.order(this.model.autoPayWithPreferredPaymentMethod)
      .then((order) => {
        ...
        this.orderTrackingLink = `${ORDER_TRACKING_URLS[this.coreConfig.getRegion()]}/${order.orderId}`;

        return this.orderTrackingLink;
      })
      .catch((error) => this.Alerter.error(
        this.$translate.instant('mxPlan_order_error', { message: get(error, 'message') }),
      ))
      .finally(() => {
        this.$timeout(() => {
          this.loading.checkout = false;
