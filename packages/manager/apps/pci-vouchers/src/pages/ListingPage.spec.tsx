...
import { ResponseAPIError, useProject } from '@/api/hooks/useProject';
...
describe('ListingPage', () => {
  ...
  it('should render loading spinner when vouchers are loading', async () => {
    vi.mocked(useProject).mockReturnValue({
      data: { id: 'projectid123' },
    } as UseQueryResult<Project, ResponseAPIError>);
  ...
  it('should render vouchers when vouchers are loaded', async () => {
    vi.mocked(useProject).mockReturnValue({
      data: { id: 'projectid123' },
    } as UseQueryResult<Project, null>);
    vi.mocked(useProjectUrl).mockReturnValue(
      'public-cloud/project/projectid123',
    );
    vi.spyOn(useVouchersModule, 'useVouchers').mockReturnValue({
  ...
  // render discover project when project is discovery project
  it('should render discover project when project is discovery project', async () => {
    vi.mocked(useProject).mockReturnValue({
      data: { id: 'projectid123' },
    } as UseQueryResult<Project, ResponseAPIError>);
    vi.mocked(useProjectUrl).mockReturnValue(
