        ...
        <ul className="flex space-x-1 items-center">
          {breadcrumbs.map((breadcrumb, index) => (
            <li key={index} className='flex space-x-1'>
              ...
              <span className="flex items-center gap-2">{index === 0 && <Home className="w-4 h-4" /> } {t(breadcrumb.name)}</span> :
              <Link to={breadcrumb.path} className="text-blue-500 hover:underline flex items-center gap-2">
                {index === 0 && <Home className="w-4 h-4" /> } {t(breadcrumb.name)}
              </Link>
              }
            </li>
          ))}
