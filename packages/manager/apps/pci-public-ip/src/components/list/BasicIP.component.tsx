...
import { useBasicIps } from '@/api/hooks/useBasicIp';
...
export default function BasicIPComponent({
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (props: TBasicIpRow) => (
        <DataGridTextCell>
          {props.associatedResourceId ? (
            <OsdsLink
              color={ODS_THEME_COLOR_INTENT.primary}
              href={goToInstanceHref(props.associatedResourceId)}
            >
              {props.associatedResourceId}
            </OsdsLink>
          ) : (
    ...
    },
    {
      ...
      cell: (props: TBasicIpRow) => (
        <div className="min-w-16">
          <BasicIPActions projectId={projectId} ipId={props.id} />
        </div>
      ),
      label: '',
    },
