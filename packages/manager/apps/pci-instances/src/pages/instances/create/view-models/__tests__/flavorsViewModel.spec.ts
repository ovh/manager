...
import { selectCpuFlavors, selectGpuFlavors } from '../flavorsViewModel';
...
const fakeDeps: Deps = {
  ...
  configurationPort: { selectSshKeys: vi.fn() },
...
const createSelectArgs = (overrides = {}) => ({
  projectId,
  flavorType,
  microRegionId,
...
describe('selectCpuFlavors', () => {
  it('should returns empty result when required args are missing', () => {
    const result = selectCpuFlavors(fakeDeps)(
      createSelectArgs({ flavorType: null }),
    );
    expect(result).toEqual({ flavors: [], preselecteFlavordId: null });
  });

  it('should returns available cpu', () => {
    const result = selectCpuFlavors(fakeDeps)(createSelectArgs());
    ...
    expect(result.preselecteFlavordId).toBe('d2-2_micro-1');
  });

  it('should add unavailable cpu flavor when enabled', () => {
    const result = selectCpuFlavors(fakeDeps)(
      createSelectArgs({ withUnavailable: true }),
    );
  ...
  it('should return null prices when no pricing data exists', () => {
    const result = selectCpuFlavors(createDepsWithoutPrices())(
      createSelectArgs(),
    );
    const cpuFlavorResult = result.flavors.find((f) => f.name === 'd2-2');
...
describe('selectGpuFlavors', () => {
  it('should returns gpu flavors with gpu fields', () => {
    const result = selectGpuFlavors(fakeDeps)({
      projectId,
      flavorType,
      microRegionId,
    ...
    const gpuFlavorResult = result.gpuFlavors.find((f) => f.name === 'l40s-90');
