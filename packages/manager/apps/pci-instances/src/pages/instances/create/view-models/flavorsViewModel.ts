...
const enrichGpuFlavor: TFlavorEnricher<TGpuFlavorDataForTable> = (
  ...
  realMinimumHourlyPrice: base.realMinimumHourlyPrice,
  realMinimumMonthlyPrice: base.realMinimumMonthlyPrice,
  estimatedMinimumMonthlyPrice: base.estimatedMinimumMonthlyPrice,
});
...
const selectFlavorsInternal = <T extends TBaseFlavorDataForTable>(
  ...
  enrichFlavor: TFlavorEnricher<T>,
): { flavors: T[]; preselecteFlavordId: string | null } => {
  const { flavorType, microRegionId, withUnavailable } = args;

  if (!flavorType || !microRegionId) {
    return { flavors: [], preselecteFlavordId: null };
  }
...
export const selectCpuFlavors: Reader<Deps, TSelectCpuFlavors> = (deps) => {
  return (args) => {
    ...
    return selectFlavorsInternal(args, data, enrichCpuFlavor);
  };
};
...
export const selectGpuFlavors: Reader<Deps, TSelectGpuFlavors> = (deps) => {
  return (args) => {
    ...
    const data = instancesCatalogPort.selectInstancesCatalog(args.projectId);
    if (!data) {
      return { gpuFlavors: [], preselecteFlavordId: null };
    }

    const result = selectFlavorsInternal(args, data, enrichGpuFlavor);
    return {
      gpuFlavors: result.flavors,
      preselecteFlavordId: result.preselecteFlavordId,
    };
  };
};
