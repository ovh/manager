...
export const mapDisksToViewModel = (
  ...
  return (disks as TDiskLike[]).map((disk, index) => {
    ...
    const isTb = unit === 'GB' && value >= GB_TO_TB_THRESHOLD;
    ...
    const display = `${disk.number}x ${capacityValue}${interfaceStr ? ` ${interfaceStr}` : ''}`;
    const id = `${disk.number}_${value}_${unit}_${disk.interface ?? 'no-interface'}_${index}`;
    return {
      id,
      display,
      number: disk.number,
