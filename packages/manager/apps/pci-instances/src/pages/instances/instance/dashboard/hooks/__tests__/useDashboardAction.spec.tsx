...
describe('useUnattachedVolumes', () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expected: [
        { label: 'fake-volume-3', value: 'fake-volume-3', canBeDetach: false },
        { label: 'fake-volume-4', value: 'fake-volume-4', canBeDetach: false },
      ],
    },
    {
      description:
      ...
      expected: [
        { label: 'fake-volume-2', value: 'fake-volume-2', canBeDetach: false },
      ],
    },
  ...
  it('should return unattached volumes with canBeDetach = true in 1AZ when status = in-use', async () => {
    ...
    await waitFor(() => {
      expect(result.current.volumes).toStrictEqual([
        { label: 'fake-volume', value: 'fake-volume-2', canBeDetach: true },
      ]);
    });
  });
  ...
  describe.each([
    ...
    ({ attachedInstancesCount, expectedCanBeDetach }) => {
      it(`should return unattached volumes with canBeDetach flag = ${expectedCanBeDetach}`, async () => {
          ...
          expect(result.current.volumes).toStrictEqual([
            {
              label: 'fake-volume',
              value: 'fake-volume',
              canBeDetach: expectedCanBeDetach,
            },
          ]);
        });
      });
