...
describe('useUnattachedVolumes', () => {
  ...
  it.each([
    ...
    {
      description:
        'should return only unattached volumes with availabilityZone = null',
      volumes: [
        {
          id: 'fake-volume-1',
          name: 'fake-volume',
      ...
      expected: [
        { label: 'fake-volume-3', value: 'fake-volume-3', toBeDetach: false },
        { label: 'fake-volume-4', value: 'fake-volume-4', toBeDetach: false },
      ],
    },
    {
      description:
      ...
      expected: [
        { label: 'fake-volume-2', value: 'fake-volume-2', toBeDetach: false },
      ],
    },
  ...
  it('should return unattached volumes with toBeDetach = true in 1AZ when status = in-use', async () => {
    ...
    await waitFor(() => {
      expect(result.current.volumes).toStrictEqual([
        { label: 'fake-volume', value: 'fake-volume-2', toBeDetach: true },
      ]);
    });
  });
  ...
  describe.each([
    {
      ...
      expectedToBeDetach: false,
    },
    {
      description: 'volumes are attached with more or equal to 16 instances',
      attachedInstancesCount: 16,
      expectedToBeDetach: true,
    },
  ])(
    'Given region is 3AZ $description',
    ({ attachedInstancesCount, expectedToBeDetach }) => {
      it(`should return unattached volumes with toBeDetach flag = ${expectedToBeDetach}`, async () => {
