...
export default function MyWebsitesPage() {
  ...
  const deleteLabel = t(`${NAMESPACES.ACTIONS}:delete`);

  const getActionItems = useMemo(() => {
    return (rowId: string, rowOriginal: ManagedWordpressWebsites) => [
      {
        id: Number(`delete-${rowId}`),
        label: deleteLabel,
        onClick: () => handleDeleteItemClick(rowId),
        color: BUTTON_COLOR.critical,
        isDisabled: !isRowSelectable(rowOriginal),
      },
    ];
  }, [deleteLabel, handleDeleteItemClick, isRowSelectable]);
      ...
      },
      {
        ...
        cell: ({ row }) => {
          return (
            <ActionMenu
              key={row.id}
              items={getActionItems(row.id, row.original)}
              isCompact
