...
const formSchema = z.object({
  ...
  subDomain: z.string().min(1, { message: 'Required' }),
  wwwNeeded: z.boolean(),
  folder: z.string().min(1, { message: 'Required' }),
    ...
    (async () => {
      if (step === 1) {
        const res = await aapi.get<TExistingDomain>(
          `/sws/hosting/web/${serviceName}/add-domain-existing?tokenNeeded=${formValues?.path ===
            'B'}`,
        );
        setExistingDomains(res?.data);
      }
    })();
  }, [step, serviceName, formValues?.path]);
        ...
        return (
          <div className="flex flex-col space-y-8 mb-10 mt-5">
            ...
            <div className="flex">
              ...
              <OdsText className="w-1/2">
                {t('hosting_add_step3_folder', { name: formValues.folder })}
              </OdsText>
            </div>
            {formValues.path === 'A' && (
              <>
                ...
                <div className="flex">
                  ...
                  <OdsText className="w-1/2">
                    {t(
                      `hosting_add_step3_${
                        formValues.firewall ? 'enabled' : 'disabled'
                      }`,
                    )}
                  </OdsText>
                </div>
                <div className="flex">
                  ...
                  <OdsText className="w-1/2">
                    {t(
                      `hosting_add_step3_${
                        formValues.logs ? 'enabled' : 'disabled'
                      }`,
                    )}
                  </OdsText>
                </div>
                  ...
                  {[
                    ...
                    ...(formValues.wwwNeeded
                      ? [
                          {
                            record: 'A',
                            label: t('hosting_add_step3_www_domain', {
                              domain: formValues.subDomain,
                            }),
                            value: hostingService?.data?.hostingIp,
                          },
