...
const ActionButtonMultisite: React.FC<ActionButtonMultisiteProps> = ({
  ...
  const allActions: ActionMenuItem[] = useMemo(() => {
    if (context === 'site') {
      ...
      const existingWebsites = websites.length;
      const allowedWebsites = website?.currentState?.linkedDomains ?? 0;
      const hasModule = !!website?.currentState?.module?.name;

      const canAssociateGit =
        (existingWebsites < allowedWebsites ||
          allowedWebsites === -1 ||
          vcsStatus === GitStatus.INITIALERROR) &&
        ![GitStatus.CREATED, GitStatus.ERROR, GitStatus.DEPLOYING].includes(
          vcsStatus ?? GitStatus.DISABLED,
        ) &&
        vcsStatus !== GitStatus.CREATING;
