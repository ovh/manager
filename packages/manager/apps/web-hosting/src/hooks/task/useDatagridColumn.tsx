...
import { TaskDetailsType } from '@/data/type';
  ...
  const columns = [
    {
      ...
      cell: (props: TaskDetailsType) => (
        <DataGridTextCell>
          {t(
            `hosting_tab_TASKS_function_${props?.function?.replace('/', '_')}`,
          )}
        </DataGridTextCell>
      ),
    ...
    },
    {
      ...
      cell: (props: TaskDetailsType) => (
        <DataGridTextCell>
          <Badge
            label={t(`hosting_tab_TASKS_status_${props?.status}`)}
            className="my-3"
    ...
    },
    {
      ...
      cell: (props: TaskDetailsType) =>
        props.doneDate && (
          <DataGridTextCell>
            {formatDate({ date: props?.doneDate, format: 'Pp' })}
          </DataGridTextCell>
        ),
