...
describe('useDatagridColumn', () => {
  ...
  it.skip('should render correct cells', () => {
    const { result } = renderHook(() => useDatagridColumn());
    ...
    const taskCell = columns[0].cell({
      row: { original: tasksMocks[0] },
    }) as ElementWithStringChildren;
    const startDateCell = columns[2].cell({
      row: { original: tasksMocks[0] },
    }) as ElementWithStringChildren;
    const finishDateCell = columns[3].cell({
      row: { original: tasksMocks[0] },
    }) as ElementWithStringChildren;
