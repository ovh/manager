...
export const isEditable = (vs?: VrackServicesWithIAM) =>
  ...
  [VrackServicesProductStatus.ACTIVE, VrackServicesProductStatus.DRAFT].includes(
    vs?.currentState.productStatus,
  );

export const hasSubnet = (vs?: VrackServicesWithIAM) => (vs?.currentState.subnets ?? []).length > 0;

export const getSubnetFromCidr = (vs?: VrackServicesWithIAM, cidr?: string) =>
  vs?.currentState?.subnets.find((s) => s.cidr === cidr);

export const getDisplayName = (vs?: VrackServicesWithIAM) => vs?.iam?.displayName || vs?.id;
