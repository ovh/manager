...
describe('RegionSelector component', () => {
  it('renders correctly', async () => {
    ...
    expect(container.querySelectorAll('div[data-ods=card]')).toHaveLength(regionList.length);
  ...
  it.each([
    ...
    expect(container.querySelectorAll('div[data-ods=card]')).toHaveLength(cardNb);
  ...
  ])('hide filters if there is no corresponding regions', async ({ list, missingFilters }) => {
    const { queryByText } = await act(() =>
      render(<RegionSelector regionList={list} setSelectedRegion={vi.fn()} />),
    );
