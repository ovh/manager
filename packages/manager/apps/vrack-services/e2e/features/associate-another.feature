Feature: Associate to another from overview page
  ...
  Scenario Outline: User associate a vRack Services to another vRack
    ...
    Examples:
      | dissociateOkOrKo  | associateOkOrKo | taskOkOrKo  | returnOverview  | anyErrorMessage |
      | ok                | OK              | ok          | returns         | no              |
      | ko                | OK              | ok          | doesn't returns | an              |
      | ok                | OK              | ko          | doesn't returns | an              |
      | ok                | KO              | ok          | doesn't returns | an              |
