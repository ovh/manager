...
import contactImportTemplate from './contact-import/contact-import.html';
...
export default /* @ngInject */ function TelecomTelephonyLinePhonePhonebookCtrl(
  $document,
  $filter,
  $q,
  $scope,
      ...
      .catch((error) => {
        if (error && error.status === 501) {
          return TucToast.error(
            $translate.instant('telephony_phonebook_brand_not_supported'),
          );
        }
  ...
  self.importPhonebookContact = function importPhonebookContact() {
    ...
    const modal = $uibModal.open({
      animation: true,
      template: contactImportTemplate,
