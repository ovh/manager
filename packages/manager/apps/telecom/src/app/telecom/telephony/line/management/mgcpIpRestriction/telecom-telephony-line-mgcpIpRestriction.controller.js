...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyLineMgcpIpRestrictionCtrl',
    function TelecomTelephonyLineMgcpIpRestrictionCtrl(
      $q,
      $stateParams,
      $translate,
      TucIpAddress,
      ...
      self.setMgcpIpRestriction = function setMgcpIpRestriction() {
        ...
        return OvhApiTelephony.Line()
          ...
          .$promise.then(() => {
            TucToast.success(
              $translate.instant(
                'telephony_line_mgcp_ip_restriction_edit_success',
              ),
            );
          ...
          .catch((error) => {
            TucToast.error(
              [
                $translate.instant(
                  'telephony_line_mgcp_ip_restriction_edit_error',
                ),
    ...
    ====================================== */
      ...
      self.onBulkError = function onBulkError(error) {
        TucToast.error(
          [
            $translate.instant(
              'telephony_line_mgcp_ip_restriction_bulk_on_error',
            ),
            get(error, 'msg.data'),
          ].join(' '),
        );
