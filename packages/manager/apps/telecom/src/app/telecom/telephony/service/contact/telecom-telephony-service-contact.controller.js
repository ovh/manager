...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyServiceContactCtrl',
    function TelecomTelephonyServiceContactCtrl(
      $state,
      $stateParams,
      $q,
      $timeout,
      ...
      OvhApiXdsl,
      tucTelephonyBulk,
      TELEPHONY_SERVICE_CONTACT_DIRECTORY_INFO,
    ) {
      const self = this;
      ...
      */
      ...
      self.onSiretChange = (function onSiretChange() {
        ...
        return function onSiretChangeFunction(model) {
          if (
            self.directoryForm.siret &&
            self.directoryForm.siret.length >= 9
          ) {
            if (pendingRequest) {
              pendingRequest.$cancelRequest();
            }
      ...
      self.isUrbanDistrictRequired = function isUrbanDistrictRequired() {
        ...
        return (
          startsWith(p, '75') || startsWith(p, '13') || startsWith(p, '69')
        );
      };
      ...
      self.getStatusLabel = function getStatusLabel() {
        if (
          self.directoryForm.status.includes(
            TELEPHONY_SERVICE_CONTACT_DIRECTORY_INFO.status.error,
          )
        ) {
        ...
        if (
          self.directoryForm.status.includes(
            TELEPHONY_SERVICE_CONTACT_DIRECTORY_INFO.status.todo,
          )
        ) {
          if (
            ...
            (!self.directoryForm.displayUniversalDirectory &&
              self.directoryForm.modificationType.includes(
                TELEPHONY_SERVICE_CONTACT_DIRECTORY_INFO.type.delete,
              ))
          ) {
