...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyAliasOrderNonGeographicalCtrl',
    function TelecomTelephonyAliasOrderNonGeographicalCtrl(
      $q,
      $translate,
      $stateParams,
      OvhApiTelephony,
        ...
        OvhApiOrder.Telephony()
          ...
          .$promise.then(
            ...
            (err) => {
              TucToastError(
                $translate.instant(
                  'telephony_order_nongeographical_prices_error',
                ),
              );
              return $q.reject(err);
            },
        ...
        OvhApiOrder.Telephony()
          ...
          .$promise.then(
            (response) => {
              ...
              TucToast.success(
                $translate.instant(
                  'telephony_order_nongeographical_order_success',
                ),
              );
              self.orderDone = true;
              return response;
            ...
            (err) => {
              ...
              if (err && err.data && err.data.message) {
                switch (err.data.message) {
                  case /^Invalid city parameter \(([^)]*)\)/.test(
                    err.data.message,
                  )
                    ? err.data.message
                    : false:
                    TucToast.error(
                      $translate.instant(
                        'telephony_order_order_error_city',
                        form,
                      ),
                    );
                    break;
                  case /^The following specified number is not longer available/.test(
                    ...
                    TucToast.error(
                      $translate.instant(
                        'telephony_order_order_error_available',
                      ),
                    );
