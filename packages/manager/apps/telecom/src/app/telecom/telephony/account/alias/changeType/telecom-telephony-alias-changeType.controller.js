...
angular.module('managerApp').controller(
  'TelecomTelephonyAliasChangeTypeCtrl',
  class TelecomTelephonyAliasChangeTypeCtrl {
    constructor(
      $q,
      $state,
      $stateParams,
    ...
    confirmChoice() {
      if (!isEqual(this.number.featureType, 'empty')) {
        const validChoiceModal = this.$uibModal.open({
          templateUrl:
            'app/telecom/telephony/account/alias/changeType/confirm/telecom-telephony-alias-changeType-confirm.html',
      ...
      this.tucVoipServiceAlias
        ...
        .catch((error) => {
          if (!isEqual(error.type, 'poller')) {
            this.TucToast.error(
              `${this.$translate.instant(
                'telephony_alias_change_type_ko',
    ...
    checkServerTasksStatus() {
      ...
      return (updatedServices) => {
        function runPollOnTask(billingAccount, serviceName, taskId) {
          return () =>
            tucVoipServiceTask.startPolling(
              billingAccount,
              serviceName,
              taskId,
            );
