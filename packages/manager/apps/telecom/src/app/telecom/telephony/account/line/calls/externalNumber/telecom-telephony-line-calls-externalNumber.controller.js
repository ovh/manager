...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyLineCallsExternalNumberCtrl',
    function TelecomTelephonyLineCallsExternalNumberCtrl(
      $scope,
      $q,
      $stateParams,
      $translate,
        ...
        return OvhApiTelephony.Trunk()
          ...
          .$promise.then((externalNumber) => {
            ...
            return validationPromise.then(() => {
              ...
              $timeout(() => {
                self.externalNumberAddForm.number.$setValidity(
                  'required',
                  true,
                );
              }, 50);
      ...
      self.onConfirmDeleteNumberBtnClick = function onConfirmDeleteNumberBtnClick() {
        ...
        return OvhApiTelephony.Trunk()
          ...
          .$promise.then(() => {
            remove(
              self.list,
              (number) => number.number === self.toDelete.number,
            );
            TucToast.success(
              $translate.instant(
                'telephony_trunk_external_number_delete_success',
              ),
            );
          })
          .catch((error) => {
            TucToast.error(
              [
                $translate.instant(
                  'telephony_trunk_external_number_delete_error',
                ),
                get(error, 'data.message'),
              ].join(' '),
            );
        ...
        return TelephonyMediator.getGroup($stateParams.billingAccount)
          ...
          .catch((error) => {
            TucToast.error(
              [
                $translate.instant(
                  'telephony_trunk_external_number_load_error',
                ),
                get(error, 'data.message'),
              ].join(' '),
            );
