...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyLineCallsFilteringCtrl',
    function TelecomTelephonyLineCallsFilteringCtrl(
      $stateParams,
      $q,
      $timeout,
      $translate,
              ...
              .then((chunkResult) => {
                ...
                return forEach(result, (filter) => {
                  set(
                    filter,
                    'shortType',
                    startsWith(filter.type, 'incoming')
                      ? 'incoming'
                      : 'outgoing',
                  );
                  set(
      ...
      self.getBulkParams = function getBulkParams(action) {
        switch (action) {
          case 'screen':
            return {
              outgoingScreenList: get(
                self,
                'screen.modified.outgoingScreenList',
              ),
              incomingScreenList: get(
                self,
                'screen.modified.incomingScreenList',
              ),
            };
          case 'options':
