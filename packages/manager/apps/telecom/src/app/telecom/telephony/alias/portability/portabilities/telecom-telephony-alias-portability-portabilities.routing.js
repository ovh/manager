export default /* @ngInject */ ($stateProvider) => {
  $stateProvider.state('telecom.telephony.billingAccount.alias.portabilities', {
    ...
    resolve: {
      ...
      deleteDocument: /* @ngInject */ ($state, billingAccount) => (portability, documentId) => $state.go('telecom.telephony.alias.portabilities.delete', {
        billingAccount,
        portabilityId: portability.id,
        documentId,
      }),
      goToPortabilities: /* @ngInject */ ($state, billingAccount, TucToast) => (message = false, type = 'success') => {
        ...
        const promise = $state.go('telecom.telephony.billingAccount.alias.portabilities', {
          billingAccount,
        },
        {
          reload,
