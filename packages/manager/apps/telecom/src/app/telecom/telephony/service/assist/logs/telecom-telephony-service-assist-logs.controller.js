...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyServiceAssistLogsCtrl',
    function TelecomTelephonyServiceAssistLogsCtrl(
      $q,
      $translate,
      $state,
      $stateParams,
        ...
        return tucVoipLineFeature
          ...
          .catch((error) => {
            TucToast.error(
              [
                $translate.instant(
                  'telephony_line_assist_support_logs_save_error',
                ),
                get(error, 'data.message', ''),
              ].join(' '),
            );
  ...
  ====================================== */
        ...
        return tucVoipService
          ...
          .catch((error) => {
            TucToast.error(
              [
                $translate.instant(
                  'telephony_line_assist_support_logs_init_error',
                ),
                get(error, 'data.message', ''),
              ].join(' '),
            );
      ...
      self.onBulkError = function onBulkError(error) {
        TucToast.error(
          [
            $translate.instant(
              'telephony_line_assist_support_logs_bulk_on_error',
            ),
            get(error, 'msg.data'),
          ].join(' '),
        );
