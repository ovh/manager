...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyLineTerminateCancelCtrl',
    function TelecomTelephonyLineTerminateCancelCtrl(
      $stateParams,
      $state,
      TelephonyMediator,
      OvhApiTelephony,
        ...
        if (self.taskDetails.executionDate) {
          return OvhApiTelephony.Service()
            ...
            .$promise.then(() => {
              TucToast.success(
                $translate.instant(
                  'telephony_group_line_cancel_terminating_ok',
                ),
              );
              $state.go('^');
            })
            .catch(() => {
              TucToast.error(
                $translate.instant(
                  'telephony_group_line_cancel_terminating_ko',
                ),
              );
            })
            .finally(() => {
