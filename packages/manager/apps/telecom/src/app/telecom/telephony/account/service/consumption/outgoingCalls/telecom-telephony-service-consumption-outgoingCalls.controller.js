...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyServiceConsumptionOutgoingCallsCtrl',
    function TelecomTelephonyServiceConsumptionOutgoingCallsCtrl(
      $stateParams,
      $q,
      $translate,
      $filter,
      ...
      function fetchOutgoingConsumption() {
        return tucVoipService
          ...
          .then((result) =>
            result
              .filter(
                (conso) => conso.wayType !== 'incoming' && conso.duration > 0,
              )
              .map((conso) => {
