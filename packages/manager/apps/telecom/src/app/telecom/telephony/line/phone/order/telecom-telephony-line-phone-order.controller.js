...
angular
  ...
  .controller(
    ...
    function TelecomTelephonyLinePhoneOrderCtrl(
      ...
      self.rmaStatusUrl = TelephonyMediator.getV6ToV4RedirectionUrl(
      ...
      function fetchMerchandiseAvailable() {
        return OvhApiTelephony.Line()
          ...
          .$promise.then((result) => {
            ...
            return filter(
              result,
              (phone) => phone.price && phone.price.value >= 0,
            );
          })
          .then((result) =>
      ...
      function init() {
        ...
        TelephonyMediator.getGroup($stateParams.billingAccount)
            ...
            OvhApiTelephony.Line()
              ...
              .$promise.then((result) => {
                assign(
                  ...
                  {
                    isAttachedToOtherLinesPhone:
                      result.isAttachedToOtherLinesPhone,
                  },
                );
              }),
          )
          ...
          .then((phone) => {
            ...
            self.rmas = [];
            return fetchOfferPhones(self.line.getPublicOffer.name).then(
              (offers) => {
          ...
          switch (step) {
            case 'hardware':
              if (self.phone) {
                ...
                fetchOfferPhones(self.line.getPublicOffer.name)
                  .then((offers) => {
                    self.phoneOffers = offers;
                  })
                  .catch((err) => new TucToastError(err))
      ...
      self.submitPhoneReturn = function submitPhoneReturn() {
        ...
        return OvhApiTelephony.Line()
          ...
          .$promise.then(() => self.phone.getRMAs())
          .then((rmas) => {
            self.rmas = rmas;
      ...
      self.getPhoneLabel = function getPhoneLabel(phone) {
        const name = $translate.instant(
          'telephony_line_phone_order_order_change_for',
          { phone: phone.displayName },
        );
        const price = $translate.instant(
          'telephony_line_phone_order_order_price_tax_free',
          { price: phone.price.text },
        );
        return [name, price].join(' ');
      };

      self.getOfferLabel = function getOfferLabel(offer) {
        const name = $translate.instant('telephony_line_phone_order_order_a', {
          brand: offer.description,
        });
        const price = $translate.instant(
          'telephony_line_phone_order_order_price2_tax_free',
      ...
      self.isSamePhone = function isSamePhone() {
        return (
          self.phone &&
          self.order.phone &&
          `phone.${self.order.phone}` === self.phone.brand
        );
      };
