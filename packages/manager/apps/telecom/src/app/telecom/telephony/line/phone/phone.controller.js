...
export default class TelecomTelephonyLinePhoneCtrl {
  /* @ngInject */
  constructor(
    $q,
    $stateParams,
  ...
  initActions() {
    this.actions = [
      ...
      },
      {
        ...
        disabled: !(
          this.isAllowedOrder('accessory') || this.isAllowedOrder('accessories')
        ),
        text: this.$translate.instant(
  ...
  init() {
    ...
    return this.TelephonyMediator.getGroup(this.$stateParams.billingAccount)
      .then((group) => {
        ...
        this.line = this.billingAccount.getLine(this.$stateParams.serviceName);
      })
      .then(() =>
