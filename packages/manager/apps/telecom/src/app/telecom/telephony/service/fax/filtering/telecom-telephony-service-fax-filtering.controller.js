...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyServiceFaxFilteringCtrl',
    function TelecomTelephonyServiceFaxFilteringCtrl(
      $filter,
      $q,
      $stateParams,
      $timeout,
  ...
  =============================== */
      ...
      self.removeSelectedScreenLists = function removeSelectedScreenLists() {
        ...
        screenListsTypes.forEach((type) => {
          const rawOfType = map(
            filter(self.screenLists.raw, { type }),
            'number',
          );
        ...
        tucTelephonyBulk
          .getTucToastInfos(bulkResult, {
            ...
            error: $translate.instant(
              'telephony_service_fax_filtering_bulk_error',
            ),
          })
          .forEach((toastInfo) => {
