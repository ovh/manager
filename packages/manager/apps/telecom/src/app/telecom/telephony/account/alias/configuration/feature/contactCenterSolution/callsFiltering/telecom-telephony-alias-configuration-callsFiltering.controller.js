...
angular.module('managerApp').controller(
  'TelecomTelephonyAliasConfigurationCallsFilteringCtrl',
  class TelecomTelephonyAliasConfigurationCallsFilteringCtrl {
    constructor(
      $filter,
      $q,
      $state,
    ...
    $onInit() {
      ...
      this.helpers = this.TELEPHONY_ALIAS_CONTACT_CENTER_SOLUTION.filtering.helperPrefixes.map(
        ({ label }) => ({
          id: label,
          label: this.$translate.instant(
    ...
    addConditions() {
      ...
      if (this.helper.id) {
        const {
          prefixes,
        } = this.TELEPHONY_ALIAS_CONTACT_CENTER_SOLUTION.filtering.helperPrefixes.find(
    ...
    getScreenListNature(callNumber) {
      ...
      const detectedPrefix = this.TELEPHONY_ALIAS_CONTACT_CENTER_SOLUTION.filtering.helperPrefixes.find(
        ({ prefixes }) =>
    ...
    deleteConditions(conditions) {
      this.$uibModal
        .open({
          templateUrl:
            'app/telecom/telephony/account/alias/configuration/feature/contactCenterSolution/callsFiltering/delete/telecom-telephony-alias-configuration-calls-filtering-delete.html',
          controller:
            'TelecomTelephonyAliasConfigurationCallsFilteringDeleteCtrl',
          controllerAs: '$ctrl',
          resolve: {
