...
angular
  .module('managerApp')
  .controller(
    'TelecomTelephonyServiceFaxCampaignsAddCtrl',
    function TelecomTelephonyServiceFaxCampaignsAddCtrl(
      $q,
      $stateParams,
      $translate,
      $timeout,
    ...
    =============================== */
      ...
      function setCampainDateTime() {
        if (self.campaign.sendDate) {
          self.campaign.sendDate.setHours(
            new Date(self.picker.time).getHours(),
          );
      ...
      function init() {
        ...
        return fetchEnums()
          .then((enums) => {
            ...
            self.campaign.sendType = last(
              pull(self.enums.sendType, 'automatic'),
            ); // scheduled
