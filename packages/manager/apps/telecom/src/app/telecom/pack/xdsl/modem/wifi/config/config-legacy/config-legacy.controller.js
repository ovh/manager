...
export default /* @ngInject */ function XdslModemWifiConfigLegacyCtrl(
  $state,
  $q,
  $timeout,
  $stateParams,
  ...
  self.displayLegacyConfig = function displayLegacyConfig() {
    return self.modem.canChangeWLAN;
  };

  self.$onInit = function $onInit() {
    return getWifi().then(
      (response) => {
        self.wifis = map(response, (wifi) => {
          set(
            wifi,
            'channelCustom',
