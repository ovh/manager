...
angular.module('managerApp').controller('XdslModemFirmwareCtrl', class XdslModemFirmwareCtrl {
  ...
  getFirmware() {
    return this.OvhApiXdsl.Modem().Firmware().v6().get({
      ...
      this.TucToast.error(this.$translate.instant('xdsl_modem_firmware_error'));
      return this.$q.reject(err);
    });
  }

  getAvailableFirmware() {
    return this.OvhApiXdsl.Modem().Firmware().v6().available({
      xdslId: this.$stateParams.serviceName,
    }).$promise.then((result) => {
  ...
  changeFirmware() {
    if (isEmpty(this.$stateParams.serviceName)
      ...
      this.firmwareTmp = this.firmware;
      this.TucToast.error(this.$translate.instant('xdsl_modem_firmware_an_error_ocurred'));
      return this.$q.reject();
    }
    this.loading = true;
    return this.OvhApiXdsl.Modem().Firmware().v6().post(
      {
        xdslId: this.$stateParams.serviceName,
      },
      {
      ...
      this.TucToast.success(this.$translate.instant('xdsl_modem_firmware_doing'));
      return data;
    }).catch((err) => {
      this.firmwareTmp = this.firmware;
      this.TucToast.error(this.$translate.instant('xdsl_modem_firmware_an_error_ocurred'));
