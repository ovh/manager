export default /* @ngInject */ function XdslModemWifiConfigEnhancedCtrl(
  ...
  self.$onInit = async function $onInit() {
    ...
    try {
      $q.all([
        wifiConfigEnhancedService.getModemMesh(self.serviceName),
        wifiConfigEnhancedService.getModemUseOnessid(self.serviceName),
      ]).then(async ([mesh, onessid]) => {
        self.mesh = mesh;
        self.onessid = onessid;
        self.loading.features = false;
