...
angular.module('managerApp').controller(
  ...
  class XdslModemTemplateConfigCtrl {
    /* @ngInject */
    constructor(
      $q,
      $stateParams,
    ...
    displayModal(title, question) {
      return this.$uibModal.open({
        animation: true,
        templateUrl:
          'app/telecom/pack/pack/xdsl/modem/templates/config/modal/pack-xdsl-modem-template-config-modal.html',
    ...
    deleteSelectedTemplate({ name }) {
      const modal = this.displayModal(
        this.$translate.instant(
          'xdsl_modem_template_config_modal_title_delete',
        ),
        this.$translate.instant(
      ...
      modal.result.then(() =>
        this.OvhApiXdsl.TemplateModem()
          ...
          .catch(() => {
            this.TucToast.error(
              this.$translate.instant(
                'xdsl_modem_template_config_delete_error',
              ),
            );
          }),
      );
    ...
    updateTemplate() {
      const modal = this.displayModal(
        this.$translate.instant(
          'xdsl_modem_template_config_modal_title_update',
        ),
        this.$translate.instant(
      ...
      return modal.result.then(() =>
        this.OvhApiXdsl.TemplateModem()
          ...
          .catch(() => {
            this.TucToast.error(
              this.$translate.instant(
                'xdsl_modem_template_config_update_error',
              ),
            );
          }),
      );
