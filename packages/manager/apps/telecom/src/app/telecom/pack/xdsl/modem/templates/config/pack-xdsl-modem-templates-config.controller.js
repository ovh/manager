angular.module('managerApp').controller('XdslModemTemplateConfigCtrl', class XdslModemTemplateConfigCtrl {
  /* @ngInject */
  constructor($q, $stateParams, $translate, $uibModal, OvhApiXdsl, TucToast, TucToastError) {
    this.$q = $q;
    ...
    return this.OvhApiXdsl.TemplateModem().v6().query()
      ...
      .then(names => this.$q
        ...
        .then((chunkResult) => {
          ...
          const templates = _.flatten(chunkResult).map(({ error, key, value }) => {
            if (value !== null) {
            ...
            return {
              ...
              detail: {
                capabilities: null,
                info: this.$translate.instant('xdsl_modem_template_config_table_is_empty'),
                creationDate: '',
                DHCP: [],
                LAN: [],
                WLAN: [],
  ...
  setSelectedTemplate(template) {
    ...
    if (this.template.detail.capabilities) {
      this.convertCapabilities(JSON.parse(this.template.detail.capabilities),
        this.template.detail.parametersToIgnore, true);
    } else {
    ...
    if (this.parametersToIgnore.dmzIP) {
      this.template.detail.dmzIP = this.$translate.instant('xdsl_modem_template_config_no_value');
    }
    if (this.parametersToIgnore.mtuSize) {
      this.template.detail.mtuSize = this.$translate.instant('xdsl_modem_template_config_no_value');
    }
    ...
    if (this.template.detail.WLAN) {
      ...
      this.template.detail.WLAN = this.template.detail.WLAN.map(row => ({
        ...row,
        ignore: _.isArray(this.parametersToIgnore.WLANList)
    ...
    if (this.template.detail.portMapping) {
      this.template.detail.portMapping = this.template.detail.portMapping.map(row => ({
        ...row,
        ignore: _.isArray(this.parametersToIgnore.portMappingList)
  ...
  displayModal(title, question) {
    return this.$uibModal.open({
      animation: true,
      templateUrl: 'app/telecom/pack/xdsl/modem/templates/config/modal/pack-xdsl-modem-template-config-modal.html',
  ...
  deleteSelectedTemplate({ name }) {
    const modal = this.displayModal(this.$translate.instant('xdsl_modem_template_config_modal_title_delete'),
      this.$translate.instant('xdsl_modem_template_config_modal_question_delete', { templateName: name }));

    modal.result.then(() => this.OvhApiXdsl.TemplateModem().v6().deleteTemplate({
      name,
    })
      .$promise
      .then(() => this.fetchTemplates())
      .then((data) => { this.templates = data; })
      .then(() => this.TucToast.success(this.$translate.instant('xdsl_modem_template_config_delete_success')))
      .catch(() => {
        this.TucToast.error(this.$translate.instant('xdsl_modem_template_config_delete_error'));
      }));
  }
  ...
  hasConfigChange() {
    ...
    const params = this.convertCapabilities(JSON.parse(this.template.detail.capabilities));
    return !_.isEqual(this.parametersToIgnore, params);
  }
  ...
  updateTemplate() {
    const modal = this.displayModal(this.$translate.instant('xdsl_modem_template_config_modal_title_update'),
      this.$translate.instant('xdsl_modem_template_config_modal_question_update', { templateName: this.template.name }));
    ...
    return modal.result.then(() => this.OvhApiXdsl.TemplateModem().v6().updateTemplate({
      ...
      .then(() => this.fetchTemplates())
      .then((data) => { this.templates = data; })
      .then(() => this.TucToast.success(this.$translate.instant('xdsl_modem_template_config_update_success')))
      .catch(() => {
        this.TucToast.error(this.$translate.instant('xdsl_modem_template_config_update_error'));
      }));
  }
});
