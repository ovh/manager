...
angular
  ...
  .factory(
    ...
    ($q, $translate, $uibModal, OvhApiXdsl, TucToast) => {
      const template = {
        serverEnabled: true,
        defaultGateway: '',
        primaryDNS: '',
          ...
          .catch((err) => {
            if (err.data && err.data.message) {
              ...
              $uibModal.open({
                animation: true,
                templateUrl:
                  'app/telecom/pack/pack/xdsl/modem/router/dhcp/pack-xdsl-modem-dhcp-modal.html',
              ...
              TucToast.error(
                $translate.instant('xdsl_modem_dhcp_submit_error'),
              );
            }
            return $q.reject(err);
          })
       ...
       */
      PackXdslModemDhcpObject.prototype.toggleEdit = function toggleEdit(
        state,
      ) {
        if (isBoolean(state)) {
          this.editMode = state;
