  ...
  self.$onInit = function $onInit() {
    OvhApiXdslOrderFollowup.Aapi().query().$promise.then((result) => {
      ...
      result.forEach((access) => {
        if (access.lastTask) {
          set(access, 'badgeClass', ORDER_STATUS[access.lastTask.status].class);
          set(access, 'badgeIconClass', ORDER_STATUS[access.lastTask.status].icon);
          set(access, 'priority', ORDER_STATUS[access.lastTask.status].priority);
        }
        set(access, 'name', access.description ? access.description : access.xdsl);
      });
