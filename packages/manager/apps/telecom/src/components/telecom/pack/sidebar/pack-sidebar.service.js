...
angular
  ...
  .service(
    'PackSidebar',
    function PackSidebarService($q, $translate, SidebarMenu, TucPackMediator) {
      const self = this;
    ...
    ================================== */
      ...
      function getPackStatus(pack) {
        const isAllAccessInError =
          ...
          every(
            pack.xdsl,
            (xdsl) => accessErrorStates.indexOf(xdsl.status) > -1,
          );
            ...
            const packSection = SidebarMenu.addMenuItem(
              {
                id: pack.packName,
                title:
                  pack.description || pack.offerDescription || pack.packName,
