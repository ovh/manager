...
angular
  ...
  .factory(
    ...
    ========================================= */
      ...
      TelephonyGroup.prototype.initNumbers = function initNumbers(
        numberOptions,
      ) {
        const self = this;
      ...
      TelephonyGroup.prototype.getNumber = function getNumber(
        numberServiceName,
      ) {
        const self = this;
      ...
      TelephonyGroup.prototype.fetchService = function fetchService(
        serviceName,
      ) {
        const self = this;
        let number;
        ...
        return OvhApiTelephony.Service()
          ...
          .$promise.then((consumptions) => {
            const calledFeesPrefix = flatten(
              valuesIn(
                get(TELEPHONY_REPAYMENT_CONSUMPTION, 'calledFeesPrefix'),
              ),
            );

            const groupRepaymentsPrefix = flatten(
              valuesIn(
                get(TELEPHONY_REPAYMENT_CONSUMPTION, 'groupRepaymentsPrefix'),
              ),
            );
