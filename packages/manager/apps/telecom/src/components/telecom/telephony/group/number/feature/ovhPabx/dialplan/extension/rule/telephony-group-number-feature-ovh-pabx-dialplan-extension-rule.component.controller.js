...
angular
  .module('managerApp')
  .controller(
    'telephonyNumberOvhPabxDialplanExtensionRuleCtrl',
    function telephonyNumberOvhPabxDialplanExtensionRuleCtrl(
      $q,
      $translate,
      TelephonyMediator,
      TucToast,
    ...
    =============================== */
      ...
      self.isLoading = function isLoading() {
        return (
          ['SAVING', 'CREATING', 'DELETING', 'MOVING'].indexOf(
            self.rule.status,
          ) > -1
        );
      };
      ...
      self.getActionFamily = function getActionFamily() {
        return self.rule.getActionFamily(
          self.rule.inEdition
            ? self.rule.saveForEdition.action
            : self.rule.action,
        );
      };
      ...
      self.getVoicemailInfos = function getVoicemailInfos() {
        return TelephonyMediator.findService(
          self.getRuleAttribute('actionParam'),
        );
      };
