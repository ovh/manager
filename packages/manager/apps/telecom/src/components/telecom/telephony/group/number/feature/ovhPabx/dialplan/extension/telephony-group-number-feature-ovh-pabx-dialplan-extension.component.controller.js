...
angular
  .module('managerApp')
  .controller(
    'telephonyNumberOvhPabxDialplanExtensionCtrl',
    function telephonyNumberOvhPabxDialplanExtensionCtrl(
      $q,
      $scope,
      $timeout,
      $translate,
    ...
    =============================== */
          ...
          (self.extension &&
            (['OK', 'DELETE_PENDING'].indexOf(self.extension.status) === -1 ||
              some(
                ...
                (screenListCondition) =>
                  ['CREATING', 'DELETING'].indexOf(
                    screenListCondition.state,
                  ) !== -1,
              ) ||
              some(
      ...
      self.getExtensionAttr = function getExtensionAttr(attr) {
        return get(
          self.extension.inEdition
            ? self.extension.saveForEdition
            : self.extension,
          attr,
        );
      };
      ...
      self.getRulesCount = function getRulesCount() {
        return (
          self.extension.rules.length + self.extension.negativeRules.length
        );
      };
