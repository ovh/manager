...
angular
  .module('managerApp')
  .directive('telephonyScheduler', ($compile, $locale) => ({
    restrict: 'E',
    transclude: true,
    templateUrl:
      'components/telecom/telephony/scheduler/telephony-scheduler.html',
      ...
      function compileEvent(element, schedulerEvent, uiCalEvent) {
        ...
        element.attr({
          'data-responsive-popover':
            "'components/telecom/telephony/scheduler/events/telephony-scheduler-events-popup.html'",
          ...
          'data-popover-class':
            'pretty-popover telephony-scheduler-events-popup',
        });
      ...
      const defaultOptions = {
        ...
        select(dateStart, dateEnd, jsEvent) {
          ...
          tmpEventInEdition.scope = compileEvent(
            jsEvent
              ? $(jsEvent.target)
              : getCalendarElem().find('.fc-highlight'),
