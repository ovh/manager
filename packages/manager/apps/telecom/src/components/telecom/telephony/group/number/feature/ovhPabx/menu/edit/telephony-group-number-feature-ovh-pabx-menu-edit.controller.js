...
angular
  .module('managerApp')
  .controller(
    'telephonyNumberOvhPabxMenuEditCtrl',
    function telephonyNumberOvhPabxMenuEditCtrl($scope) {
      const self = this;
    ...
    =============================== */
      ...
      self.getSoundInfos = function getSoundInfos(soundType) {
        const isTts =
          ['greetSoundTts', 'invalidSoundTts'].indexOf(soundType) > -1;

        return !isTts
          ? get(
              self.menuCtrl.ovhPabx.getSound(get(self.menu, soundType)),
              'name',
            )
          : get(
    ...
    ============================== */
      ...
      self.onSoudTypeChange = function onSoudTypeChange() {
        if (self.menuCtrl.popoverStatus.rightPage === 'greetSoundType') {
          ...
          self.menuCtrl.popoverStatus.rightPage === 'invalidSoundType'
        ) {
      ...
      self.onSoundChoiceButtonClick = function onSoundChoiceButtonClick(
        soundType,
      ) {
        self.menuCtrl.popoverStatus.move = true;
        self.menuCtrl.popoverStatus.rightPage = soundType;
