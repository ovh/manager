...
export default function HealthMonitor() {
  ...
  return (
    <RedirectionGuard isLoading={isPending} condition={!isPending && !healthMonitor} route="create">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-7">
        ...
        >
          <div className="flex flex-col w-full">
            ...
            <TileButton
              title={t('octavia_load_balancer_health_monitor_overview_management_edit')}
              href={useHref('./edit')}
            />
            <TileButton
              title={t('octavia_load_balancer_health_monitor_overview_management_delete')}
              href={useHref('./delete')}
            />
          </div>
        </OsdsTile>
        ...
        >
          <div className="flex flex-col w-full">
            ...
            <TileLine
              title={t('octavia_load_balancer_health_monitor_detail_overview_info_name')}
              value={
            ...
            <TileLine
              title={t('octavia_load_balancer_health_monitor_detail_overview_info_type')}
            ...
            {[HEALTH_MONITOR_TYPE.HTTP, HEALTH_MONITOR_TYPE.HTTPS].includes(
              ...
              <TileLine title={LABELS.URL_PATH} value={healthMonitor?.httpConfiguration?.urlPath} />
            )}
            ...
            {[HEALTH_MONITOR_TYPE.HTTP, HEALTH_MONITOR_TYPE.HTTPS].includes(
              ...
              <TileLine
                title={t('octavia_load_balancer_health_monitor_detail_overview_info_expected_code')}
                value={healthMonitor?.httpConfiguration?.expectedCodes}
              />
            )}

            <TileLine title={LABELS.MAX_RETRIES_DOWN} value={healthMonitor?.maxRetriesDown} />

            <TileLine title={LABELS.DELAY} value={healthMonitor?.delay} />

            <TileLine title={LABELS.MAX_RETRIES} value={healthMonitor?.maxRetries} />

            <TileLine title={LABELS.TIMEOUT} value={healthMonitor?.timeout} />

            <TileLine
              title={t('load-balancer:octavia_load_balancer_provisioning_status')}
              value={
                <div>
