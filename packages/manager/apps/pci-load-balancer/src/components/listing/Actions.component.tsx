...
export default function ActionsComponent({ loadBalancer }: Readonly<ActionsComponentProps>) {
  const { t } = useTranslation('load-balancer');
  const { trackClick } = useOvhTracking();

  const loadBalancerDetailHref = useHref(`../${loadBalancer.region}/${loadBalancer.id}`);
  const deleteHref = useHref(`./${loadBalancer.region}/${loadBalancer.id}/delete`);
