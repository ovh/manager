...
describe('TileLine', () => {
  it('renders title with correct text', () => {
    const { getByText } = render(<TileLine title="Test Title" value={<div />} />);
    expect(getByText('Test Title')).toBeInTheDocument();
  });

  it('renders value element', () => {
    const { getByText } = render(<TileLine title="Test Title" value={<div>Test Value</div>} />);
    expect(getByText('Test Value')).toBeInTheDocument();
  });

  it('renders divider', () => {
    const { container } = render(<TileLine title="Test Title" value={<div />} />);
  ...
  it('applies correct level to title', () => {
    const { getByText } = render(<TileLine title="Test Title" value={<div />} />);
    expect(getByText('Test Title')).toHaveAttribute('level', ODS_TEXT_LEVEL.heading);
  });

  it('applies correct color to title', () => {
    const { getByText } = render(<TileLine title="Test Title" value={<div />} />);
    expect(getByText('Test Title')).toHaveAttribute('color', ODS_THEME_COLOR_INTENT.text);
  });
});
