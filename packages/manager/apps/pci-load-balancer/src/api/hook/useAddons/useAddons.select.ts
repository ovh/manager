...
export const filterProductRegionBySize = (products: TProductAddonDetail[], region: string) =>
  products
    .filter(({ regions }) => regions.some(({ name }) => name === region))
    .sort((a, b) => SIZE_ORDER.indexOf(a.size) - SIZE_ORDER.indexOf(b.size));

export const getAddonsRegions = (plans: TProductAvailability['plans'], catalog?: TCatalog) =>
...
export const getHourlyAddons = (addons: TAddonRegions) =>
  addons.filter((addon) =>
    addon.pricings.find(({ intervalUnit }) => ['none', 'hour'].includes(intervalUnit)),
  );
