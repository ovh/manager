...
export const BillingInformationTileContext = createContext<
  BillingInformationContextParams
>(null);

export const useBillingInformationContextServiceDetails = () => {
  const { resourceName } = useContext(BillingInformationTileContext);
  ...
  const isLoading = useMemo(() => isServiceDetailsLoading || !resourceName, [
