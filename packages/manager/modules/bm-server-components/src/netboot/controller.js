...
export default class BmServerComponentsNetbootCtrl {
  /* @ngInject */
  constructor($scope, $http, $q, $translate, atInternet, netbootService) {
    this.$scope = $scope;
    this.$http = $http;
    this.$q = $q;
  ...
  getActiveOptions(networkOption) {
      ...
      .catch((error) =>
        this.handleError(
          ...
          this.$translate.instant(
            'server_configuration_netboot_fail',
            { t0: this.server.name },
          ),
        )
      )
  ...
  loadRescueMail() {
    this.netbootService.getRescueMail(this.serviceName)
      .then((server) => {
  ...
  loadNetboots() {
    ...
    this.netbootService.getNetboot(this.serviceName)
      .then((netboots) => {
