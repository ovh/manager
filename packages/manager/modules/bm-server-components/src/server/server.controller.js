...
export default class ServerCtrl {
  ...
  showGameDDoSModal() {
    ...
    let serverGameDDoSNotShowModale = [];

    if (storageServersString) {
      const storageServers = JSON.parse(storageServersString);
      serverGameDDoSNotShowModale = storageServers;
    }
  ...
  saveNoShowModalAgain() {
    if (this.noShowModalAgain) {
      ...
      if (storageServersString) {
        ...
        localStorage.setItem(
          this.gameDDoSModalStorageKey,
          JSON.stringify([this.server.name]),
        );
      }
    }
  }
