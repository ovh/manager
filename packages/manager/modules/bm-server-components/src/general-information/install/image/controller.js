...
export default class DedicatedServerInstallImageCtrl {
  ...
  $onInit() {
    ...
    this.trackingByoiDocumentation = `dedicated::dedicated::${this.serverType}::system-install::personalized-image::byoi-documentation`;
  }
  ...
  onImageFormSubmit() {
    ...
    this.atInternet.trackClick({
      name: `dedicated::dedicated::${this.serverType}::system-install::personalized-image::install`,
      type: 'action',
    });
    this.loaders.launchInstall = true;
    this.launchInstallError = null;

    const httpHeader = this.model.httpHeader
      .filter(({ model }) => model.key && model.value)
      .map(({ model }) => model);
    ...
    if (configdrive.enable) {
      configdrive = {
        ...
        userMetadatas: this.model.configdrive.metadata
          .filter(({ model }) => model.key && model.value)
          .map(({ model }) => model),
      };
    }
