...
const DashboardWidgetModal = <TData,>() => {
  ...
  const { serviceName, productType, widgetId } = useParams();
  ...
  if (!config) {
    return (
      <Message className="m-4" color={MESSAGE_COLOR.warning}>
        <span>{t('dashboard-widget-config-not-found')}</span>
      </Message>
    );
  }
            ...
            <div>
              {isLoading ? null : (
                <Button
                  ...
                  slot="actions"
                >
                  {t('dashboard-widget-analyse-in-all-metrics')}
                </Button>
              )}
            </div>
