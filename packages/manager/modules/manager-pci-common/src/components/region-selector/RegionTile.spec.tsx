...
vi.mock('./RegionLocalzoneChip.component', () => ({
  RegionLocalzoneChip: ({ region: { type } }: { region: TLocalisation }) =>
    `chip-${type}`,
}));
vi.mock('./RegionGlobalzoneChip.component', () => ({
  RegionGlobalzoneChip: ({ region: { type } }: { region: TLocalisation }) =>
    `chip-${type}`,
}));
vi.mock('./Region3AZChip.component', () => ({
  Region3AZChip: ({ region: { type } }: { region: TLocalisation }) =>
    `chip-${type}`,
}));
