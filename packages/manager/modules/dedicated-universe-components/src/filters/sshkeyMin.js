export default /* @ngInject */ function sshkeyMinFilter(ducSshkeyRegex) {
  let splitted;
  let minLength;
  let innerkeyLength;
  let subLength;
  ...
  return function sshkeyMin(keyParam) {
    ...
    /* eslint-disable */
      for (i = ducSshkeyRegex.length; i--; ) {
        if (ducSshkeyRegex[i].regex.test(key)) {
          type = ducSshkeyRegex[i];
          splitted = key.match(ducSshkeyRegex[i].regex);
          break;
        }
      }
      /* eslint-enable */
