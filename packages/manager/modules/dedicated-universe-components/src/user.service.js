...
export default /* @ngInject */ function userF(
  $http,
  $q,
  ducConstants,
  ducPaymentMeans,
  ...
  this.getUrlOf = function getUrlOf(link) {
    return this.getUser().then((data) => {
      try {
        return ducConstants.urls[data.ovhSubsidiary][link];
      } catch (exception) {
        return ducConstants.urls.FR[link];
      }
    });
  };
   ...
   */
  this.getUrlOfEndsWithSubsidiary = function getUrlOfEndsWithSubsidiary(link) {
    return this.getUser().then((data) => {
      try {
        return ducConstants.urls[link][data.ovhSubsidiary];
      } catch (exception) {
        return ducConstants.urls[link].FR;
      }
    });
  };
  ...
  this.getValidPaymentMeansIds = function getValidPaymentMeansIds() {
    const means = ducPaymentMeans.paymentMeans;
    const baseUrl = `${ducConstants.swsProxyRootPath}me/paymentMean`;
    const meanRequests = [];
