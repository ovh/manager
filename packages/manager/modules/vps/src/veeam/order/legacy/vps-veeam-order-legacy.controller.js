export default class VpsOrderVeeamLegacyCtrl {
  ...
  constructor(
    ...
    VpsService.getSelectedVps(this.serviceName)
      .then((data) => {
        ...
        VpsService.getVeeamOption(this.serviceName).then((veeamOption) => {
      ...
      .catch((error) => {
        this.CucCloudMessage.error(
          `${this.$translate.instant(
            'vps_configuration_veeam_order_step1_loading_error',
          )} ${error.data}`,
        );
      });
  }
  ...
  $onInit() {
    ...
    this.VpsService.canOrderOption(this.serviceName, 'automatedBackup').catch(
      () => {
        this.CucCloudMessage.error(
          this.$translate.instant('vps_tab_VEEAM_dashboard_veeam_unavailable'),
        );
      },
    );
  }
  ...
  orderOption() {
    if (
      this.$scope.model.optionDetails &&
      this.$scope.model.contractsValidated
    ) {
      this.CucServiceHelper.loadOnNewPage(
        this.VpsService.orderVeeamOption(
          this.serviceName,
          this.$scope.model.optionDetails.duration.duration,
        ),
      ).then(({ url }) => {
      ...
      this.CucCloudMessage.error(
        this.$translate.instant('vps_configuration_veeam_order_fail'),
      );
    }
  }
