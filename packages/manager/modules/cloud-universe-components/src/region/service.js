...
export default class CucRegionService {
  ...
  getTranslatedMacroRegion(region) {
    const translatedMacroRegion = this.$translate.instant(`cuc_region_${this.constructor.getMacroRegion(region)}`);
    return translatedMacroRegion || region;
  }

  getTranslatedMicroRegion(region) {
    const translatedMicroRegion = this.$translate.instant(`cuc_region_${this.constructor.getMacroRegion(region)}_micro`, {
      micro: region,
    });
  ...
  getTranslatedMicroRegionLocation(region) {
    const translatedMicroRegionLocation = this.$translate.instant(`cuc_region_location_${this.constructor.getMacroRegion(region)}`);
  ...
  getTranslatedRegionContinent(region) {
    const translatedRegionContinent = this.$translate.instant(`cuc_region_continent_${this.constructor.getMacroRegion(region)}`);
