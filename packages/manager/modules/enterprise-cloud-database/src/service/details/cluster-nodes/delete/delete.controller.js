...
export default class EnterpriseCloudDatabaseServiceDetailsClusterSizeDeleteCtrl {
  ...
  $onInit() {
    const additionalReplicaCount =
      this.hosts.length -
      (INCLUDED_CLUSTER_SIZE.PRIMARY +
        INCLUDED_CLUSTER_SIZE.REPLICA +
        INCLUDED_CLUSTER_SIZE.BACKUP);
    this.replicaCounts = map(
      range(1, additionalReplicaCount + 1),
      (replicaNumber) => ({
        replicaNumber,
        text: `${replicaNumber} ${
          replicaNumber > 1
            ? this.$translate.instant(
                'enterprise_cloud_database_common_replicas',
              )
            : this.$translate.instant(
                'enterprise_cloud_database_common_replica',
              )
        }`,
      }),
    );
  ...
  deleteReplicas() {
    ...
    this.enterpriseCloudDatabaseService
      ...
      .then(() => {
        ...
        return this.goBackToClusterSize(
          this.$translate.instant(
            'enterprise_cloud_database_service_details_cluster_nodes_delete_success',
            {
      ...
      .catch((error) =>
        this.goBackToClusterSize(
          this.$translate.instant(
            'enterprise_cloud_database_service_details_cluster_nodes_delete_error',
            {
