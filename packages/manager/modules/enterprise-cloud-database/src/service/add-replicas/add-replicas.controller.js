...
export default class EnterpriseCloudDatabaseServiceAddReplicasCtrl {
  ...
  $onInit() {
    const orderableReplicaCount = min([
      this.maxHostCount - this.hostList.length,
    ...
    this.replicaCounts = map(
      range(1, orderableReplicaCount + 1),
      (replicaNumber) => ({
        replicaNumber,
        text: `${replicaNumber} ${
          replicaNumber > 1
    ...
    if (this.createReplicas) {
      ...
      this.enterpriseCloudDatabaseService
        ...
        .then((order) => {
          ...
          return this.goBack(
            this.$translate.instant(
              'enterprise_cloud_database_service_add_replicas_success',
              {
        ...
        .catch((error) =>
          this.goBack(
            this.$translate.instant(
              'enterprise_cloud_database_service_add_replicas_error',
              {
