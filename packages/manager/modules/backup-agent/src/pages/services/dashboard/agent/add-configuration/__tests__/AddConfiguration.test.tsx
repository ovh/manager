...
import { useNavigateMock, LinkMock } from '@/test-utils/mocks/react-router-dom';
...
describe('FirstOrderFormComponent', () => {
  ...
  it('renders onboarding and expected generate order link', async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByText(`translated_required_field`).length).toBe(2),
    );
    ...
    await waitFor(
      () =>
        expect(
          screen.queryByText(`translated_error_required_field`),
