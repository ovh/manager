angular.module('managerApp').controller('PackEmailProDetailCtrl', class PackEmailProDetailCtrl {
  ...
  loadPackInfo() {
    return this.OvhApiPackXdsl.Aapi().get({ packId: this.$stateParams.packName }).$promise
      .then(({ general }) => {
        this.pack = general;
        return this.pack;
      });
  ...
  getAccount() {
    return this.OvhApiXdsl.Email().Pro().v6().get({ email: this.$stateParams.serviceName }).$promise
      .then((emailPro) => {
  ...
  changePassword() {
    ...
    return this.OvhApiXdsl.Email().Pro().v6().changePassword({
