...
export default /* @ngInject */ ($scope, $stateParams, $translate, EmailPro, User) => {
  // default values
  $scope.accountsToAdd = {
    duration: '12',
    accountsNumber: 1,
  ...
  User.getUser()
    ...
    .catch((failure) => {
      $scope.setMessage($translate.instant('emailpro_ACTION_order_accounts_step1_user_error'),
        failure.data);
  ...
  $scope.order = function order() {
    ...
    EmailPro.orderAccounts($stateParams.productId, $scope.accountsToAdd).then((data) => {
      ...
      $scope.setMessage($translate.instant('emailpro_ACTION_order_accounts_step2_error_message'),
        failure.data);
      $scope.resetAction();
    });
  };
  ...
  $scope.loadOrderList = function loadOrderList() {
    EmailPro.getOrderList($stateParams.productId).then((data) => {
      ...
      $scope.setMessage($translate.instant('emailpro_ACTION_order_accounts_step1_loading_error'),
        failure.data);
      $scope.resetAction();
    });
  };
