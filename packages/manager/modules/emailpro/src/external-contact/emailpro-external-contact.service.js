...
export default /* @ngInject */ function EmailProExternalContacts(
  EmailPro,
  OvhHttp,
) {
  ...
  this.removeContact = function removeContact(
    ...
    return EmailPro.gettingBaseAPIPath()
      .then((baseAPIPath) =>
        OvhHttp.delete(
          `/${baseAPIPath}/{exchange}/externalContact/{externalEmailAddress}`,
          {
            rootPath: 'apiv6',
            urlParams: {
              organization,
  ...
  this.addContact = function addContact(organization, serviceName, newContact) {
    return EmailPro.gettingBaseAPIPath()
      .then((baseAPIPath) =>
        OvhHttp.post(`/${baseAPIPath}/{exchange}/externalContact`, {
          rootPath: 'apiv6',
          urlParams: {
            organization,
            exchange: serviceName,
  ...
  this.getContactOptions = function getContactOptions(
    organization,
    serviceName,
  ) {
    return EmailPro.gettingBaseAPIPath()
      .then((baseAPIPath) =>
        OvhHttp.get(`/${baseAPIPath}/{exchange}/domain`, {
          rootPath: 'apiv6',
          urlParams: {
            organization,
            exchange: serviceName,
