...
import { BackupLicencesContext, BackupLicencesProviderProps } from '@/BackupLicences.context';
...
export const addAppContextProvider = (
  ...
  const backupLicencesContext = appContext ?? { appName: 'backup-licences', scope: 'Enterprise' };

  providers.push(({ children }) => (
    <BackupLicencesContext.Provider value={backupLicencesContext}>{children}</BackupLicencesContext.Provider>
  ));
};
