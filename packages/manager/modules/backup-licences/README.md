...
`@ovh-ux/backup-licences`

### update your package.json dependencies

Add the backup-licences package to your dependencies:
...
Add the backup-licences translation files to your Vite configuration using `vite-plugin-static-copy`:
...
const BACKUP_LICENCES_PACKAGE = '@ovh-ux/backup-licences';
...
export default defineConfig({
  ...
  plugins: [
    ...
    viteStaticCopy({
      targets: [{
        src: `${dirname(require.resolve(BACKUP_LICENCES_PACKAGE))}/../public/**/*`,
        dest: `translations/@ovh-ux/backup-licences`,
      }],
    }),
  ]
});
...
const backupLicencesModuleDir = pkgDir('@ovh-ux/backup-licences');   // Add backup agent dir resolution

const baseTailwindConfig = [
  ...(baseConfig.content ?? []),
  './src/**/*.{js,jsx,ts,tsx}',
  toGlob(reactComponentsDir),
  toGlob(backupLicencesModuleDir)    // Add it on base tailwind config
];
...
Wrap your application with the `BackupLicencesContext.Provider` in your main layout:

```tsx
import { BackupLicencesContext } from '@ovh-ux/backup-licences';
...
export default function MainLayout() {
  ...
  return (
    <BackupLicencesContext.Provider value={{ appName, scope: 'Enterprise' }}>
      <Suspense fallback={null}>
        <Outlet />
      </Suspense>
    </BackupLicencesContext.Provider>
  );
}
```
...
To allow the module to handle all backup-licences-related routes, configure your routes as follows:
...
The `BackupLicencesContext` accepts the following properties:
...
```bash
yarn dev:watch --filter="@ovh-ux/backup-licences"
```
