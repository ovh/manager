...
import { ServiceDetails } from '../../types/services.type';
...
export const useServiceDetailsQueryOption = ({
  ...
  return queryOptions<ApiResponse<ServiceDetails>, ApiError | Error>({
    ...
    queryFn: async () => {
      ...
      if (!data?.[0]) {
        throw new Error(
          `No service found for resource ${resourceName}, cannot fetch service details`,
        );
      }
...
export type UseServiceDetailsParams = {
  ...
  resourceName: string;
  options?: Partial<
    DefinedInitialDataOptions<ApiResponse<ServiceDetails>, ApiError | Error>
  >;
};
