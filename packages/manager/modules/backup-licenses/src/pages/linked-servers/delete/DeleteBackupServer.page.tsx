...
import { Modal, useAuthorizationIam, useNotifications } from '@ovh-ux/manager-react-components';
 ...
 *
 * Check IAM (`vspc/backupLicenses/delete`) indépendant de celui du menu ⋮ : protège un accès
 * direct par URL. `Modal` (`@ovh-ux/manager-react-components`) ne permet pas de wrapper son
...
export default function DeleteBackupServerPage() {
  ...
  return (
    <Modal
      ...
      isPrimaryButtonDisabled={isPending || !server || isIamBlocked}
      ...
      <div className="flex flex-col gap-4">
        {isIamBlocked && (
          <OdsMessage
            ...
            isDismissible={false}
            data-testid="delete-backup-server-iam-warning"
          >
            <OdsText>{t(`${NAMESPACES.IAM}:iam_actions_message`)}</OdsText>
          </OdsMessage>
        )}
        {!!error && (
          <OdsMessage
