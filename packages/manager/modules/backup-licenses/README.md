...
Add the backup-licenses package to your dependencies:
...
Add the backup-licenses translation files to your Vite configuration using `vite-plugin-static-copy`:
  ...
  plugins: [
    ...
    viteStaticCopy({
      targets: [{
        ...
        dest: `translations/@ovh-ux/backup-licenses`,
      }],
    }),
  ]
});
...
const backupLicensesModuleDir = pkgDir('@ovh-ux/backup-licenses');   // Add backup agent dir resolution

const baseTailwindConfig = [
  ...(baseConfig.content ?? []),
  './src/**/*.{js,jsx,ts,tsx}',
  toGlob(reactComponentsDir),
  toGlob(backupLicensesModuleDir)    // Add it on base tailwind config
];
...
Wrap your application with the `BackupLicensesContext.Provider` in your main layout:

```tsx
import { BackupLicensesContext } from '@ovh-ux/backup-licenses';
...
export default function MainLayout() {
  ...
  return (
    <BackupLicensesContext.Provider value={{ appName, scope: 'Enterprise' }}>
...
To allow the module to handle all backup-licenses-related routes, configure your routes as follows:
...
The `BackupLicensesContext` accepts the following properties:
...
```bash
yarn dev:watch --filter="@ovh-ux/backup-licenses"
```
