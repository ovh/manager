...
export default /* @ngInject */ function (
  ...
  self.fetchPrivateNetworks = function fetchPrivateNetworksFn() {
  ...
  self.getGlobalNetwork = function getGlobalNetworkFn() {
    return [self.models.subnet.address, self.models.subnet.mask].join('/');
  };

  self.getPrivateNetwork = function getPrivateNetworkFn() {
    return self.models.privateNetwork;
  };

  self.getSubnetTemplate = function getSubnetTemplateFn() {
    return self.models.subnet;
  };

  self.isDHCPEnabled = function isDHCPEnabledFn() {
    return self.models.subnet.dhcp;
  };

  self.fetchRegions = function fetchRegionsFn() {
  ...
  self.getRegions = function getRegionsFn() {
    return self.collections.regions;
  };

  self.hasRegion = function hasRegionFn(region) {
    return indexOf(self.collections.regions, region) !== -1;
  };

  self.toggleActiveRegion = function toggleActiveRegionFn(region) {
  ...
  self.hasActiveRegion = function hasActiveRegionFn(region) {
    return indexOf(self.models.privateNetwork.regions, region) !== -1;
  };

  self.getActiveRegions = function getActiveRegionsFn() {
    return self.models.privateNetwork.regions;
  };

  self.isRegionTogglingDisabled = function isRegionTogglingDisabledFn(region) {
  ...
  self.hasErrors = function hasErrorsFn(ngModel) {
    if (!ngModel) {
      return false;
    }
  ...
  self.getGroupClasses = function getGroupClassesFn(ngModel) {
    if (!ngModel) {
      return {};
    }
  ...
  self.getActivePage = function getActivePageFn() {
    return self.states.page;
  };

  self.getActivePageIndex = function getActivePageIndexFn() {
  ...
  self.isActivePage = function isActivePageFn(page) {
    if (!self.states.page) {
      return false;
    }
  ...
  self.setActivePage = function setActivePageFn(page) {
    self.states.page = page;
  };

  self.isSwitchMode = function isSwitchModeFn() {
  ...
  self.getApiUrl = function getApiUrlFn() {
  ...
  self.presetNetwork = function presetNetworkFn(id) {
    if (id) {
  ...
  self.hasUntaggedVlan = function hasUntaggedVlanFn(networks) {
  ...
  self.findVlanWithID = function findVlanWithIDFn(networks, id) {
    return find(networks, { vlanId: id });
  };

  self.close = function closeFn() {
    $rootScope.$emit('private-network-dialog:hide');
  };

  self.changeSubnets = function changeSubnetsFn(region) {
