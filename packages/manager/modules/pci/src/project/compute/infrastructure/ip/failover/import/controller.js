...
export default /* @ngInject */ function CloudProjectComputeInfrastructureIpFailoverImportCtrl(
  $scope,
  $uibModalInstance,
  OvhApiIp,
  $translate,
   ...
   */
  self.initIps = function initIpsFn(ips) {
    const queries = [];
    self.datas.ipsFo = [];
  ...
  self.refreshIpsFo = function refreshIpsFoFn() {
    getIpsFo(true);
  };

  self.getSelectedCount = function getSelectedCountFn() {
    return Object.keys(self.datas.selected).length;
  };

  self.getInfoSelect = function getInfoSelectFn() {
