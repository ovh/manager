...
export default /* @ngInject */ function CloudProjectComputeVolumeCtrl(
  $scope,
  $filter,
  $q,
  $timeout,
  ...
  self.orderBy = function order(by) {
    ...
    const orderByExpression = self.order.by !== 'price' ? self.order.by : function calculateMonthlyPrice(volume) {
