...
export default class PciProjectLabsService {
  /* @ngInject */
  constructor($q, OvhApiCloudProjectLab, OvhApiMeAgreements) {
    this.$q = $q;
  ...
  getLabs(projectId) {
    return this.OvhApiCloudProjectLab.v6().query({
  ...
  getLab(projectId, labId) {
    return this.OvhApiCloudProjectLab.v6()
      .get({
        serviceName: projectId,
        labId,
      })
      .$promise.then((lab) => new Lab({ ...lab }))
      .then((lab) => {
        if (lab.isOpen()) {
  ...
  getLabAgreements(projectId, lab) {
    return this.OvhApiCloudProjectLab.v6()
      ...
      .$promise.then(({ accepted, toAccept }) =>
        this.$q.all({
          lab: new Lab({
            ...lab,
            agreements: {
      ...
      .then(
        ({ lab: labWithAgreements, contracts }) =>
          new Lab({
            ...labWithAgreements,
            contracts,
          }),
  ...
  getContract(id) {
    return this.OvhApiMeAgreements.v6()
      .contract({
        id,
      })
  ...
  acceptContract(id) {
    return this.OvhApiMeAgreements.v6().accept(
      {
        id,
      },
      {},
  ...
  activateLab(projectId, lab) {
    return this.$q
      .all(lab.toAcceptContracts.map(({ id }) => this.acceptContract(id)))
      .then(
        () =>
          this.OvhApiCloudProjectLab.v6().activate({
            serviceName: projectId,
            labId: lab.id,
          }).$promise,
      );
