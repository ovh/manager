export default /* @ngInject */ ($stateProvider) => {
  $stateProvider.state('pci.projects.project.kubernetes.details.nodes', {
    ...
    resolve: {
      addNode: /* @ngInject */ ($state, kubeId, projectId) => () =>
        $state.go('pci.projects.project.kubernetes.details.nodes.add', {
          kubeId,
          projectId,
        }),

      deleteNode: /* @ngInject */ ($state, kubeId, projectId) => (nodeId) =>
        $state.go('pci.projects.project.kubernetes.details.nodes.delete', {
          kubeId,
          projectId,
          nodeId,
        }),

      switchBillingType: /* @ngInject */ ($state, kubeId, projectId) => (
        nodeId,
        nodeName,
        instanceId,
      ) =>
      ...
      nodes: /* @ngInject */ (kubeId, OvhApiCloudProjectKube, projectId) =>
        OvhApiCloudProjectKube.Node()
          .v6()
          .query({
      ...
      goToKubernetesNodes: ($state, CucCloudMessage, kubeId, projectId) => (
        message = false,
        type = 'success',
      ) => {
