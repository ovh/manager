...
export default class KubernetesServiceCtrl {
  ...
  $onInit() {
    ...
    this.$scope.$on('pci.projects.project.kubernetes.details.service.refresh', () => this.getClusterInfos());
  ...
  rename(displayName) {
    ...
    return this.Kubernetes.updateKubernetes(this.serviceName, this.cluster)
      .then(() => this.$scope.$emit('sidebar.kubernetes.name.update', { serviceName: this.serviceName, title: displayName }));
  }
  ...
  resetCluster() {
    return this.$state.go('pci.projects.project.kubernetes.details.service.reset', {
  ...
  updateCluster() {
    return this.$state.go('pci.projects.project.kubernetes.details.service.update', {
      cluster: this.cluster,
    });
  }

  terminate() {
    return this.$state.go('pci.projects.project.kubernetes.details.service.terminate', {
      cluster: this.cluster,
    });
  }
}
