...
export default /* @ngInject */ ($stateProvider) => {
  $stateProvider.state('pci.projects.project.workflow.new', {
    ...
    resolve: {
      selectedInstance: /* @ngInject */ ($transition$) =>
        $transition$.params().selectedInstance,
      instances: /* @ngInject */ (projectId, OvhApiCloudProjectInstance) =>
      ...
      regions: /* @ngInject */ ($q, projectId, OvhApiCloudProjectRegion) =>
      ...
      isWorkflowSupportedOnRegion: /* @ngInject */ (regions) => (
        regionName,
      ) => {
        const region = find(
          regions,
          (regionObj) => regionObj.name === regionName,
        );
        return !isEmpty(
          find(region.services, { name: 'workflow', status: 'UP' }),
        );
      },
      ...
      cancelLink: /* @ngInject */ ($state, projectId) =>
        $state.href('pci.projects.project.workflow', {
          projectId,
        }),

      breadcrumb: /* @ngInject */ ($translate) =>
        $translate.instant('pci_workflow_add'),
    },
  });
};
