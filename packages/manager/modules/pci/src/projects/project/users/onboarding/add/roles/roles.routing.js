...
export default /* @ngInject */ ($stateProvider) => {
  $stateProvider.state('pci.projects.project.users.onboarding.add.roles', {
    ...
    layout: {
      name: 'modal',
      redirectTo: 'pci.projects.project.users.onboarding',
    },
    resolve: {
      breadcrumb: /* @ngInject */ () => null,
      rolesList: /* @ngInject */ (roles) =>
        map(roles.roles, ({ name, description }) => ({
          id: name,
          name,
          description,
        })),
      ...
      confirmRoles: /* @ngInject */ (
        ...
        PciProjectsProjectUsersService.add(projectId, { description }, roles)
          ...
          .catch((err) =>
            goBack(
              $translate.instant('pci_projects_project_users_add_error_save', {
