...
export default /* @ngInject */ function CloudProjectComputeSnapshotCtrl(
  ...
  self.orderBy = function orderBy(by) {
    ...
    self.table.snapshotFilter = orderByFilter(
      self.table.snapshotFilter,
      self.order.by,
      self.order.reverse,
    );
    self.table.snapshotFilterCheckbox = orderByFilter(
  ...
  self.createVmBySnapshot = function createVmBySnapshot(snapshot) {
    ...
    $state.go('pci.projects.project.legacy.compute.infrastructure.diagram');
  };
