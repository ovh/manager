...
export default /* @ngInject */ function CloudProjectAddCtrl(
  ...
  this.paymentmeanUrl = PCI_REDIRECT_URLS[coreConfig.getRegion()].paymentMeans;
  // Add credit card URL
  this.addCreditCardUrl = PCI_REDIRECT_URLS[coreConfig.getRegion()].addCreditCard;
  ...
  this.createProject = function createProject() {
    atInternet.trackClick({
      name: 'project_new-project-create',
      type: 'action',
    });
      ...
      switch (response.status) {
        ...
        case 'ok':
          if (response.project) {
            // Success: go to it
            updateManager(response.project);
            return $state.go('pci.projects.project.legacy.details', {
              projectId: response.project,
              fromProjectAdd: true,
            });
          }
  ...
  this.paymentRegister = function paymentRegister() {
    atInternet.trackClick({
      name: 'project_payment-means-register',
      type: 'action',
    });
  ...
  function init() {
    ...
    if (CucFeatureAvailabilityService.hasFeature('PROJECT', 'expressOrder')) {
      $state.go('pci.project-onboarding', { location: 'replace' });
      return;
    }
