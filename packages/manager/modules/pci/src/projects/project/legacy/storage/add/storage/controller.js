...
export default /* @ngInject */ function storageCtrl(
  $q,
  $scope,
  $state,
  $stateParams,
  ...
  function loadMessage() {
    CucCloudMessage.unSubscribe('pci.projects.project.legacy.compute.storage');
    $scope.messageHandler = CucCloudMessage.subscribe('pci.projects.project.legacy.compute.storage', { onMessage: () => refreshMessage() });
  }

  $scope.loadStep = function loadStepFn(step) {
  ...
  $scope.addStorage = function addStorageFn() {
    ...
    return CloudStorageContainers.create(
      ...
      .then((resp) => {
        ...
        return $timeout(() => {
          ...
          $state.go('pci.projects.project.legacy.compute.storage.detail-container', { projectId: $scope.projectId, storageId: resp.id });
        }, 3000)
          .then(() => resp);
      })
      .finally(() => {
