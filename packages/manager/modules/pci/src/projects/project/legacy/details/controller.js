export default /* @ngInject */ function CloudProjectDetailsCtrl(
  $stateParams,
  $q,
  $state,
  $rootScope,
    ...
    switch (project.status) {
      case 'ok':
        ...
        if (!$stateParams.fromProjectAdd) {
          return $state.go('pci.projects.project.legacy.compute', {
  ...
  $scope.$on('$destroy', () => {
    Poller.kill({ namespace: 'pci.projects.project.legacy.details' });
  });
