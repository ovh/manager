...
export default class PrivateNetworksAddCtrl {
  ...
  trackCheckBoxClick(value, name) {
    ...
    this.trackPrivateNetworks(`${TRACKING_PREFIX}::checkbox-${name}::${type}`);
  }
  ...
  loadGatewayDetails() {
    return this.PciPrivateNetworksAdd.getGateways(
      this.projectId,
      ...
      .then((gateways) => {
        ...
        if (this.gateway) {
          this.trackGatewayBanners('with-public-gateway');
        } else if (!this.gateway && gateways.length) {
          this.trackGatewayBanners('with-public-gateway-snat-disabled');
          [this.gateway] = gateways;
        } else {
          this.trackGatewayBanners('no-public-gateway');
          this.gatewayName = getAutoGeneratedName(
            `gateway-${this.configuration.region?.name.toLowercase()}`,
          );
          this.PciPrivateNetworksAdd.getSmallestGatewayInfo(
  ...
  enableSnatOnGateway() {
    if (this.gateway && !this.gateway.externalInformation && this.enableSNAT) {
      return this.PciPrivateNetworksAdd.enableSnatOnGateway(
        this.projectId,
        this.configuration.region?.name,
        this.gateway.id,
      );
