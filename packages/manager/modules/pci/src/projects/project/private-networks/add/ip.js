...
export default class IP {
  static splitSubnetIpAddresses(networkAddress, cidr, numberOfChunk) {
    ...
    const splitData = IP.getIpSplitData(networkAddress, networkMask, numberOfChunk);
  ...
  static getIpSplitData(networkAddress, networkMask, numberOfChunk) {
    const hostAddressInteger = parseInt(IP.convertIpToByteString(networkAddress), 2);
    const networkMaskInteger = parseInt(IP.convertIpToByteString(networkMask), 2);
    const lastHostBit = IP.getNumberOfBitAllocatedToHost(networkAddress, networkMask);
  ...
  static validateSplitData(splitData) {
    ...
    } else if (this.isAdressPartOfSubnet(splitData.networkAddress, splitData.networkMask)) {
      isValid = false;
  ...
  static getNumberOfBitAllocatedToHost(networkAddress, networkMask) {
    const lastHostMaskBit = IP.convertIpToByteString(networkMask, true).indexOf(1);

    if (!networkAddress) {
      return lastHostMaskBit;
    }

    const lastHostAddressBit = IP.convertIpToByteString(networkAddress, true).indexOf(1);
  ...
  static convertIpToByteString(ip, reverse) {
    ...
    const bytes = ipParts.reduce((byteString, octet) => byteString.concat(padStart(octet.toString(2), 8, '0')), '');
