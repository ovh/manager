...
export default class PciProjectStreamService {
  /* @ngInject */
  constructor($q, OvhApiCloudProjectIo, PciProjectStreamsTokensService) {
    this.$q = $q;
  ...
  getAll(projectId) {
    return this.OvhApiCloudProjectIo.Stream()
      ...
      .$promise.then((streams) =>
        this.$q.all(streams.map((streamId) => this.get(projectId, streamId))),
      );
  }
  ...
  getRegion(projectId, stream) {
    return this.OvhApiCloudProjectIo.Capabilities()
      .Stream()
      .Region()
      .v6()
      .get({
  ...
  update(
    projectId,
    { id: streamId, backlog, description, retention, throttling },
  ) {
    return this.OvhApiCloudProjectIo.Stream()
      .v6()
      .edit(
  ...
  getTokens(projectId, stream) {
    return this.PciProjectStreamsTokensService.getAll(projectId, stream).then(
      (tokens) =>
        new Stream({
          ...stream,
          tokens,
