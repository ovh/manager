  ...
  $stateProvider.state('pci.projects.project.streams', {
    ...
    resolve: {
      breadcrumb: /* @ngInject */ ($translate) =>
        $translate.instant('pci_projects_project_streams_title'),
      lab: /* @ngInject */ (PciProjectLabsService, projectId) =>
        PciProjectLabsService.getLabByName(projectId, 'ioStream'),

      streams: /* @ngInject */ (PciProjectStreamService, projectId) =>
      ...
      viewStream: /* @ngInject */ ($state, projectId) => (stream) =>
        $state.go('pci.projects.project.streams.stream', {
          projectId,
          streamId: stream.id,
        }),
      deleteStream: /* @ngInject */ ($state, projectId) => (stream) =>
        $state.go('pci.projects.project.streams.delete', {
          projectId,
          streamId: stream.id,
        }),
      addStreamLink: /* @ngInject */ ($state, projectId) =>
        $state.href('pci.projects.project.streams.add', {
          projectId,
        }),
      ...
      goToStreams: /* @ngInject */ ($state, CucCloudMessage, projectId) => (
        message = false,
        type = 'success',
      ) => {
        ...
        if (message) {
          promise.then(() =>
            CucCloudMessage[type](message, 'pci.projects.project.streams'),
          );
        }
