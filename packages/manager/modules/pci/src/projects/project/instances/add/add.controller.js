...
export default class PciInstancesAddController {
  ...
  $onInit() {
    ...
    this.selectedSubnet = this.getDefaultSelectValue(
      'pci_projects_project_instances_add_private_network_select_subnet_label',
    );
  ...
  trackCreate() {
    ...
    this.trackAddInstance(
      ...
      }_${this.selectedPrivateNetwork.name}`,
    );
  }
  ...
  isNetworkExists() {
    return this.availablePrivateNetworks.some(
      (network) => network.name === this.privateNetworkModel.name,
    );
  }

  onAddPrivateNetworkClick() {
    this.trackAddInstance(`add::create-private-network`);
    this.showAddPrivateNetworkModalForm = true;
    if (this.showAddPrivateNetworkModalForm) {
      this.trackAddInstance(`add::create-private-network`);
    }
    this.privateNetworkModel.name = getAutoGeneratedName(
      `privnet-${this.model.datacenter.name?.toLowerCase()}`,
    );
  }

  onAddPrivateNetworkFormSubmit() {
    this.trackAddInstance(`add::create-private-network::confirm`);
    this.privateNetworkModel = {
      id: '',
    ...
    if (!this.isNetworkExists()) {
      this.availablePrivateNetworks.push(this.privateNetworkModel);
    }
  ...
  create() {
    ...
    return this.PciProjectsProjectInstanceService.save(
      ...
      .then(({ id: instanceId }) => {
        ...
        return this.checkInstanceCreationStatus(
          this.projectId,
          instanceId,
          INSTANCE_ACTIVE_STATUS,
        ).then(({ ipAddresses: ips }) =>
          this.onSaveInstanceSuccess(instanceId, ips, message),
        );
      })
      .catch((error) => {
  ...
  checkInstanceCreationStatus(serviceName, instanceId, status) {
    return this.Poller.poll(
      ...
      null,
      {
        ...
        namespace: 'instance-creation',
      },
    );
  }

  onSaveInstanceSuccess(instanceId, ips, message) {
  ...
  createAndAttachFloatingIp(instanceId, ip, message) {
    ...
    this.floatingIpModel = {
      gateway: {
        model: this.subnetGateways[0].model,
        name: this.subnetGateways[0].name,
      },
      ip,
    };
  ...
  cancel() {
    this.trackAddInstance(`instances_add_cancel_creation`, false);
    return this.goBack();
  }
