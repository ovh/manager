  ...
  $stateProvider.state('pci.projects.project.serving.add', {
    ...
    resolve: {
      breadcrumb: /* @ngInject */ ($translate) =>
        $translate.instant('pci_projects_project_serving_add_title'),
      cancelLink: /* @ngInject */ ($state, projectId) =>
        $state.href('pci.projects.project.serving', {
          projectId,
        }),
      regions: /* @ngInject */ (ovhManagerPciServingAdd, projectId) =>
          ...
          .then(({ name, id }) => {
            ...
            promise.then(() =>
              CucCloudMessage.success(
                $translate.instant('pci_projects_project_serving_add_success', {
                  name,
                }),
                'pci.projects.project.serving.namespace.infos',
              ),
