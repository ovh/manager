  ...
  $stateProvider
    .state('pci.projects.project.storages.objects', {
      ...
      redirectTo: (transition) => transition
        .injector()
        .getAsync('containers')
        .then((containers) => (containers.length === 0 ? { state: 'pci.projects.project.storages.objects.onboarding' } : false)),
      resolve: {
        archive: () => false,
        containers: /* @ngInject */ (
        ...
        breadcrumb: /* @ngInject */ ($translate) => $translate.instant('pci_projects_project_storages_containers_object_title'),
      },
    });
};
