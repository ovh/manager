...
const moduleName =
  'ovhManagerPciProjectStoragesVolumeBackupListCreateVolumeLazyloading';
angular
  .module(moduleName, ['ui.router', 'ovhManagerCore', 'oc.lazyLoad'])
  .config(
    /* @ngInject */ ($stateProvider) => {
      $stateProvider.state(
        `${VOLUME_BACKUP_ROUTES.LIST.ROUTES.CREATE_VOLUME.STATE}.**`,
        {
          url: VOLUME_BACKUP_ROUTES.LIST.ROUTES.CREATE_VOLUME.URL,
          lazyLoad: ($transition$) => {
            const $ocLazyLoad = $transition$.injector().get('$ocLazyLoad');

            return import('./create-volume.module').then((mod) =>
