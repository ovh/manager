export default /* @ngInject */ ($stateProvider) => {
  $stateProvider.state('pci.projects.project.analytics-data-platform.details', {
    ...
    redirectTo: (transition) =>
      transition
        ...
        .then((serviceName) => {
          ...
          return analyticsDataPlatformService
            .getAnalyticsDataPlatformDetails(serviceName)
            .then((platformDetails) =>
              analyticsDataPlatformService.isDeploymentInProgress(
                platformDetails,
              )
                ? {
                    state:
    ...
    resolve: {
      ...
      goToDetailsPage: ($state, CucCloudMessage, projectId, serviceName) => (
        message = false,
        type = 'success',
      ) => {
      ...
      goToPublicCloudPage: ($state, CucCloudMessage, projectId) => (
        message = false,
        type = 'success',
      ) => {
        const reload = message && type === 'success';
        ...
        if (message) {
          promise.then(() =>
            CucCloudMessage[type](message, 'pci.projects.project'),
          );
        }
        return promise;
      },

      breadcrumb: /* @ngInject */ (platformDetails) =>
        platformDetails.clusterName,
    },
  });
};
