  ...
  $stateProvider.state(
    'pci.projects.project.analytics-data-platform.details.progress',
    {
      ...
      redirectTo: (transition) =>
        transition
          ...
          .then((platformDetails) => {
            const analyticsDataPlatformService = transition
              .injector()
              .get('analyticsDataPlatformService');
            return analyticsDataPlatformService.isDeploymentInProgress(
              platformDetails,
            )
              ? false
              : {
      ...
      resolve: {
        ...
        /* @ngInject */
        accountDetails: (analyticsDataPlatformService) =>
          analyticsDataPlatformService.getAccountDetails(),

        progress: /* @ngInject */ (analyticsDataPlatformService, serviceName) =>
          analyticsDataPlatformService.getStatus(serviceName),

        breadcrumb: /* @ngInject */ ($translate) =>
          $translate.instant('analytics_data_platform_tracking_progress_title'),
      },
    },
  );
};
