...
export default /* @ngInject */ ($stateProvider) => {
  $stateProvider.state('pci.projects.new', {
    ...
    redirectTo: (transition) => {
      const newProjectInfoPromise = transition
        .injector()
        .getAsync('newProjectInfo');
      return newProjectInfoPromise.then(({ error }) => {
    ...
    resolve: {
      ...
      contracts: /* @ngInject */ ($q, newProjectInfo, PciProjectNewService) => {
        const agreementPromises = map(newProjectInfo.agreements || [], (id) =>
          PciProjectNewService.getNewProjectAgreementContract(id).then(
            (contract) =>
      ...
      paymentStatus: /* @ngInject */ ($transition$, getStepByName) => () =>
        get($transition$.params(), 'paymentStatus') ||
      ...
      getStepByName: /* @ngInject */ (steps) => (stepName) =>
        find(steps, {
          name: stepName,
        }),

      getStateLink: /* @ngInject */ ($state, getCurrentStep) => (
        action,
        inherit = true,
      ) => {
        switch (action) {
      ...
      shouldProcessChallenge: /* @ngInject */ (
        ...
        const isValidDefaultPaymentMethod =
          has(currentStep.model, 'defaultPaymentMethod') &&
          includes(
            CHALLENGE_PAYMENT_TYPE_SUPPORTED,
            get(currentStep.model, 'defaultPaymentMethod.paymentType'),
          );
        const shouldProcessChallenge =
          get(newProjectInfo, 'error.code') ===
            'challengePaymentMethodRequested' && isValidDefaultPaymentMethod;
      ...
      hasCreditToOrder: /* @ngInject */ (
        ...
        const hasCreditToOrder =
          newProjectInfo.order &&
      ...
      dlpStatus: /* @ngInject */ ($q, PciProjectNewService) =>
        PciProjectNewService.getDlpStatus().catch((error) => {
          if (error.status === 404) {
            return null;
          }
      ...
      project: /* @ngInject */ ($q, $transition$, PciProjectNewService) => {
        if ($transition$.params().projectId) {
          return PciProjectNewService.getProject(
      ...
      newProjectInfo: /* @ngInject */ (
        ...
        if (['success', 'accepted'].includes(paymentStatus())) {
          ...
          const checkValidPaymentMethod = (iteration = 0) =>
            $timeout(
              ...
              return PciProjectNewService.getNewProjectInfo().then(
                newProjectInfoThen,
              );
            });
      ...
      onDescriptionStepFormSubmit: /* @ngInject */ ($state, $window) => (
        navigateToUrl,
        url,
      ) => {
        if (navigateToUrl) {
      ...
      onProjectCreated: /* @ngInject */ ($state) => (projectId) =>
      ...
      trackingPage: /* @ngInject */ ($transition$) =>
        `public-cloud::${$transition$.to().name.replace(/\./g, '::')}`,
    },
  });
};
