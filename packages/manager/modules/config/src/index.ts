    ...
    .catch((err: unknown) => {
      ...
      if (status === 403) {
        const responseData = data as {
          region?: string;
          applications?: { restricted?: { publicURL?: string } };
        };
        const region = responseData?.region || RESTRICTED_DEFAULTS.region;
        const publicURL =
          responseData?.applications?.restricted?.publicURL || RESTRICTED_DEFAULTS.publicURL;
        if (window.top) {
      ...
      if (status && status >= 500) {
        ...
        Object.assign(errorObj, {
          error: {
            message: (data as string) || errorResponse?.response?.statusText,
          },
          environment,
        });
        throw errorObj;
