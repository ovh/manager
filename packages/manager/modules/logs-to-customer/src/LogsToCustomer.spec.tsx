...
describe('LogsToCustomer module', () => {
  ...
  it('should render an empty state when there is no log kinds', async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.getByText('log_kind_empty_state_description'),
        ).toBeDefined(),
      {
        timeout: 10_000,
      },
  ...
  it('should not render the log kind select when there is only one log kind', async () => {
    ...
    await waitFor(
      () =>
        expect(
          expect(screen.queryByTestId('logKindSelect')).not.toBeInTheDocument(),
        ),
      { timeout: 10_000 },
    );
    expect(
      screen.queryByText('log_kind_selector_select_label'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(logKindsMock[0].displayName),
    ).not.toBeInTheDocument();
  });
  ...
  it('should render the log kind select when there is 2 or more log kinds', async () => {
    ...
    await waitFor(
      () =>
        expect(
          expect(screen.queryByTestId('logKindSelect')).toBeInTheDocument(),
        ),
      { timeout: 10_000 },
    );
    expect(
    ...
    expect(screen.queryAllByTestId('logKindOption')).toHaveLength(
      logKindsMock.length,
    );
  });
});
