...
export const DATA_STREAM_SUBSCRIPTION_LOADING_TEST_ID =
...
const DataStreamActions = ({ stream }: TDataStreamActions) => {
  ...
  const { data: subscriptions, isPending, error } = useLogSubscriptions(
  ...
  const dataStreamSubscription = getStreamSubscription(
    subscriptions,
    stream.streamId,
  );
