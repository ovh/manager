...
export default class NashaPartitionAddCtrl {
  /* @ngInject */
  constructor($q, $translate, CucCloudMessage, OvhApiDedicatedNasha) {
    this.$q = $q;
  ...
  $onInit() {
    ...
    this.$q
      ...
      .then((data) => {
        this.protocols =
          data.schema.models['dedicated.storage.ProtocolEnum'].enum;
  ...
  isPartitionValid() {
    return (
      this.newPartition.partitionName && // Partition name is set
