...
export default class IpLoadBalancerSslCertificateService {
  ...
  getCertificates(serviceName) {
    return this.Ssl.query({ serviceName })
      .$promise.then((sslIds) =>
        this.$q.all(
          sslIds.map((sslId) => this.getCertificate(serviceName, sslId)),
        ),
      )
  ...
  getCertificate(serviceName, sslId) {
    return this.Ssl.get({ serviceName, sslId }).$promise;
  }

  create(serviceName, ssl) {
    return this.Ssl.post({ serviceName }, ssl)
      .$promise.then(() =>
        this.CucServiceHelper.successHandler('iplb_ssl_add_success')(),
      )
  ...
  update(serviceName, sslId, ssl) {
    return this.Ssl.put(
      ...
      .$promise.then(() =>
        this.CucServiceHelper.successHandler('iplb_ssl_update_success')(),
      )
  ...
  delete(serviceName, sslId) {
    return this.Ssl.delete({
      ...
      .$promise.then(() =>
        this.CucServiceHelper.successHandler('iplb_ssl_delete_success')(),
      )
  ...
  getCertificateProducts(serviceName) {
    return this.OvhApiOrder.Cart()
      ...
      .$promise.then((options) =>
        options.filter((option) => option.family === 'ssl'),
      )
      .then((options) =>
        options.map((option) => {
          ...
          set(
            option,
            'prices',
            option.prices.filter((price) => price.interval === 12),
          );
          return option;
        }),
      )
      .catch(
        this.CucServiceHelper.errorHandler('iplb_ssl_order_loading_error'),
      );
  }
  ...
  orderPaidCertificate(serviceName, orderOptions, configuration) {
    ...
    return this.User.v6()
      ...
      .then((item) => {
        ...
        const promises = Object.keys(configuration).map((label) =>
          this.configureCartItem(
            cartId,
            item.itemId,
            label,
            configuration[label],
  ...
  orderFreeCertificate(serviceName, fqdn) {
    return this.OvhApiIpLoadBalancing.v6()
      .freeCertificate({ serviceName }, { fqdn })
      .$promise.then(() =>
        this.CucServiceHelper.successHandler('iplb_ssl_order_success')(),
      )
