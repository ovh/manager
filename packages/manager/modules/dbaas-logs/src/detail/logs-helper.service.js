...
export default class LogsHelperService {
  ...
  pollOperation(serviceName, operation) {
    ...
    return this.CucCloudPoll.poll({
      ...
      stopCondition: (opn) =>
        opn.state === this.DbaasLogsConstants.FAILURE ||
        opn.state === this.DbaasLogsConstants.SUCCESS ||
        opn.state === this.DbaasLogsConstants.REVOKED,
    });
  }
  ...
  handleOperation(serviceName, operation, successMessage, messageData) {
    return this.pollOperation(serviceName, operation).$promise.then(
      (pollResult) => {
        if (
          get(pollResult, '[0].item.state') !== this.DbaasLogsConstants.SUCCESS
        ) {
    ...
    this.guides.list = [
      {
        ...
        url: this.ovhDocUrl.getDocUrl(this.DbaasLogsConstants.LOGS_DOCS_NAME),
        external: true,
      },
    ];
    this.guides.footer = {
      name: this.$translate.instant('logs_guides_footer'),
      url: this.DbaasLogsConstants.guidesUrl,
      external: true,
    };
  }
  ...
  isBasicOffer(account) {
    return !account.offer.reference.startsWith(
      this.DbaasLogsConstants.LOGS_PRO,
    );
  }

  isAccountDisabled(account) {
    return account.state === this.DbaasLogsConstants.SERVICE_STATE_DISABLED;
  }

  accountSetupRequired(account) {
    return account.state === this.DbaasLogsConstants.SERVICE_STATE_TO_CONFIG;
  }
}
