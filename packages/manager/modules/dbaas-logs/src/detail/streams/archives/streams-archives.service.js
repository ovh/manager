...
export default class LogsStreamsArchivesService {
  /* @ngInject */
  constructor($http, $q, DbaasLogsConstants, OvhApiDbaas, CucServiceHelper) {
    this.$http = $http;
    this.$q = $q;
  ...
  getDownloadUrl(serviceName, streamId, archiveId) {
    return this.ArchivesApiService.url({
      ...
      streamId,
      archiveId,
      expirationInSeconds: this.DbaasLogsConstants.expirationInSeconds,
  ...
  transformArchive(archive) {
    set(
      archive,
      'retrievalStateType',
      this.DbaasLogsConstants.stateType[archive.retrievalState],
    );
  }
}
