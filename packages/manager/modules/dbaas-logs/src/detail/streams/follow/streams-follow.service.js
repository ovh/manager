...
export default class LogsStreamsFollowService {
  ...
  constructor(
    ...
    this.testTypeEnum = keyBy([
      this.DbaasLogsConstants.GELF,
      this.DbaasLogsConstants.LTSV,
      this.DbaasLogsConstants.RFC5424,
    ]);
    this.webSocket = null;
    this.messages = [];
  ...
  copyWebSocketAddress(stream) {
    const url = this.CucUrlHelper.constructor.findUrl(
      stream,
      this.DbaasLogsConstants.WEB_SOCKET_URL,
    );
    if (!url) {
  ...
  connectToWebSocket(stream) {
    ...
    if (url) {
      ...
      this.webSocket.onMessage((event) => {
        ...
        if (this.totalMessages === this.DbaasLogsConstants.MESSAGE_THRESHOLD) {
