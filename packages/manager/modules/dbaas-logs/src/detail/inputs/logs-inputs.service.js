...
export default class LogsInputsService {
  ...
  getNewInput() {
    return {
      data: {
        info: {
          exposedPort: this.DbaasLogsConstants.INPUT_DEFAULT_PORT,
        },
      },
      loading: false,
    };
  ...
  getSubscribedOptions(serviceName) {
    return this.LogsOptionsService.getSubscribedOptionsByType(
      serviceName,
      this.DbaasLogsConstants.INPUT_OPTION_REFERENCE,
    );
  }
  ...
  transformInput(input) {
    ...
    const isProcessing =
      input.info.status === this.DbaasLogsConstants.inputStatus.PROCESSING;
    const isToBeConfigured =
      input.info.status === this.DbaasLogsConstants.inputStatus.INIT &&
      !input.actionsMap.START;
    const isPending =
      (input.info.status === this.DbaasLogsConstants.inputStatus.INIT ||
        input.info.status === this.DbaasLogsConstants.inputStatus.PENDING) &&
      input.actionsMap.START;
    const isRunning =
      input.info.status === this.DbaasLogsConstants.inputStatus.RUNNING;
    ...
    set(
      ...
      'info.state',
      isProcessing
        ? this.DbaasLogsConstants.inputState.PROCESSING
        : input.info.isRestartRequired
        ? this.DbaasLogsConstants.inputState.RESTART_REQUIRED
        : isToBeConfigured
        ? this.DbaasLogsConstants.inputState.TO_CONFIGURE
        : isPending
        ? this.DbaasLogsConstants.inputState.PENDING
        : isRunning
        ? this.DbaasLogsConstants.inputState.RUNNING
        : this.DbaasLogsConstants.inputState.UNKNOWN,
    );
    /* eslint-disable no-nested-ternary */
    set(
      input,
      'info.stateType',
      this.DbaasLogsConstants.inputStateType[input.info.state],
    );
    return input;
  }
