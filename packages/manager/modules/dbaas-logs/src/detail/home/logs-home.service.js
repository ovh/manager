...
export default class LogsHomeService {
      ...
      .then((account) => {
        ...
        const query = {
          start: Math.max(
            moment()
              .subtract(
                this.DbaasLogsConstants.DATA_STORAGE.TIME_PERIOD_MONTHS,
                'month',
              )
              .unix() * 1000,
          ...
          queries: [
            {
              metric: this.DbaasLogsConstants.DATA_STORAGE.METRICS.SUM,
              aggregator: this.DbaasLogsConstants.DATA_STORAGE.AGGREGATORS
                .ZIMSUM,
              downsample: this.DbaasLogsConstants.DATA_STORAGE
                .DOWNSAMPLING_MODE['24H_MAX'],
            },
            {
              metric: this.DbaasLogsConstants.DATA_STORAGE.METRICS.COUNT,
              aggregator: this.DbaasLogsConstants.DATA_STORAGE.AGGREGATORS
                .ZIMSUM,
              downsample: this.DbaasLogsConstants.DATA_STORAGE
                .DOWNSAMPLING_MODE['24H_MAX'],
  ...
  getColdstorage(serviceName) {
    return this.getAccount(serviceName).then((account) => {
      ...
      return this.$http({
        ...
        params: {
          timeseries: this.DbaasLogsConstants.DATA_STORAGE.METRICS
            .COLD_STORAGE_TOTAL,
        },
        headers: {
          Authorization: `Basic ${token}`,
  ...
  getElasticSearchApiUrl(object) {
    const elasticSearchApiUrl = this.constructor.findUrl(
      object.urls,
      this.DbaasLogsConstants.URLS.ELASTICSEARCH_API,
    );
    set(
      object,
      'elasticSearchApiUrl',
  ...
  getGrayLogApiUrl(object) {
    set(
      ...
      this.constructor.findUrl(
        object.urls,
        this.DbaasLogsConstants.URLS.GRAYLOG_API,
      ),
    );
    return object;
  }
  ...
  getGrayLogUrl(object) {
    set(
      ...
      this.constructor.findUrl(
        object.urls,
        this.DbaasLogsConstants.URLS.GRAYLOG_WEBUI,
      ),
    );
    return object;
  }
  ...
  getPortsAndMessages(accountDetails) {
    ...
    accountDetails.urls.forEach((url) => {
      const urlInfo = this.DbaasLogsConstants.URL_TYPES[url.type];
      if (urlInfo) {
        portsAndMessages[urlInfo.PORT] = portsAndMessages[urlInfo.PORT] || {
          name: this.DbaasLogsConstants.PORT_TYPES[urlInfo.PORT],
        };
  ...
  transformAccount(account) {
    if (isEmpty(account.offer)) {
    ...
    } else if (account.offer.reference === this.DbaasLogsConstants.basicOffer) {
      set(
        account,
        'offer.description',
        this.DbaasLogsConstants.offertypes.BASIC,
      );
    } else {
      ...
      set(
        account,
        'offer.description',
        `${this.DbaasLogsConstants.offertypes.PRO} - ${dataVolume}: ${dataVolumeValue}`,
      );
    }
    return account;
  }
