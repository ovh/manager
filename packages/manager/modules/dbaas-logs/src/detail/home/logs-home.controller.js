...
export default class LogsHomeCtrl {
      ...
      loaderFunction: () =>
        this.LogsDetailService.getServiceDetails(this.serviceName).then(
          (service) => {
            ...
            if (
              service.state === this.DbaasLogsConstants.SERVICE_STATE_TO_CONFIG
            ) {
              this.goToAccountSetupPage();
            } else {
              this.dataUsageGraphData = this.DbaasLogsConstants.DATA_USAGE_GRAPH_CONFIGURATION;
  ...
  prepareDataUsageGraphData() {
    const offerLimit =
      ...
      this.DbaasLogsConstants.OFFER_STORAGE_MULTIPLIER;
    ...
    this.dataUsageGraphData.options.scales.yAxes[0].ticks = {
      ...
      callback: (value) =>
        value % 1 === 0 ? this.cucBytesFilter(value, 2, true) : '',
    };
      ...
      label: (tooltipItem, data) => {
        ...
        label +=
          tooltipItem.datasetIndex === 0
            ? this.cucBytesFilter(tooltipItem.yLabel, 2, true)
