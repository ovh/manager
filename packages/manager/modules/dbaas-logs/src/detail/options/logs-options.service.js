...
export default class LogsOptionsService {
  /* @ngInject */
  constructor(
    $translate,
    $window,
  ...
  getOptions(serviceName) {
    return this.OvhApiOrderCartServiceOption.v6()
      .get({
        productName: this.DbaasLogsConstants.productName,
        serviceName,
      })
  ...
  getSubscribedOptionsByType(serviceName, optionType) {
    return this.getSubscribedOptions(serviceName).then((response) => {
      switch (optionType) {
        case this.DbaasLogsConstants.STREAM_OPTION_REFERENCE:
          return this.filterOptions(response.options, 'maxNbStream');
        case this.DbaasLogsConstants.INDEX_OPTION_REFERENCE:
          return this.filterOptions(response.options, 'maxNbIndex');
        case this.DbaasLogsConstants.ALIAS_OPTION_REFERENCE:
          return this.filterOptions(response.options, 'maxNbAlias');
        case this.DbaasLogsConstants.INPUT_OPTION_REFERENCE:
          return this.filterOptions(response.options, 'maxNbInput');
        case this.DbaasLogsConstants.ROLE_OPTION_REFERENCE:
          return this.filterOptions(response.options, 'maxNbRole');
        case this.DbaasLogsConstants.DASHBOARD_OPTION_REFERENCE:
    ...
    return this.getSubscribedOptionsMap(serviceName).then((options) => {
      const groupedOptionsMap = options.reduce((groupedOptions, option) => {
        ...
        groupedOptions[option.type].quantity +=
          option.quantity * this.DbaasLogsConstants.PRODUCT_COUNT[option.id];
  ...
  transformOptionForOrder(option, serviceName) {
    return {
      ...
      serviceName,
      productId: this.DbaasLogsConstants.productName,
    };
  }
