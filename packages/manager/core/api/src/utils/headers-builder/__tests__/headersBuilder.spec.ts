...
import { getIcebergFilter, buildHeaders, appendIamTags } from '../headersBuilder';
import {
  FilterComparator,
...
describe('getIcebergFilter', () => {
  ...
  it('should handle special characters in values', () => {
    const filter = getIcebergFilter(FilterComparator.IsEqual, 'test@example.com');
    expect(filter).toEqual('eq=test%40example.com');
  });

  it('should handle spaces in values', () => {
    const filter = getIcebergFilter(FilterComparator.Includes, 'test value');
    expect(filter).toEqual('like=%25test%20value%25');
  });

  it('should handle empty string values', () => {
    const filter = getIcebergFilter(FilterComparator.IsEqual, '');
    expect(filter).toEqual('eq=');
  });

  it('should handle numeric values as strings', () => {
    const filter = getIcebergFilter(FilterComparator.IsEqual, '123');
    expect(filter).toEqual('eq=123');
  });

  it('should throw error for unsupported comparator', () => {
    expect(() => {
      getIcebergFilter('unsupported' as FilterComparator, 'test');
