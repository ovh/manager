...
const toOperationList =
  ...
  ({ path, description, operations }) =>
    operations.map(
      ({ httpMethod, parameters, responseType, description: operationDescription }) => {
        ...
        return {
          ...
          params: parameters.map(({ name, fullType, required, description: paramDescription }) => {
            const paramType = fullType ? getTypeFromString(fullType) : 'unknown';
            const paramName =
              name || `${paramType[0].toLowerCase()}${paramType.substring(1).toLowerCase()}Custom`;
          ...
          unknownTypeList: new Set(
            [tsResponseType, ...parameters.map(({ fullType }) => getTypeFromString(fullType))]
...
const setUrlAndBodyParams = (operation) => {
  const urlParamNameList =
    operation.apiPath.match(/{[a-zA-Z]+}/g)?.map((param) => param.replace(/[{}]/g, '')) || [];
  const urlParams = urlParamNameList.map((urlParamName) =>
    operation.params.find(({ name }) => name === urlParamName),
  );
  const bodyParams = operation.params.filter(({ name }) => !urlParamNameList.includes(name));
  return {
    ...operation,
...
const groupOperationsByHttpMethod = (groupedByMethod, operation) => {
  ...
  return {
    ...groupedByMethod,
    [operation.httpMethod]: {
      operationList: [...groupedByMethod[operation.httpMethod].operationList, operation],
...
export const getApiTemplateData = async (apiPaths) => {
  const results = await Promise.all(apiPaths.map((path) => getApiServiceOperations(path)));
