...
- 🔄 **Multi-version support**: Works with Iceberg, API v2, and API v6
- 📦 **Automatic routing**: Selects the correct adapter based on configuration
- ⚡ **React Query integration**: Built on React Query for efficient caching and state management
- 🔍 **Search capabilities**: Available with Iceberg adapter
- 🎯 **Filter management**: Available with Iceberg adapter
- 📊 **Sorting support**: Available with Iceberg and v6 adapters
- 📥 **Pagination**: Supports infinite queries and fetching all data at once
- 🔄 **Auto-refetch**: Configurable refetch intervals for v6 adapter
...
- 🔄 When using Iceberg adapter, search and filter capabilities are available for columns marked with `isSearchable` and `isFilterable`
- 📥 The `fetchAll` option will automatically fetch all available data, bypassing pagination
- 🔍 Search functionality is only available with the Iceberg adapter
- 🎯 Filter functionality is only available with the Iceberg adapter
- 📊 Sorting is available with both Iceberg and v6 adapters
- 🔄 The `refetchInterval` option is only available with the v6 adapter
- 📦 The `fetchDataFn` option is only available with the v6 adapter and allows you to provide a custom data fetching function
