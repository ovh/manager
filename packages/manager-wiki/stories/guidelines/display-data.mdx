...
Use the `Price` component from `@ovh-ux/muk` for displaying prices, [here](?path=/docs/manager-ui-kit-components-price--technical-information) the documentation.
...
Use the `useFormatDate` hook from `@ovh-ux/muk` for displaying dates.
...
Use the `Storage` component from `@ovh-ux/muk` for displaying storage values:

```tsx
import { Storage } from '@ovh-ux/muk';
...
```tsx
import { Skeleton } from from '@ovhcloud/ods-react';
...
- [ ] Use appropriate components from `@ovh-ux/muk`
