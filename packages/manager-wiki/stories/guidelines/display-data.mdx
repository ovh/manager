...
```tsx
import { ErrorMessage } from '@ovh-ux/manager-react-components';

// Error state for price
<ErrorMessage message="Unable to load price" />

// Error state for date
<ErrorMessage message="Invalid date format" />

// Error state for storage
<ErrorMessage message="Storage value unavailable" />
```
