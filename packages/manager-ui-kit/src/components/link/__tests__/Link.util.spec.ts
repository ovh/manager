...
describe('generateSurveyLink', () => {
  it('should generate a survey link with just applicationKey (no lang)', () => {
    ...
    expect(result).toBe(`${LINK_SURVEY_BASE_URL}/COM_evaluation_survey?email=test%40example.com`);
  });
  ...
  it('should generate a survey link with productId', () => {
    ...
    expect(result).toBe(
      `${LINK_SURVEY_BASE_URL}/COM_evaluation_survey?email=test%40example.com&productId=1234567890`,
    );
  });
  ...
  it('should generate a survey link with uppercase language suffix when lang is provided and not "fr"', () => {
    ...
    expect(result).toBe(
      `${LINK_SURVEY_BASE_URL}/COM_evaluation_survey_DE?email=test%40example.com`,
    );
  });
  ...
  it('should handle different application keys', () => {
    ...
    expect(result).toBe(
      `${LINK_SURVEY_BASE_URL}/PCI_evaluation_survey_DE?email=test%40example.com&nichandle=test&productId=1234567890`,
    );
  });
});
