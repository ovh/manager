...
import { UseQueryOptions } from 'src/hooks/data-api/adapters/use-query';
...
describe('useV6', () => {
  ...
  it('forwards enabled', () => {
    ...
    expect((callArg as UseQueryOptions).enabled).toBeFalsy();
  });

  it('forwards the refreshInterval', () => {
    renderUseV6Hook({ refetchInterval: 1000 });
    const callArg = vi.mocked(useQuery).mock.calls[0]?.[0];
    expect((callArg as UseQueryOptions).refetchInterval).toBe(1000);
  });
