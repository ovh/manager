...
function fetchNextPage(fn, numberOfTimes) {
  for (let i = 0; i < numberOfTimes; i++) {
    fn();
  }
}

function renderUseV6Hook<T>(hookParams: Partial<ResourcesV6Params<T>> = {}) {
  return renderHook(
    () =>
      useV6({
        columns,
...
describe('useV6', () => {
  ...
  // it('fetches data with custom queryFn', () => {
  //   const queryFn = vi.fn();
  //   renderUseV6Hook({ queryFn });
  //   expect(queryFn).toHaveBeenCalled();
  // });

  it('forwards enabled', () => {
    renderUseV6Hook({ enabled: false });
    const [[callArg]] = vi.mocked(useQuery).mock.calls;
    expect(callArg.enabled).toBeFalsy();
  });

  it('forwards the refreshInterval', () => {
    renderUseV6Hook({ refetchInterval: 1000 });
    const [[callArg]] = vi.mocked(useQuery).mock.calls;
    expect(callArg.refetchInterval).toBe(1000);
  });
  ...
  it('applies default sorting', () => {
    ...
    result.current.flattenData.forEach((item, index) => {
  ...
  it('applies searching with onSearch', () => {
    ...
    result.current.flattenData.forEach((item) => {
