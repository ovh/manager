...
export const useV2 = <TData = Record<string, unknown>>({
  ...
  >({
    ...
    select: (queryFnResponse: InfiniteData<FetchV2Result<TData>>): SelectData<TData> => {
      const { totalCount } = queryFnResponse.pages[0] as FetchV2Result<TData>;
      return {
        data: queryFnResponse,
        totalCount,
        flattenData: queryFnResponse.pages.flatMap((page) => page.data),
      };
    },
  });
