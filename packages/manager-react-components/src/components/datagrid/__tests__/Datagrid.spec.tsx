...
describe('Datagrid', () => {
  ...
  describe('Sorting', () => {
    it('should handle sorting with onSortChange', () => {
      render(
        <Datagrid
          columns={mockBasicColumns}
          data={mockData}
  ...
  describe('Footer & Results', () => {
    it('should render footer with items count and total count', () => {
      ...
      expect(screen.getByText('2 of 100 results')).toBeInTheDocument();
    });

    it('should render footer with only items count when totalCount is not provided', () => {
      render(<Datagrid columns={mockBasicColumns} data={mockData} />);
