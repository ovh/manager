...
export interface DatagridProps<T> {
  ...
  renderSubComponent?: (
    row: Row<T>,
    headerRefs?: React.MutableRefObject<Record<string, unknown>>,
  ) => JSX.Element;
  /** function to define if row can be expanded or not */
  getRowCanExpand?: (row: Row<T>) => boolean;
  /** Hide datagrid header if true */
  hideHeader?: boolean;
  /** Function datagrid will perform when items change */
...
export const Datagrid = <T,>({
  ...
  useEffect(() => {
    resetExpandedRowsOnItemsChange && table.resetExpanded();
  }, [items]);
