...
- adding more complexe filters through a Menu (Button + Popover) → ce composant pourrait exister côté MRC peut-être ? réutilisable 3 fois, just an idea (smile)
- managing filters dynamically, including removing tags as needed
...
5. **Actions** including two buttons (is this part only functional?):
   1. **An ODS Select**: for choosing the filed to filter on (e.g., column)
...
- aria-expanded for the Menu (on its trigger) ?
