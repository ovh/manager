...
const stackItems = function stackItems<I, U>(
  ...
  if (cb) {
    ...
    uniques.forEach((unique) => {
      const key = cb(unique);
      stacks.set(key, []);
      const stackItems = stacks.get(key);
      if (stackItems) {
        stackItems.push(...items.filter((item) => isEqual(key, cb(item))));
      }
    });
  } else {
...
>({
  ...
  useEffect(() => {
    if (stack && state.selectedStack !== undefined && value) {
      const stackItems = state.stacks.get(state.selectedStack);
      if (
        stackItems?.length &&
        !isEqual(state.selectedStack, stack.by(value))
      ) {
        set.value(stackItems[0]);
      }
    }
      ...
      <ul className="simple-tiles-input-ul grid gap-6 list-none p-6 m-0 grid-cols-1 md:grid-cols-3">
        {stack
          ? [...state.stacks.keys()].map((key) => {
              const stackItems = state.stacks.get(key);
              ...
              return (
                <li className="w-full px-1" key={hashCode(key)}>
                  ...
                  >
                    {is.stack.singleton(key)
                      ? label(stackItems[0])
                      : key !== undefined && stack?.label(key, stackItems)}
                  </OdsCard>
                </li>
              );
            })
