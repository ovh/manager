...
<Meta title="Templates/Delete Modal/Documentation" />
...
- Feedback for loading states to prevent duplicate actions or interruptions?
...
- **Buttons**
  ...
  - Cancel: dismisses the modal without performing any action (pas géré par MRC?)
  - Confirm/Delete: Executes the delete action only when enabled (pas géré par MRC?)
