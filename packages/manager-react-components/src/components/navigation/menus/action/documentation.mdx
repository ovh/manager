...
<Meta title="Navigation/Menus/Documentation" />
...
1. **an ODS Button**: it will trigger the list of action which contains:
   ...
   1. an ODS Icon (can be set to left or right? or always at the left of the label)?
   2. a label

2. **an ODS Popover** which can contain:
   1. **a group** title: it is used to group related actions?
   2. **actions**: each action is an ODS Button or an ODS Link depending of the usage
   3. **a divider**: it can be used to separate some actions?
...
<StorybookBestPractices
  dos={[
    ...
    'Keep Action Menu list of actions content short for abetter and faster understanding',
  ]}
  donts={[
